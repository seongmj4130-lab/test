...
def create_final_metrics_summary():
    ...
    bt20_avg_return = sum([v['총수익률'] for v in bt20_strategies.values()]) / len(bt20_strategies)
    bt20_avg_sharpe = sum([v['Sharpe'] for v in bt20_strategies.values()]) / len(bt20_strategies)
    bt20_avg_hit = sum([v['Hit_Ratio'] for v in bt20_strategies.values()]) / len(bt20_strategies)

    # BT120 그룹
    bt120_strategies = {k: v for k, v in unified_results.items() if 'BT120' in k}
    bt120_avg_return = sum([v['총수익률'] for v in bt120_strategies.values()]) / len(bt120_strategies)
    bt120_avg_sharpe = sum([v['Sharpe'] for v in bt120_strategies.values()]) / len(bt120_strategies)
    bt120_avg_hit = sum([v['Hit_Ratio'] for v in bt120_strategies.values()]) / len(bt120_strategies)
    ...
    # Sharpe 기준 순위
    sorted_by_sharpe = sorted(unified_results.items(), key=lambda x: x[1]['Sharpe'], reverse=True)
