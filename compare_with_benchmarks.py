...
def generate_practical_comparison_report(analysis, benchmarks, market_timing):
    ...
    mt = market_timing
    print(
        f"시장 환경: 상승장 {mt['market_counts']['bull_months']}개월, 하락장 {mt['market_counts']['bear_months']}개월"
    )
    ...
    print(
        f"  강점: 낮은 MDD ({bt120_best['mdd']:.1f}%), 높은 Profit Factor ({bt120_best['profit_factor']:.1f})"
    )
    ...
    print(
        f"  개선점: CAGR 목표 {quant_avg['annual_return']:.1f}% 대비 {bt20_ens_best['cagr']:.1f}%"
    )
