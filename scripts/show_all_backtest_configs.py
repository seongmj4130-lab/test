...
bt_config_fields = {f.name: getattr(default_cfg, f.name) for f in fields(BacktestConfig)}
...
for field in fields(BacktestConfig):
    field_type = str(field.type).replace('<class \'', '').replace('\'>', '').replace('typing.', '')
...
for strategy_name, config_key in strategies.items():
    ...
    for config_key_name, attr_name in basic_configs:
        ...
        if config_val is not None:
            ...
            print(f"  {config_key_name:<35} = {config_val_str:<30} (기본값: {default_val}){mark}")
        else:
    ...
    for config_key_name, attr_name in tranche_configs:
        ...
        if config_val is not None:
            mark = " ⚠️ (기본값과 다름)" if config_val != default_val else " ✅"
            config_val_str = f'"{config_val}"' if isinstance(config_val, str) else str(config_val)
            default_val_str = f'"{default_val}"' if isinstance(default_val, str) else str(default_val)
            print(f"  {config_key_name:<35} = {config_val_str:<30} (기본값: {default_val_str}){mark}")
        else:
            default_val_str = f'"{default_val}"' if isinstance(default_val, str) else str(default_val)
            print(f"  {config_key_name:<35} = {default_val_str:<30} (기본값 사용) ✅")
    ...
    for config_key_name, attr_name in regime_configs:
        ...
        if attr_name is None:
            ...
            if config_val is not None:
                config_val_str = f'"{config_val}"' if isinstance(config_val, str) else str(config_val)
                print(f"  {config_key_name:<35} = {config_val_str:<30} (BacktestConfig에 없음)")
            else:
