...
if csv_path.exists():
    ...
    for strategy, metrics in actual_results.items():
        ...
        monthly_return_pct = (
            (1 + total_return_pct / 100) ** (1 / total_months) - 1
        ) * 100

        monthly_returns[strategy] = monthly_return_pct

        print(
            f"{strategy}: 월별 평균 수익률 {monthly_return_pct:.3f}% (총수익률 {total_return_pct}%)"
        )
    ...
    print("\n목표 vs 실제 비교:")
    targets = {
        "short_cum_return_pct": 4.90,
        "long_cum_return_pct": 2.88,
        "mix_cum_return_pct": 4.03,
    }
    ...
    for col, target in targets.items():
        ...
        print(
            f"{col}: 목표 {target:.2f}% vs 실제 {actual:.2f}% (차이: {diff:.2f}%) - {status}"
        )
