...
def compare_before_after(baseline: dict, improved: dict) -> dict:
    ...
    comparison = {
        ...
        "ic_improvement_pct": (
            ((improved["avg_ic"] - baseline["avg_ic"]) / abs(baseline["avg_ic"])) * 100
            if baseline["avg_ic"] != 0
            else 0
        ),
        "hit_ratio_improvement": improved["avg_hit_ratio"] - baseline["avg_hit_ratio"],
        "hit_ratio_improvement_pct": (
            (
                (improved["avg_hit_ratio"] - baseline["avg_hit_ratio"])
                / baseline["avg_hit_ratio"]
            )
            * 100
            if baseline["avg_hit_ratio"] != 0
            else 0
        ),
...
def generate_improvement_report(
    baseline: dict, improved: dict, comparison: dict
) -> str:
    """
    개선 효과 보고서 생성
    ...
    report.append("|------|--------|--------|--------|--------|")
    report.append(
        "| 평균 IC | {:.4f} | {:.4f} | {:.4f} | {:.1f}% |".format(
            baseline["avg_ic"],
            improved["avg_ic"],
            comparison["ic_improvement"],
            comparison["ic_improvement_pct"],
        )
    )
    report.append(
        "| 평균 Hit Ratio | {:.1%} | {:.1%} | {:.1%} | {:.1f}% |".format(
            baseline["avg_hit_ratio"],
            improved["avg_hit_ratio"],
            comparison["hit_ratio_improvement"],
            comparison["hit_ratio_improvement_pct"],
        )
    )
    report.append("")
    ...
    if baseline.get("feature_details") and improved.get("feature_details"):
        ...
        # 공통 피쳐들 비교
        common_features = set(baseline_df["feature_name"]) & set(
            improved_df["feature_name"]
        )
        if common_features:
            ...
            for feature in list(common_features)[:10]:  # 상위 10개만
                baseline_ic = baseline_df[baseline_df["feature_name"] == feature][
                    "holdout_ic_mean"
                ].iloc[0]
                improved_ic = improved_df[improved_df["feature_name"] == feature][
                    "holdout_ic_mean"
                ].iloc[0]
        ...
        # 새로운 피쳐들의 성과
        new_features = set(improved_df["feature_name"]) - set(
            baseline_df["feature_name"]
        )
        if new_features:
            ...
            new_feature_results = improved_df[
                improved_df["feature_name"].isin(new_features)
            ]
    ...
    if comparison["ic_improvement"] > 0:
        ...
        report.append(
            "- IC 평균: {:.1f}% 개선".format(comparison["ic_improvement_pct"])
        )
        report.append(
            "- Hit Ratio: {:.1f}% 개선".format(comparison["hit_ratio_improvement_pct"])
        )
        report.append("")
        report.append("**추천 사항**:")
