...
def main():
    ...
    print(
        f"⚠️ 결측치 행 수: {len(missing_rows)}/{len(df)} ({len(missing_rows)/len(df)*100:.1f}%)"
    )
    ...
    for idx, row in sample_missing.iterrows():
        ...
        print(
            f"  행 {idx}: 결측 컬럼 {len(missing_in_row)}개 - {missing_in_row[:3]}{'...' if len(missing_in_row) > 3 else ''}"
        )
    ...
    if final_missing == 0:
        ...
        print("\n🔍 추가 검증:")
        print(
            f"  score_ens 범위: {df_fixed['score_ens'].min():.4f} ~ {df_fixed['score_ens'].max():.4f}"
        )
