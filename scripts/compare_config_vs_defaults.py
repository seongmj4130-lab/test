...
config_attrs = [
    ...
    "risk_scaling_bear_multiplier",
    "risk_scaling_neutral_multiplier",
    "risk_scaling_bull_multiplier",
...
for strategy_name, config_key in strategies.items():
    ...
    for attr in config_attrs:
        ...
        if attr == "regime_enabled":
            ...
            config_val = l7_cfg.get(
                "softmax_temperature", l7_cfg.get("softmax_temp", None)
            )
        else:
    ...
    if differences:
        ...
        for diff in differences:
            print(
                f"    - {diff['attr']:35s}: 기본값={diff['default']!s:15s} | config.yaml={str(diff['config']):15s}"
            )
    else:
        print(
            "  ✅ 모든 설정값이 BacktestConfig 기본값과 동일 (또는 config.yaml에 명시되지 않음)"
        )
...
if total_diffs:
    ...
    for attr, strategy_diffs in total_diffs.items():
        ...
        for strategy_name, diff in strategy_diffs.items():
            print(
                f"      {strategy_name}: {diff['config']} (기본값 {diff['default']}에서 변경)"
            )
else:
    print(
