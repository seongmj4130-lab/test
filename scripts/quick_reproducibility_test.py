...
bt_files = ['bt_metrics_bt20_ens.parquet', 'bt_metrics_bt20_short.parquet', 'bt_metrics_bt120_ens.parquet', 'bt_metrics_bt120_long.parquet']
...
for run_id in range(1, 4):
    ...
    # 백테스트 실행
    result = subprocess.run([sys.executable, 'scripts/run_backtest_4models.py'],
...
# 결과 분석
print('\n📊 재현성 분석')
for strategy in ['bt_metrics_bt20_ens', 'bt_metrics_bt20_short', 'bt_metrics_bt120_ens', 'bt_metrics_bt120_long']:
    print(f'\n🎯 {strategy}')
    sharpes = []
    for run_id in range(1, 4):
    ...
    if len(sharpes) == 3:
        ...
        cv = std / abs(pd.Series(sharpes).mean()) if pd.Series(sharpes).mean() != 0 else 0
        print(f'  표준편차: {std:.6f}')
        print(f'  변동계수: {cv:.6f}')
