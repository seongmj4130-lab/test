...
def create_summary_report(df):
    ...
    print("\n📊 기간별 평균 성과:")
    period_avg = (
        df.groupby("holding_days")[["sharpe", "cagr(%)", "mdd(%)"]].mean().round(3)
    )
    print(period_avg)
    ...
    pivot_sharpe = df.pivot(
        index="holding_days", columns="strategy", values="sharpe"
    ).round(3)
    print(pivot_sharpe)
...
def save_final_report(df):
    ...
    output_file = (
        project_root / "results" / f"final_18_cases_backtest_report_{timestamp}.csv"
    )
