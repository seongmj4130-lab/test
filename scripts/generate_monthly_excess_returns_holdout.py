...
def load_data():
    ...
    monthly_returns_path = (
        base_dir / "results" / "monthly_cumulative_returns_holDOUT.csv"
    )
...
def process_monthly_data(monthly_df, kospi_df):
    ...
    for _, row in kospi_df.iterrows():
        ...
        kospi_monthly.append({"month": month, "kospi200_cumulative_pct": kospi_value})
    ...
    for _, strategy_info in unique_strategies.iterrows():
        ...
        merged_df["strategy_cumulative_return"] = (
            merged_df["cumulative_return_pct"] / 100
        )
        merged_df["kospi_cumulative_return"] = (
            merged_df["kospi200_cumulative_pct"] / 100
        )

        # 활성 수익률 = 전략 수익률 / KOSPI 수익률
        merged_df["active_equity"] = (1 + merged_df["strategy_cumulative_return"]) / (
            1 + merged_df["kospi_cumulative_return"]
        )
...
def calculate_kpi_metrics(monthly_df, kospi_df):
    ...
    # 성과 지표 데이터 로드
    perf_path = (
        Path(__file__).parent / "results" / "performance_metrics_basic_holDOUT.csv"
    )
...
def create_ui_format_data(monthly_excess_df, kpi_df):
    ...
    for holding_days in [20, 40, 60, 80, 100, 120]:
        ...
        # 월별 시계열 데이터
        series_data = []
        trench_data = monthly_excess_df[
            monthly_excess_df["holding_days"] == holding_days
        ]
            ...
            series_data.append(
                {
                    ...
                    "short_excess_pct": (
                        ...
                        if row["strategy_type"] == "short"
                        else None
                    ),
                    "long_excess_pct": (
                        row["excess_return_pct"]
                        if row["strategy_type"] == "long"
                        else None
                    ),
                    "mix_excess_pct": (
                        row["excess_return_pct"]
                        if row["strategy_type"] == "ens"
                        else None
                    ),
                }
            )
...
def main():
    ...
    for holding_days in [20, 40, 60, 80, 100, 120]:
        ...
        for _, row in trench_data.iterrows():
            ...
            print(
                f"  {strategy_name}: 초과수익률 {row['final_excess_pct']:.2f}%, Sharpe {row['sharpe']:.3f}, MDD {row['mdd_pct']:.2f}%"
            )
