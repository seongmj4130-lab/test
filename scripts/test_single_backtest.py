...
def test_single_backtest():
    ...
    backtest_cfg = BacktestConfig(
        ...
        tranche_allocation_mode=strategy_config.get(
        ...
        max_names_per_group=strategy_config.get("diversify", {}).get(
    ...
    print(
        f"  overlapping_tranches_enabled: {backtest_cfg.overlapping_tranches_enabled}"
    )
    ...
    try:
        (
            ...
            rebalance_scores=rebalance_scores, cfg=backtest_cfg, market_regime=None
        )
        ...
        if len(metrics_df) > 0:
            ...
            for _, row in metrics_df.iterrows():
                ...
                print(
                    f"  {phase.upper()}: Sharpe={sharpe}, CAGR={cagr}, MDD={mdd}, Total Return={total_return}"
                )
