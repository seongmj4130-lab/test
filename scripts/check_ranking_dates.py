...
if rebalance_scores_path.exists():
    ...
    if ranking_short_path.exists():
        ...
        print(f"  rebalance_scores가 ranking의 부분집합인가? {rebalance_dates.issubset(ranking_dates)}")
        print(f"  rebalance_scores에만 있는 날짜: {len(rebalance_dates - ranking_dates)}개")
