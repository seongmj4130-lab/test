...
for model in models:
    # 백업 파일 찾기
    backup_file = backup_dir / latest_backup.name.replace(
        "bt_metrics_bt20_ens", f"bt_metrics_{model}"
    )
    ...
    # 현재 파일
    current_file = (
        base_dir / "artifacts" / "reports" / f"bt_metrics_{model}_optimized.csv"
    )
    ...
    for phase in ["dev", "holdout"]:
        ...
        for metric in ["net_sharpe", "net_cagr", "net_mdd", "net_calmar_ratio"]:
            ...
            improvement = (
                "✅"
                if (change > 0 and metric != "net_mdd")
                or (change < 0 and metric == "net_mdd")
                else "❌"
            )

            print(
                f"    {metric_name:20s}: {before_val:8.4f} -> {after_val:8.4f} ({change:+.4f}, {change_pct:+.2f}%) {improvement}"
            )
    print()
