...
print("=" * 100)
print("백테스트 결과 요약 (리팩토링 후 - config.yaml 우선 적용)")
print("=" * 100)
print()
...
for _, row in holdout.iterrows():
    print(f"{row['strategy']:15s} | Sharpe: {row['net_sharpe']:7.4f} | CAGR: {row['net_cagr']:7.4f} | MDD: {row['net_mdd']:7.4f} | Calmar: {row['net_calmar_ratio']:7.4f}")
...
# CSV로 저장
output_path = base_dir / 'artifacts' / 'reports' / 'backtest_results_refactored.csv'
