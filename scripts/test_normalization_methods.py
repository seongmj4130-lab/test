...
def test_normalization_method(method: str, cfg: dict, artifacts: dict):
    ...
    save_artifact(
        outputs_short["ranking_short_daily"],
        interim_dir / f"ranking_short_daily_{method}",
        force=True,
    )
    save_artifact(
        outputs_long["ranking_long_daily"],
        interim_dir / f"ranking_long_daily_{method}",
        force=True,
    )
    ...
    # 단기 랭킹 Hit Ratio
    hr_short = (
        calculate_hit_ratio(rebalance_scores, "true_short", "score_total_short")
        if "score_total_short" in rebalance_scores.columns
        else None
    )

    # 장기 랭킹 Hit Ratio
    hr_long = (
        calculate_hit_ratio(rebalance_scores, "true_long", "score_total_long")
        if "score_total_long" in rebalance_scores.columns
        else None
    )
    ...
    print(f"  ✓ {method} 완료:")
    print(
        f"    - 통합 랭킹 Hit Ratio: {hr_ensemble:.2%}"
        if hr_ensemble
        else "    - 통합 랭킹 Hit Ratio: N/A"
    )
    print(
        f"    - 단기 랭킹 Hit Ratio: {hr_short:.2%}"
        if hr_short
        else "    - 단기 랭킹 Hit Ratio: N/A"
    )
    print(
        f"    - 장기 랭킹 Hit Ratio: {hr_long:.2%}"
        if hr_long
        else "    - 장기 랭킹 Hit Ratio: N/A"
    )
...
def main():
    ...
    artifacts = {
        ...
        "universe_k200_membership_monthly": load_artifact(
            interim_dir / "universe_k200_membership_monthly"
        ),
    ...
    if results:
        ...
        results_sorted = sorted(
            results, key=lambda x: x["hit_ratio_ensemble"] or 0, reverse=True
        )

        print("\n[통합 랭킹 Hit Ratio 기준]")
        for i, r in enumerate(results_sorted, 1):
            print(
                f"  {i}. {r['method']:15s}: {r['hit_ratio_ensemble']:.2%}"
                if r["hit_ratio_ensemble"]
                else f"  {i}. {r['method']:15s}: N/A"
            )
        ...
        print(
            f"  통합 랭킹 Hit Ratio: {best['hit_ratio_ensemble']:.2%}"
            if best["hit_ratio_ensemble"]
            else "  통합 랭킹 Hit Ratio: N/A"
        )
        print(
            f"  단기 랭킹 Hit Ratio: {best['hit_ratio_short']:.2%}"
            if best["hit_ratio_short"]
            else "  단기 랭킹 Hit Ratio: N/A"
        )
        print(
            f"  장기 랭킹 Hit Ratio: {best['hit_ratio_long']:.2%}"
            if best["hit_ratio_long"]
            else "  장기 랭킹 Hit Ratio: N/A"
        )
