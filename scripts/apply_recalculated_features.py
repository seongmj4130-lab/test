...
def update_csv_file(horizon: str, max_dates: Optional[int] = None):
    ...
    csv_path = (
        project_root
        / "data"
        / f"daily_all_business_days_{horizon}_ranking_top20_formatted.csv"
    )
    ...
    original_df["company_name"] = (
        original_df["ticker_formatted"].map(ticker_to_name).fillna("Unknown")
    )
...
def validate_updated_file(horizon: str):
    ...
    for date in sample_dates:
        ...
        unique_patterns = (
            day_data["top1_feature_group"].astype(str)
            + ","
