...
def deep_analysis_backtest_paradox():
    ...
    # score_ens 계산 검증
    df_date["calculated_ens"] = (
        0.5 * df_date["score_short"] + 0.5 * df_date["score_long"]
    )
    ...
    print(
        f"단기 Top 15 ↔ 통합 Top 15 overlap: {overlap_se}/15 ({overlap_se/15*100:.1f}%)"
    )
    ...
    print(
        f"장기 Top 15 ↔ 통합 Top 15 overlap: {overlap_le}/15 ({overlap_le/15*100:.1f}%)"
    )
    ...
    print(
        f"단기 전략 포트폴리오 ↔ 앙상블 전략 포트폴리오 overlap: {portfolio_overlap}/15 ({portfolio_overlap/15*100:.1f}%)"
    )
