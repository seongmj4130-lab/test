...
for strategy_name in strategies.keys():
    ...
    basic_configs[strategy_name] = {
        ...
        'smart_buffer_stability_threshold': config.get('smart_buffer_stability_threshold'),
        ...
        'risk_scaling_neutral_multiplier': config.get('risk_scaling_neutral_multiplier'),
...
for key in basic_configs['bt20_short'].keys():
    values = [basic_configs[s][key] for s in strategies.keys() if basic_configs[s][key] is not None]
    if len(values) == len(strategies) and len(set(values)) == 1:
        ...
        print(f"⚠️  {key:<40} = {most_common[0]} (대부분, {most_common[1]}/{len(strategies)} 전략)")
...
# 공통값 기반 권장값
recommended = {
    'holding_days': common_configs.get('holding_days', current_defaults['holding_days']),
    'top_k': 20,  # 가장 자주 사용 (bt120_ens)
    'cost_bps': common_configs.get('cost_bps', current_defaults['cost_bps']),
    'slippage_bps': common_configs.get('slippage_bps', current_defaults['slippage_bps']),
    ...
    'smart_buffer_stability_threshold': common_configs.get('smart_buffer_stability_threshold', current_defaults['smart_buffer_stability_threshold']),
    'volatility_adjustment_enabled': True,  # 모든 전략이 사용
    'volatility_lookback_days': common_configs.get('volatility_lookback_days', current_defaults['volatility_lookback_days']),
    'target_volatility': common_configs.get('target_volatility', current_defaults['target_volatility']),
    'volatility_adjustment_max': common_configs.get('volatility_adjustment_max', current_defaults['volatility_adjustment_max']),
    ...
    'risk_scaling_bull_multiplier': common_configs.get('risk_scaling_bull_multiplier', current_defaults['risk_scaling_bull_multiplier']),
    'overlapping_tranches_enabled': False,  # bt120만 사용
    'tranche_holding_days': common_configs.get('tranche_holding_days', current_defaults['tranche_holding_days']),
    'tranche_max_active': common_configs.get('tranche_max_active', current_defaults['tranche_max_active']),
    'tranche_allocation_mode': common_configs.get('tranche_allocation_mode', current_defaults['tranche_allocation_mode']),
}
