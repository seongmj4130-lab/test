...
current_file = base_dir / 'artifacts' / 'reports' / 'bt_metrics_bt20_short_optimized.csv'
...
for phase in ['dev', 'holdout']:
    ...
    print(f"  CAGR:      {b['net_cagr']:.4f} → {a['net_cagr']:.4f} ({a['net_cagr'] - b['net_cagr']:+.4f})")
    print(f"  MDD:       {b['net_mdd']:.4f} → {a['net_mdd']:.4f} ({a['net_mdd'] - b['net_mdd']:+.4f})")
...
print("과적합 분석")
print("="*80)
print(f"  변경 전: Dev {dev_hr_before:.4f} - Holdout {holdout_hr_before:.4f} = {overfitting_before:.4f}")
print(f"  변경 후: Dev {dev_hr_after:.4f} - Holdout {holdout_hr_after:.4f} = {overfitting_after:.4f}")
print(f"  개선: {overfitting_before:.4f} → {overfitting_after:.4f} ({overfitting_after - overfitting_before:+.4f})")
...
print(f"  Holdout Hit Ratio ≥ 50%: {'✅' if target_met_before else '❌'} → {'✅' if target_met_after else '❌'}")
print(f"  과적합 ≤ 10%: {'✅' if overfitting_ok_before else '❌'} → {'✅' if overfitting_ok_after else '❌'}")
