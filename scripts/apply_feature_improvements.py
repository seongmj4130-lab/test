...
def apply_price_improvements(panel_df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(
        f"🎯 가격 개선 완료: {added_features//len(df['ticker'].unique())}개 피쳐 추가"
    )
    return df
...
def apply_momentum_improvements(panel_df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(
        f"🎯 모멘텀 강화 완료: {added_features//max(1, len(df['ticker'].unique()))}개 피쳐 추가"
    )
    return df
...
def apply_volatility_improvements(panel_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for ticker in df["ticker"].unique():
        ...
        # 변동성 비대칭도
        if "volatility_60d" in ticker_data.columns:
            upside_vol = ticker_data["returns"].where(
        ...
        ticker_data["tail_risk_5pct"] = (
            ticker_data["returns"].rolling(60).quantile(0.05)
        )
    ...
    print(
        f"🎯 변동성 개선 완료: {added_features//max(1, len(df['ticker'].unique()))}개 피쳐 추가"
    )
    return df
...
def apply_news_improvements(panel_df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(
        f"🎯 뉴스 강화 완료: {added_features//max(1, len(df['ticker'].unique()))}개 피쳐 추가"
    )
    return df


def create_feature_improvement_summary(
    original_df: pd.DataFrame, improved_df: pd.DataFrame
):
    """
    피쳐 개선 전후 비교 요약
    """
    ...
    price_features = [
        ...
        if any(x in col for x in ["52w", "intraday", "price_range"])
    ]
    momentum_features = [
        col
        for col in improved_df.columns
        if "momentum" in col and col not in original_df.columns
    ]
    volatility_features = [
        col
        for col in improved_df.columns
        if any(x in col for x in ["asymmetry", "tail_risk", "regime"])
        and col not in original_df.columns
    ]
    news_features = [
        col
        for col in improved_df.columns
        if "news_" in col and col not in original_df.columns
    ]
