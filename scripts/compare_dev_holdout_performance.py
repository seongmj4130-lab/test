...
def evaluate_on_fold(ranking_daily: pd.DataFrame, forward_returns: pd.DataFrame,
    ...
    returns_filtered = forward_returns[
        (forward_returns["date"].isin(dates)) &
    ...
    for date in merged["date"].unique():
        ...
        # 지표 계산
        hit_ratio = calculate_hit_ratio(returns.values, returns.values)  # returns > 0 비율
...
def analyze_overfitting(dev_results: dict, holdout_results: dict) -> dict:
    ...
    # 종합 과적합 위험 평가
    high_risk_count = sum(1 for v in analysis["overfitting_risk"].values() if v["risk_level"] == "high")
    medium_risk_count = sum(1 for v in analysis["overfitting_risk"].values() if v["risk_level"] == "medium")

    if high_risk_count >= 2:
        overall_risk = "high"
        analysis["recommendations"].append("⚠️ 높은 과적합 위험: Holdout 구간 성과가 Dev 구간 대비 크게 저하됨")
        ...
        overall_risk = "medium"
        analysis["recommendations"].append("⚠️ 중간 과적합 위험: 일부 지표에서 성과 차이 관찰됨")
        analysis["recommendations"].append("  - 정규화 조정 검토")
        analysis["recommendations"].append("  - 추가 검증 데이터로 재평가")
    else:
        overall_risk = "low"
        analysis["recommendations"].append("✅ 낮은 과적합 위험: Dev/Holdout 구간 성과가 유사함")
    ...
    for horizon in ["short", "long"]:
        ...
        try:
            ...
            if horizon == "short":
                ...
                ranking_daily = ranking_result.get("ranking_short_daily", pd.DataFrame())
            else:  # long
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        for horizon, results in all_results.items():
            ...
            for metric, comp in analysis['metrics_comparison'].items():
                ...
                f.write(f"| {metric} | {comp['dev']:.4f} | {comp['holdout']:.4f} | "
