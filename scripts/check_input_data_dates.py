...
for f in sorted(score_files):
    ...
    if len(dates) > 1:
        ...
        print(
            f"  rebalance_interval=20일 때 예상 리밸런싱 날짜: {len([dates[i] for i in range(0, len(dates), 20)])}개"
        )
        print(
            f"  rebalance_interval=120일 때 예상 리밸런싱 날짜: {len([dates[i] for i in range(0, len(dates), 120)])}개"
        )
