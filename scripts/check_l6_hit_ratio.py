...
def check_l6_hit_ratio():
    ...
    # Hit Ratio 관련 컬럼 찾기
    hit_related_cols = [
        col for col in df.columns if "hit" in col.lower() or "ratio" in col.lower()
    ]
    ...
    if "true_short" in df.columns and "true_long" in df.columns:
        ...
        print(
            f"true_short > 0: {(df['true_short'] > 0).sum()} / {len(df)} ({(df['true_short'] > 0).mean():.1%})"
        )
        print(
            f"true_long > 0: {(df['true_long'] > 0).sum()} / {len(df)} ({(df['true_long'] > 0).mean():.1%})"
        )
        ...
        for phase in df["phase"].unique():
            ...
            print(
                f"{phase.upper()} 구간: short_hit={short_hit:.1%}, long_hit={long_hit:.1%}"
            )
