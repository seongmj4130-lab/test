...
def main():
    ...
    print(
        f"  regime_aware_weights_config: {l8.get('regime_aware_weights_config', 'N/A')}"
    )
    ...
    if short_weights_path.exists():
        ...
        with open(short_weights_path, encoding="utf-8") as f:
        ...
        if "feature_weights" in short_weights:
            ...
            sorted_weights = sorted(
                short_weights["feature_weights"].items(),
                key=lambda x: x[1],
                reverse=True,
            )[:10]
    ...
    if long_weights_path.exists():
        ...
        with open(long_weights_path, encoding="utf-8") as f:
    ...
    elif l8_short.get("feature_weights_config") or l8_long.get(
