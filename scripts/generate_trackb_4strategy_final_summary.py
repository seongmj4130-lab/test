...
def main() -> None:
    ...
    for s in STRATEGIES:
        ...
        regime_by_strategy[s] = _read_parquet(
            interim / f"bt_regime_metrics_{s}.parquet"
        )
    ...
    def build_section(
        title: str, cols: list[tuple[str, str]], nd_map: Optional[dict[str, int]] = None
    ) -> str:
        rows = []
        for s in STRATEGIES:
    ...
    for s in STRATEGIES:
        ...
        if nr_i is not None and nr_i <= 5:
            caution_lines.append(
                f"- `{s}` Holdout은 Rebalances={nr_i}로 표본이 매우 작아 지표 변동이 큽니다."
            )
        if hd_i is not None and hd_i >= 60:
            caution_lines.append(
                f"- `{s}`는 holding_days={hd_i}로 긴 주기 전략이라 구간별 표본이 작을 수 있습니다."
            )
    if caution_lines:
        md.append("> 주의:")
    ...
    md.append(
        build_section("1) 핵심 성과 (Headline Metrics)", headline_cols, nd_map=nd_map)
    )
    md.append(
        build_section("2) 모델 예측력 (Alpha Quality)", alpha_cols, nd_map=nd_map)
    )
    md.append(
        build_section("3) 운용 안정성 (Operational Viability)", op_cols, nd_map=nd_map)
    )
