...
def load_comparison_data():
    """비교 데이터 로드"""
    comparison_file = (
        PROJECT_ROOT / "artifacts" / "reports" / "backtest_4models_comparison.csv"
    )
...
def create_before_after_comparison():
    ...
    comparison_df = pd.merge(
        before_df, after_df, on=["strategy", "holding_days"], how="left"
    )
...
def generate_summary_report(df):
    ...
    improvements = df.groupby("지표").agg({"변화율": "mean"}).round(2)
    ...
    # 전략별 최고 성과
    print("\n4️⃣ 전략별 주요 성과:")
    best_sharpe = df[df["지표"] == "Sharpe Ratio"].loc[
        df[df["지표"] == "Sharpe Ratio"]["변화율"].idxmax()
    ]
    print(
        f"   • Sharpe 최고 개선: {best_sharpe['전략']} (+{best_sharpe['변화율']:.2f}%)"
    )

    best_cagr = df[df["지표"] == "CAGR"].loc[
        df[df["지표"] == "CAGR"]["변화율"].idxmax()
    ]
    ...
    if (
        pipeline_health_improved > 0
        and data_quality_improved > 0
        and avg_performance_improved >= 0
    ):
