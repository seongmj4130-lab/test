...
def run_backtest_with_params(
    ...
    try:
        ...
        if rebalance_interval != holding_days:
            logger.warning(
                f"[{strategy}] rebalance_interval({rebalance_interval}) != holding_days({holding_days}), 완전 교체 전략이 아닙니다."
            )
        ...
        temp_config_path = (
            base_dir / f"configs/config_temp_full_replacement_topk_{top_k}.yaml"
        )
        ...
        try:
            ...
            if result is None:
                logger.warning(
                    f"[{strategy}] top_k={top_k}, buffer_k={buffer_k}, interval={rebalance_interval}: 결과가 None"
                )
            ...
            if bt_metrics is None or bt_metrics.empty:
                logger.warning(
                    f"[{strategy}] top_k={top_k}, buffer_k={buffer_k}, interval={rebalance_interval}: 메트릭 없음"
                )
                return (None, None, None, None, None, None)
            ...
            if dev_metrics.empty or holdout_metrics.empty:
                logger.warning(
                    f"[{strategy}] top_k={top_k}, buffer_k={buffer_k}, interval={rebalance_interval}: Dev/Holdout 구분 실패"
                )
                return (None, None, None, None, None, None)
        ...
        logger.error(
            f"[{strategy}] top_k={top_k}, buffer_k={buffer_k}, interval={rebalance_interval} 실행 실패: {type(e).__name__}: {e}"
        )
        import traceback
...
def optimize_top_k_full_replacement(
    ...
    logger.info(
        f"  전략: 완전 교체 (rebalance_interval={rebalance_interval}, buffer_k={buffer_k})"
    )
    ...
    with ProcessPoolExecutor(max_workers=n_jobs) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                logger.info(
                    f"  top_k={top_k}: Dev TR={safe_format(total_return_dev, '{:.4f}')}, "
    ...
    logger.info(
        f"[{strategy}] 최적 top_k: {best_top_k} (Holdout Total Return: {best_score:.4f})"
    )
...
def generate_optimization_report(result: dict, config_path: str):
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# 완전 교체 전략 + top_k 최적화 결과\n\n")
        f.write(f"생성 시간: {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        f.write("## 실험 조건\n\n")
        f.write("- **전략**: 완전 교체 (rebalance_interval = holding_days)\n")
        f.write("  - Day1: top_k 매수 → Day20: 전량 매도 → Day20 top_k 재매수\n")
        f.write("  - buffer_k=0 고정 (매번 100% 교체)\n")
        f.write("  - rebalance_interval=20 (holding_days와 동일)\n")
        f.write("- **전략**: bt20_short (단기 랭킹)\n")
        f.write("- **top_k 그리드**: [20, 15, 10]\n\n")
        f.write("## 최적화 결과\n\n")

        if best_top_k is None:
            f.write("- 최적 top_k를 찾을 수 없습니다.\n\n")
        else:
            f.write(f"- **최적 top_k**: {best_top_k}\n")
            f.write(
                f"- **최적화 점수 (Holdout Total Return)**: {safe_format(best_score, '{:.4f}')}\n\n"
            )

            # 최적값의 상세 메트릭
            best_row = (
                df_results[df_results["top_k"] == best_top_k].iloc[0]
                if not df_results.empty
                else None
            )
            if best_row is not None:
                f.write("### 최적 파라미터 상세 메트릭 (Holdout)\n\n")
                f.write(
                    f"- **Net Sharpe**: {safe_format(best_row['sharpe_holdout'], '{:.4f}')}\n"
                )
                f.write(
                    f"- **Net CAGR**: {safe_format(best_row['cagr_holdout'], '{:.4%}')}\n"
                )
                f.write(
                    f"- **Net Total Return**: {safe_format(best_row['total_return_holdout'], '{:.4f}')}\n\n"
                )
        ...
        if best_top_k is not None:
            f.write(
                f"완전 교체 전략에서 **top_k={best_top_k}**이 Holdout 구간 Total Return 기준으로 최적 성과를 보였습니다.\n\n"
            )
        f.write("### 주요 발견사항\n\n")
        f.write(
            "- 완전 교체 전략 (buffer_k=0, rebalance_interval=holding_days)은 매 리밸런싱마다 100% 종목 교체\n"
        )
        f.write("- top_k 값에 따른 성과 차이 분석 완료\n")
...
def main():
    ...
    logger.info(
        f"완전 교체 조건: buffer_k={buffer_k}, rebalance_interval={rebalance_interval}"
    )
    logger.info(f"top_k 그리드: {top_k_list}")
