...
def analyze_backtest_reproducibility(results_df):
    ...
    for strategy in strategies:
        ...
        print(
            f"재현성 평가: {reproducibility} (문제 지표: {len(reproducibility_issues)}개)"
        )
    ...
    unique_results = len(
        results_df.drop_duplicates(
            subset=["strategy", "net_sharpe", "net_cagr", "net_mdd"]
        )
    )
    ...
    print(
        f"결과 일관성: {unique_results}/{total_runs} ({unique_results/total_runs:.1%})"
    )
...
def run_reproducibility_test(n_iterations=3):
    ...
    output_file = (
        ...
        / "artifacts"
        / "reports"
        / f"backtest_reproducibility_test_{timestamp}.csv"
    )
