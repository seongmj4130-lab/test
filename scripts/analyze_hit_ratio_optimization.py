...
def main():
    ...
    for _, row in results_df.iterrows():
        ...
        print(
            f"| {model:12s} | {phase:7s} | {net_hr:13.4f} {target_icon} | {gross_hr:15.4f} | {sharpe:6.4f} | {cagr:4.4f} | {mdd:5.4f} |"
        )
    ...
    for _, row in recommendations.iterrows():
        print(
            f"| {row['model']:12s} | {row['dev_hit_ratio']:6.4f} | {row['holdout_hit_ratio']:10.4f} | {row['overfitting']:6.4f} | {'✅' if row['target_met'] else '❌'} | {row['status']:10s} | {row['priority']:6s} |"
        )
    ...
    for _, row in recommendations.iterrows():
        ...
        if target_met and overfitting <= 0.10:
            ...
            print(
                f"⚠️ 목표 달성했으나 과적합 존재: Holdout = {holdout_hr:.4f}, 과적합 = {overfitting:.4f}"
            )
            print("   권장사항:")
            ...
            print(
                f"❌ 목표 미달 + 과적합: Holdout = {holdout_hr:.4f}, 과적합 = {overfitting:.4f}"
            )
            print("   권장사항:")
            print("   1. 정규화 강화 (ridge_alpha 증가)")
            print("   2. 피쳐 선택 (IC 낮은 피쳐 제거)")
    ...
    results_df.to_csv(
        output_dir / "hit_ratio_analysis.csv", index=False, encoding="utf-8-sig"
    )
    recommendations.to_csv(
        output_dir / "hit_ratio_optimization_recommendations.csv",
        index=False,
        encoding="utf-8-sig",
    )
