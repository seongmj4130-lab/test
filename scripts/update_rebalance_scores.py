...
def update_rebalance_scores():
    ...
    if config_path.exists():
        ...
        print(
            f"Config에서 읽은 가중치 - weight_long: {weight_long}, weight_short: {weight_short}"
        )
    else:
        # 기본값 사용
        weight_long = 0.8
        weight_short = 0.2
        print(
            f"기본 가중치 사용 - weight_long: {weight_long}, weight_short: {weight_short}"
        )
    ...
    if "score_short" in df_new.columns and "score_long" in df_new.columns:
        ...
        df_new["score_ens"] = (
            weight_long * df_new["score_long"] + weight_short * df_new["score_short"]
        )
        ...
        df_old_calc = (
            old_weight_long * df_old["score_long"]
            + old_weight_short * df_old["score_short"]
        )
