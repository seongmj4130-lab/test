...
def compare_rankings(data_sources, strategy_name):
    ...
    for source_name, df in rankings_data.items():
        if len(df) > 0:
            top1 = df.iloc[0]
            ticker_col = (
                "종목명(ticker)" if "종목명(ticker)" in df.columns else "ticker"
            )
            score_col = "score" if "score" in df.columns else "Score"
            features_col = (
                "top3 피쳐그룹" if "top3 피쳐그룹" in df.columns else "top3_features"
            )
    ...
    if len(rankings_data) >= 2:
        ...
        for i in range(len(sources_list)):
            for j in range(i + 1, len(sources_list)):
                ...
                ticker_col1 = (
                    "종목명(ticker)" if "종목명(ticker)" in df1.columns else "ticker"
                )
                ticker_col2 = (
                    "종목명(ticker)" if "종목명(ticker)" in df2.columns else "ticker"
                )
...
# 통합 전략 (holdout만 있음)
integrated_sources = {"Holdout 통합": "data/holdout_daily_ranking_integrated_top20.csv"}
