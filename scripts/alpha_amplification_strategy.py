...
class AlphaAmplifier:
    ...
    def develop_alpha_amplification_plan(self):
        ...
        for strategy, data in self.current_alpha.items():
            ...
            print(
                f"  • 현재 Alpha: {data['current_alpha']:+.2f}% (vs KOSPI +{kospi_return:.1f}%)"
            )
            print(
                f"  • Alpha 부족분: {quant_avg_return - data['current_cagr']:.2f}% (퀀트 평균 도달까지)"
            )
    ...
    def estimate_alpha_improvement(self):
        ...
        current_avg_alpha = np.mean(
            [data["current_alpha"] for data in self.current_alpha.values()]
        )
