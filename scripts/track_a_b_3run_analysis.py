...
def analyze_results(results_list):
    ...
    for strategy in strategies:
        ...
        for iteration in [1, 2, 3]:
            ...
            if len(iter_data) > 0:
                ...
                print(
                    f"실행 {iteration}: Sharpe {row['net_sharpe']:.4f}, "
        ...
        # 재현성 평가
        sharpe_cv = np.std(strategy_data["net_sharpe"]) / abs(
            np.mean(strategy_data["net_sharpe"])
        )
    ...
    output_file = (
        ...
        / "artifacts"
        / "reports"
        / f"track_a_b_3run_analysis_{timestamp}.csv"
    )
