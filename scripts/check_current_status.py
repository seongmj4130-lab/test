...
def check_status():
    ...
    if Path(short_file).exists() and Path(long_file).exists():
        ...
        print("=" * 80)
        print(f"단기: 조합 {len(short_df)}개, 최적 Score {short_best['objective_score']:.4f}, IC {short_best['ic_mean']:.4f}")
        print(f"장기: 조합 {len(long_df)}개, 최적 Score {long_best['objective_score']:.4f}, IC {long_best['ic_mean']:.4f}")
