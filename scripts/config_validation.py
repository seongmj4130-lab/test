...
def validate_configs_batch(
    configs_dir: Path, verbose: bool = False
...
def get_top_failures(
    failures: dict[str, list[str]], top_n: int = 5
) -> list[tuple[str, int]]:
...
def main():
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="상세한 검증 결과 출력"
    )
    parser.add_argument(
        "--top-errors", type=int, default=5, help="상위 N개 에러 유형 표시 (기본값: 5)"
    )
