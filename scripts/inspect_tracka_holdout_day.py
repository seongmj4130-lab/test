...
def main():
    ...
    p.add_argument(
        "--save_md", action="store_true", help="artifacts/reports/ 에 Markdown 저장"
    )
    args = p.parse_args()
    ...
    results = {
        k: v
        for k, v in [("short", result.get("short")), ("long", result.get("long"))]
        if v is not None
    }
    ...
    for h, df in results.items():
        print("\n" + "=" * 100)
        print(
            f"[Track A] {h.upper()} | date={date.date()} | Top{args.topk} + 팩터셋 Top3"
        )
        print("=" * 100)
    ...
    if args.save_md:
        ...
        out_path = (
            out_dir
            / f"track_a_holdout_day_{date.date()}_{args.horizon}_top{args.topk}.md"
        )
