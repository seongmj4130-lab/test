        ...
        if baseline_file and baseline_file.exists():
            ...
            if len(baseline_holdout) > 0:
                ...
                if row["baseline_sharpe"] != 0:
                    row["sharpe_change_pct"] = (
                        (row["current_sharpe"] - row["baseline_sharpe"])
                        / abs(row["baseline_sharpe"])
                        * 100
                    )
                if row["baseline_cagr"] != 0:
                    row["cagr_change_pct"] = (
                        (row["current_cagr"] - row["baseline_cagr"])
                        / abs(row["baseline_cagr"])
                        * 100
                    )
                if row["baseline_calmar"] != 0:
                    row["calmar_change_pct"] = (
                        (row["current_calmar"] - row["baseline_calmar"])
                        / abs(row["baseline_calmar"])
                        * 100
                    )
