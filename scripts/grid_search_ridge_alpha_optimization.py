...
def run_backtest_with_ridge_alpha(
    strategy: str,
    ridge_alpha: float,
    ...
    try:
        ...
        logger.error(f"[{strategy}] ridge_alpha={ridge_alpha:.3f} 실행 실패: {e}")
        return (None, None, None, None, None, None)
...
def optimize_ridge_alpha_grid_search(
    ...
    Args:
        ...
        ridge_alpha_grid: 테스트할 ridge_alpha 값 리스트
        config_path: 설정 파일 경로
    ...
    """
    logger.info(f"[{strategy}] Ridge Alpha 최적화 시작 (목표: {optimization_target})")
    logger.info(f"  Ridge Alpha 그리드: {ridge_alpha_grid}")
...
def generate_optimization_report(
    ...
    # 리포트 생성
    report_lines = []
    report_lines.append("# Grid Search Ridge Alpha 최적화 결과")
    report_lines.append("")
    ...
    for _, row in df_summary.iterrows():
        ...
        best_ridge_alpha = row["best_ridge_alpha"]
        ...
        report_lines.append("")
        if best_ridge_alpha is not None:
            report_lines.append(f"- **최적 Ridge Alpha**: {best_ridge_alpha:.3f}")
    ...
    # 리포트 저장
    report_path = reports_dir / "ridge_alpha_optimization_report.md"
...
def main():
    ...
    # Ridge Alpha 그리드 정의 (일반적인 범위)
    ridge_alpha_grid = [0.01, 0.1, 0.3, 0.5, 1.0, 3.0, 5.0, 10.0]
    ...
    for opt in optimizations:
        ...
        result = optimize_ridge_alpha_grid_search(
            strategy=strategy,
        ...
        # 상세 결과 저장
        result_path = reports_dir / f"ridge_alpha_optimization_{strategy}.csv"
