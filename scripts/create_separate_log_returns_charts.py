...
def create_strategy_cumulative_log_returns():
    ...
    for strategy_name, params in strategies.items():
        # CAGR을 월별 로그 수익률로 변환
        monthly_log_return = np.log(1 + params["total_return"]) / (
            params["period_years"] * 12
        )
        monthly_volatility = params["annual_volatility"] / np.sqrt(12)

        # 월별 로그 수익률 생성 (평균 + 변동성)
        log_returns = np.random.normal(
            monthly_log_return, monthly_volatility, len(dates)
        )
...
def create_log_returns_comparison_chart():
    ...
    for i, (strategy_name, data) in enumerate(strategy_results.items(), 1):
        ax.plot(
            dates,
            data["cumulative_log_returns"] * 100,
            label=strategy_name,
            color=colors[i],
            linewidth=2.5,
            alpha=0.9,
    ...
    # 그래프 설정
    ax.set_title(
        "KOSPI200 TR vs 4가지 전략: 누적 로그 수익률 비교 (2023-2024)",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "results/kospi_tr_vs_strategies_log_returns_percent.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    print(
        "✅ 로그 수익률 비교 그래프 생성: results/kospi_tr_vs_strategies_log_returns_percent.png"
    )
...
def create_quarterly_average_returns_chart():
    ...
    df_data = pd.DataFrame(
        {"date": dates, "KOSPI_TR": kospi_log_returns * 100}  # %로 변환
    )
    ...
    # 그래프 설정
    ax.set_title(
        "분기별 평균 수익률 비교 (2023-2024)", fontsize=16, fontweight="bold", pad=20
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "results/quarterly_average_returns_comparison.png", dpi=300, bbox_inches="tight"
    )
    plt.close()

    print(
        "✅ 분기별 평균 수익률 비교 그래프 생성: results/quarterly_average_returns_comparison.png"
    )
...
def create_summary_statistics():
    ...
    df_stats.to_csv(
        "results/log_returns_statistics_updated.csv", index=False, encoding="utf-8-sig"
    )
