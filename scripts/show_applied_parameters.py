    ...
    for strategy in strategies:
        ...
        if section_name in config:
            ...
            strategy_params = {
                ...
                "rebalance_interval": strategy_config.get(
                    "rebalance_interval", "기본값 사용"
                ),
                ...
                "target_volatility": strategy_config.get(
                    "target_volatility", "기본값 사용"
                ),
                "volatility_adjustment_enabled": strategy_config.get(
                    "volatility_adjustment_enabled", "기본값 사용"
                ),
                "volatility_adjustment_min": strategy_config.get(
                    "volatility_adjustment_min", "기본값 사용"
                ),
                "volatility_adjustment_max": strategy_config.get(
                    "volatility_adjustment_max", "기본값 사용"
                ),
                "risk_scaling_enabled": strategy_config.get(
                    "risk_scaling_enabled", "기본값 사용"
                ),
                "smart_buffer_enabled": strategy_config.get(
                    "smart_buffer_enabled", "기본값 사용"
                ),
                "score_col": strategy_config.get("score_col", "기본값 사용"),
                "buffer_k": strategy_config.get("buffer_k", "기본값 사용"),
                "regime.enabled": strategy_config.get("regime", {}).get(
                    "enabled", "기본값 사용"
                ),
                "overlapping_tranches_enabled": strategy_config.get(
                    "overlapping_tranches_enabled", "기본값 사용"
                ),
            }
