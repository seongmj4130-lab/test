...
# 공통 날짜 선택
common_dates = set(l6_scores_interval20['date'].unique()) & set(l8_short['date'].unique())
...
if 'score_total_short' in merged_short.columns and 'score_total_l8' in merged_short.columns:
    valid = merged_short[merged_short['score_total_l8'].notna() & merged_short['score_total_short'].notna()]
    if len(valid) > 0:
...
if 'score_total' in merged_short.columns and 'score_total_l8' in merged_short.columns:
    valid = merged_short[merged_short['score_total_l8'].notna() & merged_short['score_total'].notna()]
    if len(valid) > 0:
        corr = valid['score_total'].corr(valid['score_total_l8'])
        print(f'\nL6 score_total vs L8 Short score_total (참고):')
        print(f'  상관계수: {corr:.4f}')
        print(f'  참고: L6의 score_total은 score_ens일 수 있음')
...
# score_total_long 비교
if 'score_total_long' in merged_long.columns and 'score_total_l8' in merged_long.columns:
    valid = merged_long[merged_long['score_total_l8'].notna() & merged_long['score_total_long'].notna()]
    if len(valid) > 0:
