...
# 월별 수익률 그래프
ax2.bar(
    data_20d["month"] - pd.Timedelta(days=5),
    data_20d["kospi_mret_pct"],
    width=5,
    label="KOSPI",
    alpha=0.7,
)
ax2.bar(
    data_20d["month"], data_20d["short_mret_pct"], width=5, label="단기 전략", alpha=0.7
)
ax2.bar(
    data_20d["month"] + pd.Timedelta(days=5),
    data_20d["long_mret_pct"],
    width=5,
    label="장기 전략",
    alpha=0.7,
)
...
plt.suptitle(
    "KOSPI200 vs 전략별 성과 비교 (20일 보유 기준)",
    fontsize=16,
    fontweight="bold",
    y=0.98,
)
...
print(
    "✅ 월별 수익률 분석 그래프가 'monthly_returns_analysis_20d.png'로 저장되었습니다."
)
