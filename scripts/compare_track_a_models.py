    ...
    if files:
        ...
        try:
            with open(latest_file, encoding="utf-8") as f:
                ...
                results[model_key] = {
                    "dev_hit_ratio": metadata.get("dev_hit_ratio")
                    or metadata.get("hit_ratio"),
                    "dev_ic_mean": metadata.get("dev_ic_mean")
                    or metadata.get("ic_mean"),
                    "dev_icir": metadata.get("dev_icir") or metadata.get("icir"),
                    "dev_objective": metadata.get("dev_objective_score")
                    or metadata.get("objective_score"),
...
for key, vals in results.items():
    ...
    model = parts[0]
    horizon = parts[1]
    rows.append({"model": model, "horizon": horizon, **vals})
...
print(
    f"{'모델':<15} {'Dev Hit':<8} {'Dev IC':<8} {'Dev ICIR':<10} {'Dev Obj':<8} {'Holdout Hit':<12} {'Holdout IC':<12} {'Holdout ICIR':<12} {'Holdout Obj':<12}"
)
print("-" * 100)
...
for _, row in short_df.iterrows():
    ...
    ho_hr = (
        f"{row['holdout_hit_ratio']:.3f}"
        if pd.notna(row["holdout_hit_ratio"])
        else "N/A"
    )
    ho_ic = (
        f"{row['holdout_ic_mean']:.3f}" if pd.notna(row["holdout_ic_mean"]) else "N/A"
    )
    ho_icir = f"{row['holdout_icir']:.3f}" if pd.notna(row["holdout_icir"]) else "N/A"
    ho_obj = (
        f"{row['holdout_objective']:.3f}"
        if pd.notna(row["holdout_objective"])
        else "N/A"
    )
    print(
        f"{model:<15} {dev_hr:<8} {dev_ic:<8} {dev_icir:<10} {dev_obj:<8} {ho_hr:<12} {ho_ic:<12} {ho_icir:<12} {ho_obj:<12}"
    )
