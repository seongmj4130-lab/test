    ...
    results.append(
        {
            ...
            "diff_pct": (
                (actual["net_sharpe"] - exp["net_sharpe"])
                / abs(exp["net_sharpe"])
                * 100
                if exp["net_sharpe"] != 0
                else 0
    ...
    results.append(
        {
            ...
            "diff_pct": (
                (actual["net_cagr"] - exp["net_cagr"]) / abs(exp["net_cagr"]) * 100
                if exp["net_cagr"] != 0
                else 0
            ),
        }
    ...
    results.append(
        {
            ...
            "diff_pct": (
                (actual["net_mdd"] - exp["net_mdd"]) / abs(exp["net_mdd"]) * 100
                if exp["net_mdd"] != 0
                else 0
            ),
        }
    ...
    results.append(
        {
            ...
            "diff_pct": (
                (actual["net_calmar_ratio"] - exp["net_calmar_ratio"])
                / abs(exp["net_calmar_ratio"])
                * 100
                if exp["net_calmar_ratio"] != 0
                else 0
            ),
        }
    )
...
for strategy in strategies:
    ...
    for _, row in df_strat.iterrows():
        ...
        print(
            f"  {match} {row['metric']:15s} | 기대: {row['expected']:8.4f} | 실제: {row['actual']:8.4f} | 차이: {row['diff']:8.4f} ({row['diff_pct']:6.2f}%)"
        )
