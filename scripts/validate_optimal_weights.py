...
def evaluate_on_folds(
    ...
    if len(holdout_folds) > 0:
        holdout_dates = pd.to_datetime(
            holdout_folds["test_end"].unique(), errors="coerce"
        )
    ...
    for horizon in ["short", "long"]:
        ...
        try:
            ...
            fold_results = evaluate_on_folds(
                ranking_daily, forward_returns, cv_folds, horizon
            )
    ...
    with open(summary_file, "w", encoding="utf-8") as f:
        ...
        for horizon, result in results_summary.items():
            ...
            if "dev" in fold_results and "holdout" in fold_results:
                ...
                f.write(
                    f"- Hit Ratio 차이: {holdout.get('hit_ratio', 0) - dev.get('hit_ratio', 0):.4f}\n"
                )
                f.write(
                    f"- IC Mean 차이: {holdout.get('ic_mean', 0) - dev.get('ic_mean', 0):.4f}\n"
                )
                f.write(
                    f"- ICIR 차이: {holdout.get('icir', 0) - dev.get('icir', 0):.4f}\n\n"
                )
