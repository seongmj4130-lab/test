...
def create_comprehensive_ranking_report():
    ...
    if not h_short.empty and not d_short.empty:
        ...
        print(
            f"   단기: Holdout={h_short_ticker} | 일간={d_short_ticker} | {'✅ 일치' if short_match else '❌ 불일치'}"
        )
    ...
    if not h_long.empty and not d_long.empty:
        ...
        print(
            f"   장기: Holdout={h_long_ticker} | 일간={d_long_ticker} | {'✅ 일치' if long_match else '❌ 불일치'}"
        )
    ...
    for strategy, h_df, d_df in [("단기", h_short, d_short), ("장기", h_long, d_long)]:
        if not h_df.empty and not d_df.empty:
            h_top5 = set(
                [normalize_ticker(x) for x in h_df.head(5)["종목명(ticker)"].tolist()]
            )
            d_top5 = set(
                [normalize_ticker(str(x)) for x in d_df.head(5)["ticker"].tolist()]
            )
    ...
    print("\n📋 일간 데이터 구조:")
    print(
        "   - 컬럼: ranking, ticker, date, score_short, score_long, score_ens, top1_feature_group..."
    )
