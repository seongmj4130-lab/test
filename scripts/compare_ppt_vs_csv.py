...
for strategy, metrics in ppt_data.items():
    print(
        f'{strategy}: 총수익률 {metrics["total_return"]}%, Sharpe {metrics["sharpe"]}'
    )
...
if csv_path.exists():
    ...
    for col in ["short_mret_pct", "long_mret_pct", "mix_mret_pct"]:
        ...
        strategy_name = (
            col.replace("mret_pct", "")
            .replace("short", "bt20_short")
            .replace("long", "bt120_long")
            .replace("mix", "bt120_ens")
        )
    ...
    csv_calculated = {
        "bt20_short": {
            "total_return": (1 + df_20d["short_mret_pct"] / 100).prod() - 1,
            "sharpe": (df_20d["short_mret_pct"] / 100).mean()
            / (df_20d["short_mret_pct"] / 100).std(),
        },
        "bt120_long": {
            "total_return": (1 + df_20d["long_mret_pct"] / 100).prod() - 1,
            "sharpe": (df_20d["long_mret_pct"] / 100).mean()
            / (df_20d["long_mret_pct"] / 100).std(),
        },
        "bt120_ens": {
            "total_return": (1 + df_20d["mix_mret_pct"] / 100).prod() - 1,
            "sharpe": (df_20d["mix_mret_pct"] / 100).mean()
            / (df_20d["mix_mret_pct"] / 100).std(),
        },
    }
    ...
    for strategy in ppt_data.keys():
        ...
        print(
            f"  차이: 총수익률 {return_diff:.2f}%, Sharpe {sharpe_diff:.3f} - {status}"
        )
        print()
