...
def analyze_ranking_quality(ranking_df):
    ...
    stats = {
        ...
        'top_10_score_avg': ranking_df[ranking_df['rank_total'] <= 10]['score_total'].mean(),
        'bottom_10_score_avg': ranking_df[ranking_df['rank_total'] > ranking_df['rank_total'].max() - 10]['score_total'].mean()
    }
...
def compare_track_a_results(results):
    ...
    for ranking_type in ranking_types:
        ...
        if extracted_stats and all(s is not None for s in extracted_stats):
            print(f'\n{ranking_type} 평균:')
            numeric_keys = ['score_total_mean', 'score_total_std', 'rank_total_mean', 'rank_total_std',
                          'top_10_score_avg', 'bottom_10_score_avg']

            for key in numeric_keys:
                values = [s[key] for s in extracted_stats if key in s and s[key] is not None]
                if values:
