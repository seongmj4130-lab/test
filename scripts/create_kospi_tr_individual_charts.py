...
def create_kospi_tr_monthly_returns_chart():
    ...
    bars = ax.bar(
        df["year_month"],
        df["kospi_tr_monthly_log_return"],
        color="#FF6B6B",
        alpha=0.8,
        edgecolor="white",
        linewidth=0.5,
    ...
    # 그래프 설정
    ax.set_title(
        "KOSPI TR 월별 로그 수익률 (2023-2024)", fontsize=16, fontweight="bold", pad=20
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "results/kospi_tr_monthly_log_returns_chart.png", dpi=300, bbox_inches="tight"
    )
    plt.close()

    print(
        "✅ KOSPI TR 월별 로그 수익률 그래프 생성: results/kospi_tr_monthly_log_returns_chart.png"
    )
...
def create_kospi_tr_cumulative_returns_chart():
    ...
    ax.scatter(
        [df["year_month"].iloc[0]], [start_value], color="green", s=100, zorder=5
    )
    ...
    # 그래프 설정
    ax.set_title(
        "KOSPI TR 누적 로그 수익률 (2023-2024)", fontsize=16, fontweight="bold", pad=20
    )
    ...
    # 범례 추가
    ax.legend(
        ["KOSPI TR 누적 수익률", "시작점", "종료점"], loc="upper left", fontsize=10
    )

    plt.tight_layout()
    plt.savefig(
        "results/kospi_tr_cumulative_log_returns_chart.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    print(
        "✅ KOSPI TR 누적 로그 수익률 그래프 생성: results/kospi_tr_cumulative_log_returns_chart.png"
    )
...
def create_combined_kospi_tr_analysis():
    ...
    # 전체 제목
    fig.suptitle(
        "KOSPI TR 로그 수익률 분석: 월별 vs 누적 (2023-2024)",
        fontsize=16,
        fontweight="bold",
        y=0.98,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "results/kospi_tr_monthly_vs_cumulative_analysis.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    print(
        "✅ KOSPI TR 월별 vs 누적 종합 분석 그래프 생성: results/kospi_tr_monthly_vs_cumulative_analysis.png"
    )
