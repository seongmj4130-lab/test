...
if dummy_path.exists() and price_path.exists() and strategy_path.exists():
        ...
        try:
            ...
            if str(month_period) in monthly_strategy_returns:
                ...
                if "BT20 단기 (20일)" in strategy_data:
                    df_real.loc[idx, "short_mret_pct"] = strategy_data[
                        "BT20 단기 (20일)"
                    ]
                if "BT120 장기 (120일)" in strategy_data:
                    df_real.loc[idx, "long_mret_pct"] = strategy_data[
                        "BT120 장기 (120일)"
                    ]
                if "BT120 앙상블 (120일)" in strategy_data:
                    df_real.loc[idx, "mix_mret_pct"] = strategy_data[
                        "BT120 앙상블 (120일)"
                    ]
    ...
    real_2023 = df_real[
        (df_real["horizon_days"] == 20) & (df_real["month"].str.startswith("2023"))
    ]
    print("실제 데이터 평균 수익률:")
