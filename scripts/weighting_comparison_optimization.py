...
def run_backtest_with_weighting(
    ...
    try:
        ...
        if strategy_config_key not in cfg:
            logger.error(
                f"[{strategy}] 설정 섹션을 찾을 수 없습니다: {strategy_config_key}"
            )
        ...
        temp_config_path = (
            base_dir / f"configs/config_temp_weighting_{strategy}_{weighting}.yaml"
        )
        ...
        try:
            ...
            if dev_metrics.empty or holdout_metrics.empty:
                logger.warning(
                    f"[{strategy}] weighting={weighting}: Dev/Holdout 구분 실패"
                )
                return (None, None, None, None, None, None)
        ...
        logger.error(
            f"[{strategy}] weighting={weighting} 실행 실패: {type(e).__name__}: {e}"
        )
        import traceback
...
def compare_weighting_methods(
    ...
    with ProcessPoolExecutor(max_workers=n_jobs) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                all_results.append(
                    {
                        ...
                        "weighting": weighting,
                        "softmax_temperature": (
                            softmax_temperature if weighting == "softmax" else None
                        ),
                ...
                logger.info(
                    f"  [{strategy}] {weighting}: Dev TR={safe_format(total_return_dev, '{:.4f}')}, "
    ...
    for strategy in strategies:
        ...
        logger.info(
            f"[{strategy}] 최적 weighting: {best_w} (Holdout Total Return: {best_score:.4f})"
        )
...
def generate_comparison_report(result: dict, config_path: str):
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# 가중치 방식(equal vs softmax) 비교 최적화 결과\n\n")
        f.write(f"생성 시간: {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        f.write("## 실험 조건\n\n")
        f.write("- **전략**: bt20_short, bt20_ens, bt120_long, bt120_ens (4가지)\n")
        f.write("- **가중치 방식**: equal, softmax (2가지)\n")
        f.write("- **softmax_temperature**: 0.5\n")
        f.write("- **비교 기준**: Holdout 구간 Total Return\n\n")
        f.write("## 최적화 결과\n\n")

        f.write("### 전략별 최적 가중치 방식\n\n")
        f.write(
        ...
        for strategy in ["bt20_short", "bt20_ens", "bt120_long", "bt120_ens"]:
            ...
            f.write(
                "| Weighting | Net Sharpe (Holdout) | Net CAGR (Holdout) | Net Total Return (Holdout) | "
                "Net Sharpe (Dev) | Net CAGR (Dev) | Net Total Return (Dev) |\n"
            )
            f.write(
                "|-----------|---------------------|-------------------|---------------------------|"
                "----------------|----------------|----------------------|\n"
            ...
            if len(df_strategy) >= 2:
                ...
                if not equal_row.empty and not softmax_row.empty:
                    ...
                    if pd.notna(equal_tr) and pd.notna(softmax_tr):
                        ...
                        f.write(
                            f"- **{strategy}**: {better}가 더 우수 (차이: {diff:.4f}, {diff_pct:+.2f}%)\n"
                        )

        f.write("\n")
        f.write("### 권장사항\n\n")
        f.write("- 각 전략별로 최적 가중치 방식을 선택하여 config.yaml에 반영 권장\n")
        f.write("- Holdout 구간 성과를 기준으로 최적화 완료\n")
...
def main():
    ...
    for strategy in strategies:
        if (
            strategy in result["best_weighting"]
            and result["best_weighting"][strategy] is not None
        ):
