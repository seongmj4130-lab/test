    ...
    for holding_days in holding_days_options:
        ...
        for strategy in strategies:
            try:
                ...
                # 백테스트 실행
                bt_positions, bt_returns, bt_metrics, warns = run_backtest(
                    rebalance_scores, bt_config
                )
                ...
                if len(holdout) > 0:
                    ...
                    print(
                        f"    ✅ 완료: Sharpe {result['net_sharpe']:.3f}, CAGR {result['net_cagr']:.1%}"
                    )

                else:
                    print("    ❌ Holdout 데이터 없음")
