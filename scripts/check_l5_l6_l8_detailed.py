...
common_dates_l8 = set(l6_scores_interval20["date"].unique()) & set(
...
if common_dates_l8:
    ...
    if l6_score_col and l8_score_col:
        ...
        merged_col_l8 = (
            "score_total_l8" if "score_total_l8" in merged.columns else "score_total"
        )
...
if (
    "score_ens" in l6_scores.columns
    and "score_short" in l6_scores.columns
    and "score_long" in l6_scores.columns
):
    valid = l6_scores[
    ...
    if len(valid) > 0:
        ...
        print("  score_ens 계산 확인 (weight_short=0.5, weight_long=0.5):")
        ...
        if max_diff >= 0.0001:
            ...
            valid["diff"] = diff
            print(
                valid.nlargest(5, "diff")[
                    ["ticker", "date", "score_short", "score_long", "score_ens", "diff"]
...
print(
    f'L5 Long:  {l5_long["date"].min()} ~ {l5_long["date"].max()} ({len(l5_long):,}행)'
)
print(
    f'L8 Short: {l8_short["date"].min()} ~ {l8_short["date"].max()} ({len(l8_short):,}행)'
)
print(
    f'L8 Long:  {l8_long["date"].min()} ~ {l8_long["date"].max()} ({len(l8_long):,}행)'
)
print(
    f'L6 기본: {l6_scores["date"].min()} ~ {l6_scores["date"].max()} ({len(l6_scores):,}행, {l6_scores["date"].nunique()}개 날짜)'
)
print(
    f'L6 Interval20: {l6_scores_interval20["date"].min()} ~ {l6_scores_interval20["date"].max()} ({len(l6_scores_interval20):,}행, {l6_scores_interval20["date"].nunique()}개 날짜)'
)
