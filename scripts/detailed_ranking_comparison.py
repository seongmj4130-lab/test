...
def compare_specific_date_rankings(date_str="2023-06-21"):
    ...
    holdout_integrated_filtered = holdout_integrated[
        holdout_integrated["날짜"] == date_str
    ]
    ...
    if not holdout_short_filtered.empty and not daily_short_filtered.empty:
        ...
        comparisons.append(
            (
                "단기",
                h_short_1st["종목명(ticker)"],
                str(d_short_1st["ticker"]),
                short_match,
            )
        )
    ...
    if not holdout_long_filtered.empty and not daily_long_filtered.empty:
        ...
        comparisons.append(
            (
                "장기",
                h_long_1st["종목명(ticker)"],
                str(d_long_1st["ticker"]),
                long_match,
            )
        )
    ...
    for strategy, holdout_ticker, daily_ticker, match in comparisons:
        ...
        print(
            f"  {strategy}: Holdout={holdout_ticker} | 일간={daily_ticker} | {status}"
        )
    ...
    for strategy, holdout_df, daily_df, score_col, daily_score_col in [
        ...
        if not holdout_df.empty and not daily_df.empty:
            ...
            print(
                f"    Holdout: {h_scores.mean():.6f} (min: {h_scores.min():.6f}, max: {h_scores.max():.6f})"
            )
            print(
                f"    일간: {d_scores.mean():.6f} (min: {d_scores.min():.6f}, max: {d_scores.max():.6f})"
            )
    ...
    for strategy, holdout_df, daily_df in [
        ...
        if not holdout_df.empty and not daily_df.empty:
            ...
            print("    일간: top1/top2/top3_feature_group 컬럼 사용")
    ...
    for strategy, holdout_ticker, daily_ticker, match in comparisons:
        if match:
            ...
            print(
                f"   ❌ {strategy} 전략: 불일치 (Holdout: {holdout_ticker} vs 일간: {daily_ticker})"
            )
