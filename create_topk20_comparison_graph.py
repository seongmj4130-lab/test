...
def load_backtest_returns():
    ...
    for strategy_code, strategy_name in strategies.items():
        ...
        try:
            ...
            if len(df_holdout) > 0:
                # 누적 수익률 계산
                df_holdout["cumulative_return"] = (
                    1 + df_holdout["net_return"]
                ).cumprod() - 1

                returns_data[strategy_name] = df_holdout[
                    ["date", "net_return", "cumulative_return"]
                ]
...
def calculate_performance_metrics(returns_data):
    ...
    # CSV로 저장
    metrics_df.to_csv(
        "results/topk20_performance_metrics.csv", index=False, encoding="utf-8-sig"
    )
