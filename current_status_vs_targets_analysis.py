        ...
        for period in current[strategy].keys():
            if period in target[strategy]:
                gaps[strategy][period] = {
                    "sharpe_gap": current[strategy][period]["sharpe"]
                    - target[strategy][period]["sharpe"],
                    "cagr_gap": current[strategy][period]["cagr"]
                    - target[strategy][period]["cagr"],
                    "mdd_gap": current[strategy][period]["mdd"]
                    - target[strategy][period]["mdd"],  # 더 작은 값이 좋음
                    "calmar_gap": current[strategy][period]["calmar"]
                    - target[strategy][period]["calmar"],
                }
    return gaps
...
def generate_final_report(
    current_perf, target_perf, gaps, priorities, improvement_plan
):
    """최종 보고서 생성"""
    ...
    for strategy in current_perf.keys():
        ...
        print(f"{strategy}:")
        print(
            f"    현재 Sharpe: {current_best[1]['sharpe']:.3f}, 목표 Sharpe: {target_best[1]['sharpe']:.3f}"
        )
        print(
            f"    현재 CAGR: {current_best[1]['cagr']:.2f}%, 목표 CAGR: {target_best[1]['cagr']:.2f}%"
        )
    print("\n❌ 주요 격차 분석:")
    print("-" * 70)
