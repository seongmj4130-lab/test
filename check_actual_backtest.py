...
if perf_path.exists():
    ...
    for ppt_strategy, ppt_metrics in ppt_data.items():
        # 백테스트 데이터에서 해당 전략 찾기
        matching_rows = holdout_data[
            holdout_data["strategy"].str.contains(
                ppt_strategy.replace("bt", "").replace("_", "")
            )
        ]
        if len(matching_rows) > 0:
            ...
            status = (
                "✅ 일치" if return_diff < 0.1 and sharpe_diff < 0.01 else "❌ 불일치"
            )

            print(f"{ppt_strategy}:")
            print(
                f'  PPT: {ppt_metrics["total_return"]}%, Sharpe {ppt_metrics["sharpe"]}'
            )
