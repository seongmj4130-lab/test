...
**버전**: Phase 9 + 뉴스 피처 추가 (2026-01-04) + 투트랙 구조 리팩토링 (2026-01-05) + 시장 국면 분류 개선 (2026-01-06)  
**대상**: 사내 퀀트/ML 개발자, 리서처  
...
**하이퍼파라미터**:
- `ridge_alpha`: 1.0 (기본값, `config.yaml::l5.ridge_alpha`)
...
**방법 선택**: `percentile` (기본값) 또는 `zscore`
...
**설정**: `config.yaml::l8.normalization_method = "percentile"`, `use_sector_relative = true`

### 5.2 피처 그룹 & 가중치

**설정 파일**: `configs/feature_groups.yaml`
...
- **BT120 Holdout Rank IC**: 0.1005
- **BT120 Holdout ICIR**: 0.9299
- **BT120 Long/Short Alpha**: 7.39% (CAGR)

### 6.2 Alpha Quality 계산

**위치**: 백테스트(L7) 단계에서 계산 (이 보고서 범위 밖)
...
**Phase 9 기준 최종 성과** (BT120 기준):

| 지표 | Dev | Holdout | 목표 | 달성 여부 |
|------|-----|---------|------|----------|
| **Net Sharpe** | 0.1730 | 0.4565 | ≥ 0.45 | ✅ Holdout 달성 |
| **Net CAGR** | 6.58% | 14.92% | ≥ 14% | ✅ Holdout 달성 |
| **Net MDD** | -50.73% | -9.20% | ≤ -10% | ✅ Holdout 달성 |
| **Calmar Ratio** | 0.1298 | 1.6209 | ≥ 1.5 | ✅ Holdout 초과 달성 |
| **Rank IC** | -0.0015 | 0.1005 | ≥ 0.04 | ✅ Holdout 초과 달성 |
| **ICIR** | -0.0147 | 0.9299 | ≥ 0.5 | ✅ Holdout 초과 달성 |
| **Long/Short Alpha** | 4.34% | 7.39% | > 0 | ✅ 달성 |

**뉴스 피처 추가 후 최종 성과** (BT20 기준, 2026-01-04):

| 지표 | Dev | Holdout | 목표 | 달성 여부 |
|------|-----|---------|------|----------|
| **Net Sharpe** | -0.2136 | **0.7370** | ≥ 0.50 | ✅ Holdout 초과 달성 |
| **Net CAGR** | -3.67% | **12.08%** | ≥ 10% | ✅ Holdout 달성 |
| **Net MDD** | -39.64% | -8.53% | ≤ -10% | ✅ Holdout 달성 |
| **Calmar Ratio** | -0.0925 | **1.4155** | ≥ 0.8 | ✅ Holdout 초과 달성 |
| **Hit Ratio** | 46.34% | **65.22%** | ≥ 55% | ✅ Holdout 초과 달성 |
| **Avg Turnover** | 92.44% | **73.36%** | ≤ 500% | ✅ Holdout 달성 |

**데이터 소스**: 
- BT120: `artifacts/reports/phase8_final_metrics_report.md`
- BT20 (뉴스 피처 추가 후): `artifacts/reports/news_features_performance_comparison.md`
