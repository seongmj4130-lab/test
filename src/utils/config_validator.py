...
class ConfigValidator:
    ...
    def validate_types(
        self, config: dict[str, Any], type_specs: dict[str, type]
    ) -> bool:
        """
        키별 타입을 검증합니다.
        ...
        for key_path, (min_val, max_val) in range_specs.items():
            ...
            if value is not None:
                ...
                if min_val is not None and value < min_val:
                    self.errors.append(
                        f"값이 최소값보다 작음: {key_path} = {value} < {min_val}"
                    )
                    return False

                if max_val is not None and value > max_val:
                    self.errors.append(
                        f"값이 최대값보다 큼: {key_path} = {value} > {max_val}"
                    )
                    return False
        return True
        ...
        for key_path in date_keys:
            ...
            if value is not None:
                ...
                try:
                    ...
                    self.errors.append(
                        f"잘못된 날짜 형식: {key_path} = {value} (YYYY-MM-DD 필요)"
                    )
                    return False
        return True
    ...
    def validate_backtest_params(self, config: dict[str, Any]) -> bool:
        ...
        if rebalance_interval is not None:
            if not isinstance(rebalance_interval, int) or rebalance_interval <= 0:
                self.errors.append(
                    f"rebalance_interval은 양의 정수여야 함: {rebalance_interval}"
                )
                return False
