...
class RankingRebalanceConfig:
    ...
    alpha_long: Optional[float] = None    # 장기 랭킹 가중치 (0.0~1.0, alpha_short가 있으면 1-alpha_short로 자동 계산)
...
def build_rebalance_scores_from_ranking(
    *,
    ...
    if dataset_daily is None or not isinstance(dataset_daily, pd.DataFrame) or dataset_daily.empty:
        raise ValueError("dataset_daily is empty")
    if cv_folds_short is None or not isinstance(cv_folds_short, pd.DataFrame) or cv_folds_short.empty:
        raise ValueError("cv_folds_short is empty")
    if universe_k200_membership_monthly is None or universe_k200_membership_monthly.empty:
        ...
        if regime_alpha_config and regime_enabled:
            ...
            if ohlcv_daily is None or ohlcv_daily.empty:
                ...
                try:
                    ...
                    warns.append(f"[L6R Dual Horizon] 시장 국면별 α 조정 활성화: {len(market_regime_df_for_alpha)}개 날짜")
    ...
    if use_dual_horizon:
        ...
        feature_groups_config = l8.get("feature_groups_config", "configs/feature_groups.yaml")
        feature_weights_config = l8.get("feature_weights_config", None)  # [IC 최적화] 최적 가중치 파일 경로
        regime_aware_weights_config = l8.get("regime_aware_weights_config", None)  # [국면별 전략] 국면별 가중치 파일 경로
        ...
        if feature_weights_config:
            ...
            if weights_path.exists():
                try:
                    ...
                    warns.append(f"[L6R IC 최적화] 최적 가중치 로드 완료: {len(optimal_feature_weights)}개 피처")
                except Exception as e:
            ...
            if len(regime_weights_config) > 0:
                ...
                if ohlcv_daily is None or ohlcv_daily.empty:
                    ...
                    try:
                        ...
                        warns.append(f"[L6R 국면별 전략] 시장 국면 데이터 생성 완료: {len(market_regime_df)}개 날짜")
                    except Exception as e:
    ...
    if "bt20_pro" in str(strategy_name) or "bt20_pro" in str(cfg.get("l7", {})):
        logger.info("[bt20 프로페셔널] L6R에서 rebalance_interval=1로 설정하여 L7에서 적응형 리밸런싱 적용")
    ...
    if rebalance_interval == 1:
        ...
        if not req_cols.issubset(set(cv_folds_short.columns)):
            raise KeyError(f"cv_folds_short missing required columns: {sorted(list(req_cols - set(cv_folds_short.columns)))}")
        ...
        if dup_dates:
            warns.append("[L6R] Duplicate rebalance dates in cv_folds_short.test_end -> resolving by phase priority(holdout>dev)")
            pr = folds["phase"].map({"dev": 0, "holdout": 1}).fillna(0).astype(int)
            folds = folds.assign(_prio=pr).sort_values(["test_end", "_prio"]).drop_duplicates(["test_end"], keep="last")
            folds = folds.drop(columns=["_prio"])

        rebal_map = folds[["test_end", "phase"]].rename(columns={"test_end": "date"}).copy()
        warns.append(f"[L6R rebalance_interval] 월별 리밸런싱 사용 (cv_folds_short.test_end): {len(rebal_map)}개 날짜")
    else:
        ...
        if use_dual_horizon:
            ...
            if ranking_short_daily is None or ranking_short_daily.empty:
                raise ValueError("[L6R rebalance_interval] rebalance_interval > 1 requires ranking_short_daily")
        ...
        warns.append(f"[L6R rebalance_interval] 일별 리밸런싱 사용 (interval={rebalance_interval}): {len(rebal_map)}개 날짜 (전체 {len(all_dates)}개 중)")
    ...
    if use_dual_horizon:
        ...
        if score_source == "rank_total":
            ...
            r["score_short_norm"] = -pd.to_numeric(r["rank_total_short"], errors="coerce")
    ...
    if ret_fwd_col_short not in ds.columns:
        raise KeyError(f"[L6R] dataset_daily missing required return column: {ret_fwd_col_short}")

    ret_info_short = ds[["date", "ticker", ret_fwd_col_short]].copy()
    ret_info_short = ret_info_short.drop_duplicates(["date", "ticker"])
    r = r.merge(ret_info_short, on=["date", "ticker"], how="left", validate="one_to_one")
    ...
    if ret_fwd_col_long in ds.columns:
        ...
        r = r.merge(ret_info_long, on=["date", "ticker"], how="left", validate="one_to_one")
    ...
    for hd in dynamic_holding_days:
        ...
        if ret_fwd_col in ds.columns:
            ...
            r = r.merge(ret_info, on=["date", "ticker"], how="left", validate="one_to_one")
            r[f"true_{hd}d"] = r[ret_fwd_col].astype(float)  # [North Star] BT120 지원
        warns.append(f"[L6R North Star] true_long 부착 완료 (BT120 지원): {ret_fwd_col_long}")
    else:
        warns.append(f"[L6R North Star] true_long 부착 스킵: {ret_fwd_col_long} 컬럼 없음 (BT120 비활성화)")
        r["true_long"] = np.nan
    ...
    if "score_ens" not in r.columns:
        ...
        if score_source == "rank_total":
            ...
            r["score_ens"] = (-pd.to_numeric(r["rank_total"], errors="coerce")).astype(float)
        else:
            if "score_total" not in r.columns:
                raise KeyError("[L6R] score_total not found in ranking_daily output")
            r["score_ens"] = pd.to_numeric(r["score_total"], errors="coerce").astype(float)

    # 8) L7이 기대하는 최소 스키마로 정리 (North Star: true_short/true_long 모두 포함)
    # [단기/장기 랭킹 분리 분석] score_total_short와 score_total_long 포함
    out_cols = ["date", "ticker", "phase", "score_ens", cfg_rank.return_col, "true_long", "score_total", "rank_total", "in_universe"]
    ...
    summary["ym"] = pd.to_datetime(summary["date"], errors="coerce").dt.to_period("M").astype(str)
    summary = summary.merge(un_counts, on="ym", how="left")
    summary["coverage_vs_universe_pct"] = summary["n_tickers"] / summary["universe_n_tickers"].replace(0, np.nan) * 100.0
    ...
    quality = {
        "scoring_ranking": {
            ...
            "true_long_attached": bool("true_long" in out.columns and out["true_long"].notna().any()),  # [North Star] BT120 지원 여부 (JSON 직렬화를 위해 bool 변환)
            "rows": int(len(out)),
    ...
    return out.sort_values(["phase", "date", "score_ens", "ticker"], ascending=[True, True, False, True]).reset_index(drop=True), summary, quality, warns
...
def run_L6R_ranking_scoring(
    ...
    *,
    ...
    # 앙상블 가중치 로드 확인 로그
    if ensemble_weights:
        warns.append(f"[L6R] 앙상블 가중치 로드 완료: 단기 {len(ensemble_weights.get('short', {}))}개, 장기 {len(ensemble_weights.get('long', {}))}개 모델")
    ...
    rebalance_interval = l7_rebalance_interval if l7_rebalance_interval is not None else (l6r_rebalance_interval if l6r_rebalance_interval is not None else 1)
