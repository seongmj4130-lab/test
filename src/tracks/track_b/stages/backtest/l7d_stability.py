...
def build_bt_rolling_sharpe(
    ...
    *,
    ...
    df[net_return_col] = (
        df[net_return_col].replace([np.inf, -np.inf], np.nan).fillna(0.0)
    )
    ...
    if not res.empty and res.duplicated(keys).any():
        raise ValueError(
            "[L7D] bt_rolling_sharpe must be unique on (date,phase,net_return_col_used)"
        )
...
def run_l7d_stability_from_artifacts(
    *,
    ...
    if want_col not in bt_returns.columns:
        ...
        warns.append(
            f"[L7D] net_return_col '{want_col}' not found. fallback to '{net_col_used}'"
        )

    yearly = build_bt_yearly_metrics(
        bt_returns, holding_days=holding_days, net_return_col=net_col_used
    )
    ...
    if (
        bt_equity_curve is not None
        and not bt_equity_curve.empty
        and "equity" in bt_equity_curve.columns
    ):
