...
def _apply_dynamic_params_for_holding_days(cfg: BacktestConfig, strategy_name: str = None) -> BacktestConfig:
    """
    ...
    try:
        ...
        config_path = current_file.parent.parent.parent.parent.parent / "configs" / "config.yaml"

        # 절대 경로로도 시도
        if not config_path.exists():
            config_path = Path("C:/Users/seong/OneDrive/Desktop/bootcamp/000_code/configs/config.yaml")
        if not config_path.exists():
            logger.warning(f"[동적 파라미터] config.yaml을 찾을 수 없습니다: {config_path}")
            return cfg
        ...
        if dynamic_available:
            ...
            logger.info(f"[동적 파라미터] holding_days={holding_days} 파라미터 준비: {dynamic_param_values}")
        else:
            dynamic_param_values = {}
            if not use_dynamic_fallback:
                logger.info(f"[동적 파라미터] holding_days={holding_days}에 대한 동적 파라미터 없음 (config 우선 모드)")

        # 파라미터 적용 로직
        if use_dynamic_fallback:
            # 동적 파라미터 우선 모드 (기존 방식)
            logger.info(f"[동적 파라미터] 동적 우선 모드: 동적 파라미터 우선 적용")
            updated_cfg = cfg.__class__(
                ...
                tranche_holding_days=dynamic_param_values.get('tranche_days', cfg.tranche_holding_days),
                ...
                rebalance_interval=dynamic_param_values.get('rebalance_interval', cfg.rebalance_interval),
                diversify_enabled=cfg.diversify_enabled,
                group_col=cfg.group_col,
                max_names_per_group=cfg.max_names_per_group,
                regime_enabled=dynamic_param_values.get('regime_enabled', cfg.regime_enabled),
            ...
            target_volatility=dynamic_param_values.get('target_vol', cfg.target_volatility),  # 동적 적용
            ...
            logger.info(f"[동적 파라미터] config 우선 모드: 전략별 설정 우선 적용")
            ...
            updated_cfg = cfg.__class__(
                ...
                overlapping_tranches_enabled=cfg.overlapping_tranches_enabled,
                tranche_holding_days=dynamic_param_values.get('tranche_days') or cfg.tranche_holding_days,
                tranche_max_active=cfg.tranche_max_active,
                tranche_allocation_mode=cfg.tranche_allocation_mode,
        ...
        logger.info(f"[동적 파라미터] 파라미터 적용 완료: top_k={updated_cfg.top_k}, cost_bps={updated_cfg.cost_bps}, target_vol={updated_cfg.target_volatility}")
        return updated_cfg
...
def _pick_ret_col(df: pd.DataFrame, preferred: str, holding_days: int = 20) -> str:
    ...
    if ret_cols:
        ...
        best_match = min(ret_cols, key=lambda x: abs(int(x.split("_")[-1].replace("d", "")) - holding_days))
        return best_match
...
def _long_short_alpha(scores: pd.Series, rets: pd.Series, k: int) -> float:
    ...
    if k <= 0:
        return float("nan")
    df = pd.DataFrame({"s": pd.to_numeric(scores, errors="coerce"), "r": pd.to_numeric(rets, errors="coerce")})
    df = df.dropna()
    if len(df) < max(2 * k, 6):
        return float("nan")
    ...
    *,
    ...
    for current_date in all_dates:
            ...
            if not current_weights:
                ...
                if volatility_adjustment_enabled and len(volatility_history) >= 2:
                    exposure_multiplier = _calculate_volatility_adjustment(
                        recent_returns=np.array(volatility_history[-volatility_lookback_days:]),
                ...
                if risk_scaling_enabled and market_regime is not None:
                    # 현재 국면 찾기
                    current_regime_data = market_regime[market_regime['date'] <= current_date]
        ...
        daily_returns.append({
            ...
            'valid_positions': valid_positions if 'valid_positions' in locals() else 0,
...
def _convert_daily_to_monthly_returns(daily_returns: pd.DataFrame) -> pd.DataFrame:
    ...
    for ym, group in daily_returns.groupby('year_month'):
        ...
        monthly_returns.append({
            ...
            'total_trading_cost': group['daily_trading_cost'].sum() if 'daily_trading_cost' in group.columns else 0.0,
            'n_rebalance_dates': group['is_rebalance_date'].sum() if 'is_rebalance_date' in group.columns else 0,
        })
    ...
    *,
    ...
    if cfg.regime_enabled:
        if market_regime is None or len(market_regime) == 0:
            ...
            if "date" not in market_regime.columns or "regime" not in market_regime.columns:
                ...
                warns.append(f"[Stage5] 시장 국면 데이터 로드 완료: {len(regime_dict)}개 날짜")
    else:
        regime_enabled_actual = False
    ...
    for phase, dphase in df_sorted.groupby(phase_col, sort=False):
        ...
        if rebalance_interval > 1:
            ...
            warns.append(f"[Phase 7 Step 2] rebalance_interval={rebalance_interval} 적용: {len(rebalance_dates_filtered)}/{len(rebalance_dates_all)}개 날짜 사용")
        else:
        ...
        logger.info(f"[L7 일별 백테스트] Phase '{phase}' 시작 (리밸런싱 {len(rebalance_dates_filtered)}개)")
            ...
            if len(phase_date_range) > 0:
                ...
                for _, row in daily_portfolio_returns.iterrows():
                    daily_returns_rows.append({
                        ...
                        "gross_return": row['portfolio_return'] + row['trading_cost'],  # 총 수익률 + 비용 = 총 수익률
                        "net_return": row['portfolio_return'],  # 비용 차감 후 수익률
                ...
                warns.append(f"[일별 백테스트] Phase '{phase}': {len(daily_portfolio_returns)}일 처리 완료")

        t_phase_end = time.time()
        phase_time = t_phase_end - t_phase_start
        logger.info(f"[L7 일별 백테스트] Phase '{phase}' 완료: {len(rebalance_dates_filtered)}개 리밸런싱, 총 {phase_time:.1f}초")

    # 결과 데이터프레임 생성
    bt_positions = pd.DataFrame(positions_rows).sort_values(["phase", "date", "ticker"]).reset_index(drop=True)
    bt_returns = pd.DataFrame(daily_returns_rows).sort_values(["phase", "date"]).reset_index(drop=True)
    ...
    for phase, g in bt_returns.groupby("phase", sort=False):
        ...
        for dt, r in zip(g["date"], g["net_return"]):
            eq *= (1.0 + float(r))
            eq_rows.append({"date": dt, "phase": phase, "equity": float(eq), "drawdown": 0.0})  # DD 계산 생략
    ...
    for phase, g in bt_returns.groupby("phase", sort=False):
        ...
        met_rows.append({
            ...
            "n_rebalances": len(g[g["is_rebalance_date"] == True]) if "is_rebalance_date" in g.columns else len(g),
    ...
    quality = {
        "backtest": {
            ...
            "monthly_returns": bt_monthly_returns.to_dict('records') if len(bt_monthly_returns) > 0 else [],
        }
    }
...
def run_backtest(
    ...
    *,
    ...
    daily_prices: Optional[pd.DataFrame] = None,  # [개선안: 일별 백테스트] 일별 가격 데이터 (date, ticker, close)
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, dict, List[str]]:
    warns: List[str] = []
    ...
    score_col = _pick_score_col(rebalance_scores, cfg.score_col)
    ret_col = _pick_ret_col(rebalance_scores, cfg.ret_col, cfg.holding_days)
    print(f"[DEBUG] Strategy {cfg.score_col}: holding_days={cfg.holding_days}, selected ret_col={ret_col}")
    ...
    for phase, dphase in df_sorted.groupby(phase_col, sort=False):
        ...
        if cfg.adaptive_rebalancing_enabled:
            ...
            # 적응형 리밸런싱 설정 구성
            adaptive_config = {
                'min_rebalance_days': cfg.rebalance_intervals.get('strong', 15) if cfg.rebalance_intervals else 15,
                'max_rebalance_days': cfg.rebalance_intervals.get('weak', 25) if cfg.rebalance_intervals else 25,
                'default_rebalance_days': 20,
                'signal_strength_threshold_high': cfg.signal_strength_thresholds.get('strong', 0.8) if cfg.signal_strength_thresholds else 0.8,
                'signal_strength_threshold_low': cfg.signal_strength_thresholds.get('weak', 0.6) if cfg.signal_strength_thresholds else 0.6,
            ...
            if not intervals_df.empty:
                # 적응형 간격에 따른 날짜 필터링
                filtered_dates = adaptive_rebalancer.filter_adaptive_dates(rebalance_dates_all, intervals_df)

                rebalance_dates_filtered = sorted(filtered_dates)

                logger.info(f"[bt20 프로페셔널] 적응형 리밸런싱 적용 완료: {len(rebalance_dates_filtered)}/{len(rebalance_dates_all)}개 날짜 선택")
                logger.info(f"[bt20 프로페셔널] 평균 리밸런싱 간격: {intervals_df['rebalance_interval'].mean():.1f}일")

                # 간격 분포 로깅
                interval_counts = intervals_df['rebalance_interval'].value_counts()
                logger.info(f"[bt20 프로페셔널] 간격 분포: {interval_counts.to_dict()}")
            else:
                logger.warning("[bt20 프로페셔널] 적응형 간격 계산 실패, 기본 로직 사용")
                rebalance_dates_filtered = rebalance_dates_all
        ...
        if cfg.adaptive_rebalancing_enabled:
            ...
            rebalance_dates_filtered = rebalance_dates_all
            logger.info(f"[bt20 프로페셔널] L6R 필터링 우회, 모든 날짜 사용: {len(rebalance_dates_all)}개 날짜")
        ...
        tranche_alloc_mode = str(getattr(cfg, "tranche_allocation_mode", "fixed_equal")).strip().lower()
        ...
        for dt, g in dphase.groupby(date_col, sort=True):
            ...
            if cfg.overlapping_tranches_enabled:
                ...
                tranches = [
                    t for t in tranches
                    if int((dt_ts - pd.to_datetime(t["open_date"])).days) < tranche_holding_days
                ]
                ...
                g_new_sel = g_new_sel.sort_values([score_col, ticker_col], ascending=[False, True]).reset_index(drop=True)
                w_new = _weights_from_scores(g_new_sel[score_col], cfg.weighting, cfg.softmax_temp)
                tranche_w = {t: float(wi) for t, wi in zip(g_new_sel[ticker_col].tolist(), w_new.tolist())}
                tranches.append({"open_date": dt_ts, "w": tranche_w, "top_k": int(current_top_k)})
                # 동시 보유 트랜치 수 제한(가장 오래된 것부터 제거)
                if len(tranches) > tranche_max_active:
                    tranches = sorted(tranches, key=lambda x: pd.to_datetime(x["open_date"]))[-tranche_max_active:]
                ...
                ic = _safe_corr(g_alpha[score_col], g_alpha[ret_col]) if len(g_alpha) else float("nan")
                ric = _rank_ic(g_alpha[score_col], g_alpha[ret_col]) if len(g_alpha) else float("nan")
                ...
                gross_ret = float(sum(float(wi) * float(ret_dict.get(tk, 0.0) or 0.0) for tk, wi in new_w.items()))
                ...
                selection_diagnostics_rows.append(
                    {
                        ...
                        "filled_from_next_rank": diagnostics.get("filled_from_next_rank"),
                        "drop_reasons": str(diagnostics.get("drop_reasons")) if diagnostics.get("drop_reasons") else None,
            ...
            returns_row = {
                ...
                "rank_ic": float(ric) if pd.notna(ric) else np.nan,
                "long_short_alpha": float(ls_alpha) if pd.notna(ls_alpha) else np.nan,
                "daily_trading_cost": float(daily_trading_cost),  # [개선안 1번][개선안 3번] 실제 차감된 총 비용(하위호환)
                "turnover_cost": float(turnover_cost),  # [개선안 1번] 수수료/세금/스프레드 비용(턴오버 기반)
                "slippage_cost": float(slippage_cost),  # [개선안 3번] 슬리피지 비용(턴오버 기반)
    ...
    if len(selection_diagnostics) > 0 and "phase" in selection_diagnostics.columns and "date" in selection_diagnostics.columns:
        selection_diagnostics = selection_diagnostics.sort_values(["phase", "date"]).reset_index(drop=True)
    else:
    ...
    for phase, g in bt_returns.groupby("phase", sort=False):
        ...
        ls_alpha_ann = float(ls_alpha_mean * periods_per_year) if pd.notna(ls_alpha_mean) else float("nan")
    ...
    if len(runtime_profile) > 0 and "phase" in runtime_profile.columns and "date" in runtime_profile.columns:
