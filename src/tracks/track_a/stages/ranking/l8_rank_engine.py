...
def run_L8_rank_engine(
    ...
    *,
    ...
    use_sector_relative = l8.get(
        "use_sector_relative", True
    )  # [Stage8] sector-relative 정규화 사용 여부
    sector_col = l8.get("sector_col", "sector_name")  # [Stage8] 섹터 컬럼명
    enforce_group_balance = l8.get(
        "enforce_group_balance", True
    )  # [Stage8] 그룹별 밸런스 강제 여부
    ...
    if panel_merged_daily is None:
        ...
        if artifact_exists(panel_merged_path):
            ...
            warns.append(
                f"[Stage8] panel_merged_daily를 base_interim_dir에서 로드: {panel_merged_path}"
            )
    ...
    if "in_universe" not in input_df.columns:
        warns.append(
            "[L8] 'in_universe' column not found. All rows will be treated as in_universe=True."
        )
    ...
    if feature_groups_config:
        ...
        if not feature_groups_path.exists():
            warns.append(
                f"[L8] Feature groups config not found: {feature_groups_path}. Using equal weights."
            )
            feature_groups_path = None
    ...
    if regime_aware_config and regime_enabled:
        ...
        if len(regime_weights_config) > 0:
            warns.append(
                f"[L8 국면별 전략] 국면별 가중치 로드 완료: {list(regime_weights_config.keys())}"
            )
            ...
            try:
                market_regime_df = build_market_regime(
                    ...
                    end_date=end_date,
                    lookback_days=cfg.get("l7", {})
                    .get("regime", {})
                    .get("lookback_days", 60),
                    threshold_pct=cfg.get("l7", {})
                    .get("regime", {})
                    .get("threshold_pct", 0.0),
                )
                warns.append(
                    f"[L8 국면별 전략] 시장 국면 데이터 생성 완료: {len(market_regime_df)}개 날짜"
                )
                # [국면 세분화] 5단계 국면 분포 출력
                ...
                warns.append(
                    f"[L8 국면별 전략] 시장 국면 데이터 생성 실패: {e}. 국면별 가중치 비활성화."
                )
        ...
        if feature_weights_config:
            ...
            if weights_path.exists():
                try:
                    ...
                    warns.append(
                        f"[L8 IC 최적화] 최적 가중치 로드 완료: {len(optimal_feature_weights)}개 피처 ({weights_path})"
                    )
                except Exception as e:
                    warns.append(
                        f"[L8 IC 최적화] 최적 가중치 로드 실패: {e}. feature_groups 사용."
                    )
            else:
                warns.append(
                    f"[L8 IC 최적화] 최적 가중치 파일이 없습니다: {weights_path}. feature_groups 사용."
                )
    ...
    if use_sector_relative and sector_col in input_df.columns:
        if input_df[sector_col].notna().sum() > 0:
            ...
            warns.append(
                f"[Stage8] {sector_col} 컬럼이 모두 NaN이어서 전체 시장 기준 정규화 사용"
            )
    else:
        if use_sector_relative:
            warns.append(
                f"[Stage8] {sector_col} 컬럼이 없어서 전체 시장 기준 정규화 사용"
            )
    ...
    if enforce_group_balance and feature_groups_path and feature_groups_path.exists():
        ...
        for group_name, group_features in groups_map.items():
            ...
            warns.append(
                f"[Stage8] 그룹 '{group_name}': {n_features}개 피처, 목표 가중치 {target_weight:.2%}"
            )
        ...
        if ungrouped:
            warns.append(
                f"[Stage8] 그룹에 속하지 않은 피처 {len(ungrouped)}개: {', '.join(ungrouped[:5])}..."
            )
    ...
    try:
        ...
        use_feature_groups = (
            feature_groups_path if optimal_feature_weights is None else None
        )
        ...
        if actual_sector_col:
            if actual_sector_col in ranking_daily.columns:
                ...
                warns.append(
                    f"[Stage8] {actual_sector_col}이 이미 ranking_daily에 포함됨: {ranking_daily[actual_sector_col].notna().sum():,} / {len(ranking_daily):,} 행"
                )
                ...
                warns.append(
                    f"[Stage8] {actual_sector_col}을 ranking_daily에 병합 완료: {ranking_daily[actual_sector_col].notna().sum():,} / {len(ranking_daily):,} 행"
                )
            else:
                warns.append(
                    f"[Stage8] {actual_sector_col}이 input_df에 없어 ranking_daily에 포함하지 않음"
                )
    except Exception as e:
    ...
    for date, group in ranking_daily.groupby("date", sort=False):
        ...
        if not ranks.equals(expected_ranks):
            ...
            if ranks.duplicated().any():
                warns.append(
                    f"[L8] Duplicate ranks found on {date}. Ranks: {ranks.tolist()[:10]}..."
                )
    ...
    for date, group in ranking_daily_final.groupby("date", sort=False):
        ...
        # Top 20
        top20 = universe_group.nsmallest(20, "rank_total")[
            ["ticker", "score_total", "rank_total"]
        ].copy()
        top20["snapshot_type"] = "top20"
        top20["date"] = date

        # Bottom 20
        bottom20 = universe_group.nlargest(20, "rank_total")[
            ["ticker", "score_total", "rank_total"]
        ].copy()
    ...
    if snapshot_data:
        ...
        ranking_snapshot = ranking_snapshot[
            ["date", "snapshot_type", "ticker", "score_total", "rank_total"]
        ].copy()
    else:
        ranking_snapshot = pd.DataFrame(
            columns=["date", "snapshot_type", "ticker", "score_total", "rank_total"]
        )
