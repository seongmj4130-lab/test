...
class NewsSentimentConfig:
    ...
    rel_path: str = (
        "news_sentiment_daily.parquet"  # data/external/news_sentiment_daily.parquet
    )
...
def attach_news_sentiment_features(
    ...
    *,
    ...
    if existing_news_cols:
        warns.append(
            f"[L4 News] news features already exist -> skipped (from L3): {existing_news_cols}"
        )
        return ds, warns
    ...
    if len(keep) <= 2:
        warns.append(
            f"[L4 News] news file loaded but no known feature columns -> skipped. path={src_path}"
        )
        return ds, warns
    ...
    if lag_days != 0:
        ...
        warns.append(
            f"[L4 News] applied lag_days={lag_days} (news.date shifted +{lag_days}d)"
        )

    # 중복 제거
    if news.duplicated(["date", "ticker"]).any():
        before = len(news)
        news = news.sort_values(["date", "ticker"]).drop_duplicates(
            ["date", "ticker"], keep="last"
        )
        warns.append(
            f"[L4 News] dropped duplicate (date,ticker) rows: {before - len(news)}"
        )

    out = ds.merge(news, on=["date", "ticker"], how="left", validate="one_to_one")
    warns.append(
        f"[L4 News] attached cols={keep[2:]} from {src_path} (rows={len(out):,})"
    )
