...
def build_panel_merged_daily(
    *,
    ...
    if not f.empty:
        ...
        if fundamentals_effective_date_col in f.columns:
            ...
            if f[fundamentals_effective_date_col].isna().all():
                ...
                warns.append(
                    f"[L3] {fundamentals_effective_date_col} exists but all NaN -> using fallback calculation"
                )
                ...
                warns.append(
                    f"[L3] Using {fundamentals_effective_date_col} from fundamentals_annual (Stage 1)"
                )
        else:
            # 기존 방식: date + lag_days
            f["effective_date"] = f["date"] + pd.to_timedelta(
            ...
            warns.append(
                f"[L3] {fundamentals_effective_date_col} not found -> using date + {fundamental_lag_days} days (legacy)"
            )
    ...
    if universe_membership_monthly is not None and len(universe_membership_monthly) > 0:
        ...
        if missing_ym:
            warns.append(
                f"[L3] universe_membership_monthly missing months: {missing_ym[:10]} (total={len(missing_ym)})"
            )

    else:
        merged["in_universe"] = True
        warns.append(
            "[L3] universe_membership_monthly is empty -> in_universe forced True (NOT OK for K200 strategy)"
        )
    ...
    if filter_k200_members_only:
        ...
        after = len(merged)
        warns.append(
            f"[L3] filter_k200_members_only=True applied: {before} -> {after} rows"
        )
    ...
    if sector_map is not None and not sector_map.empty:
        if (
            "date" in sector_map.columns
            and "ticker" in sector_map.columns
            and "sector_name" in sector_map.columns
        ):
            ...
            if not sector.empty:
                ...
                sector_sorted = sector.sort_values(
                    ["date", "ticker"], kind="mergesort"
                ).reset_index(drop=True)
                ...
                warns.append(
                    "[Stage 4] sector_map이 비어있어 sector_name merge를 건너뜁니다."
                )
        else:
            warns.append(
                "[Stage 4] sector_map에 필요한 컬럼(date, ticker, sector_name)이 없어 merge를 건너뜁니다."
            )
    else:
        warns.append(
            "[Stage 4] sector_map이 제공되지 않아 sector_name을 추가하지 않습니다."
        )
    ...
    if pykrx_fundamentals_daily is not None and not pykrx_fundamentals_daily.empty:
        ...
        if "date" not in pykrx.columns or "ticker" not in pykrx.columns:
            warns.append(
                "[L1B] pykrx_fundamentals_daily에 date/ticker가 없어 병합을 건너뜁니다."
            )
        else:
            ...
            if not pykrx.empty:
                ...
                pykrx_sorted = pykrx.sort_values(
                    ["date", "ticker"], kind="mergesort"
                ).reset_index(drop=True)

                # 중복 제거 (같은 date, ticker에 여러 행이 있을 수 있음)
                pykrx_sorted = pykrx_sorted.drop_duplicates(
                    subset=["date", "ticker"], keep="last"
                )

                merged = merged_sorted.merge(
                    pykrx_sorted, on=["date", "ticker"], how="left"
                )

                pykrx_cols = ["PER", "PBR", "EPS", "BPS", "DIV", "market_cap"]
                added_cols = [c for c in pykrx_cols if c in merged.columns]
                warns.append(
                    f"[L1B] pykrx 재무데이터 병합 완료: {len(added_cols)}개 컬럼 추가 ({', '.join(added_cols)})"
                )
            else:
                warns.append(
                    "[L1B] pykrx_fundamentals_daily가 비어있어 병합을 건너뜁니다."
                )
    else:
        warns.append(
            "[L1B] pykrx_fundamentals_daily가 제공되지 않아 pykrx 재무데이터를 추가하지 않습니다."
        )
    ...
    if "market_cap" in merged.columns:
        ...
        if trading_value is not None:
            ...
            # 0으로 나누기 방지
            merged["turnover_ratio"] = merged["turnover_ratio"].replace(
    ...
    if "sector_name" in merged.columns and not merged.empty:
        ...
        for base_col, z_col in [
            ("debt_ratio", "debt_ratio_sector_z"),
            ("roe", "roe_sector_z"),
        ]:
            if base_col in merged.columns:
                ...
                merged[z_col] = merged.groupby(
                    ["date", "sector_name"], group_keys=False
                )[base_col].transform(calc_sector_z)

                # z-score가 계산된 행 수 확인
                z_count = merged[z_col].notna().sum()
                warns.append(
                    f"[Stage6] {z_col} 생성 완료: {z_count:,} / {len(merged):,} 행 (NaN={merged[z_col].isna().sum():,})"
                )
            else:
                warns.append(
                    f"[Stage6] {base_col} 컬럼이 없어 {base_col}_sector_z를 생성하지 않습니다."
                )
    else:
        warns.append(
            "[Stage6] sector_name이 없어 업종 내 z-score 피처를 생성하지 않습니다."
        )
    ...
    try:
        ...
        warns.append(
            f"[L3 Features] 팩터 추가 실패 (계속 진행): {type(e).__name__}: {e}"
        )
