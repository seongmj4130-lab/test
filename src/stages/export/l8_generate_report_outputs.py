...
def _load_config_paths(base_dir: Path, config_path: Optional[Path]) -> dict[str, Path]:
    ...
    if yaml is None:
        print(
            "[WARN] PyYAML이 없어 config.yaml 파싱을 건너뜁니다. 기본 경로로 진행합니다."
        )
        return paths
...
def _ensure_date_index(
    ...
    if found is None:
        raise ValueError(
            f"날짜 컬럼을 찾지 못했습니다. columns={list(df.columns)[:20]} ..."
        )
...
def _pick_col(
    df: pd.DataFrame, preferred: list[str], required: bool = True
) -> Optional[str]:
    for c in preferred:
        if c in df.columns:
            return c
    if required:
        raise ValueError(
            f"필요 컬럼을 찾지 못했습니다. 후보={preferred}, 실제={list(df.columns)[:50]}"
        )
    return None
...
def _monthly_table(ret: pd.Series) -> pd.DataFrame:
    ...
    pivot = out.pivot_table(
        index="year", columns="month", values="monthly_return", aggfunc="first"
    )
    pivot = pivot.sort_index()
    pivot.columns = [f"{int(c):02d}" for c in pivot.columns]
    pivot["YTD"] = pivot.apply(
        lambda row: float((1.0 + row.dropna()).prod() - 1.0), axis=1
    )
    return pivot
...
def _rolling_beta(
    strat: pd.Series, bench: pd.Series, window: int = 252, min_periods: int = 60
) -> pd.Series:
...
# Loaders
# -------------------------
def load_returns_auto(
    paths: dict[str, Path], tag: str, returns_path: Optional[Path]
) -> pd.DataFrame:
...
def load_holdings_auto(
    paths: dict[str, Path], tag: str, holdings_path: Optional[Path]
    ...
    if picked is None:
        print(
            "[WARN] holdings 파일을 찾지 못했습니다. 섹터/턴오버 관련 산출물을 건너뜁니다."
        )
        return None
...
def load_universe_auto(
    paths: dict[str, Path], universe_path: Optional[Path]
) -> Optional[pd.DataFrame]:
    ...
    if picked is None:
        print(
            "[WARN] universe(섹터맵) 파일을 찾지 못했습니다. 섹터 비중 산출을 건너뜁니다."
        )
        return None
...
def make_equity_and_tables(
    ...
    # pick columns
    net_col = strat_net_col or _pick_col(
        dfi, ["net_return", "strategy_net_return", "ret_net", "pnl_net"]
    )
    bmk_col = bench_col or _pick_col(
        dfi, ["bench_return", "benchmark_return", "bm_return", "index_return"]
    )
    gross_col = strat_gross_col
    if gross_col is None:
        gross_col = _pick_col(
            dfi,
            ["gross_return", "strategy_gross_return", "ret_gross", "pnl_gross"],
            required=False,
        )
    ...
    print(
        f"[INFO] return scale: net={net_scale}, bench={bench_scale}, gross={gross_scale}"
    )
    ...
    # rolling beta
    beta = _rolling_beta(
        df["strategy_net"], df["benchmark"], window=beta_window, min_periods=beta_minp
    )
...
def make_sector_and_turnover(
    ...
    wcol = weight_col or _pick_col(
        h, ["weight", "w", "port_weight", "allocation", "value_weight"]
    )
    ...
    if scol is None:
        scol = _pick_col(
            u,
            ["sector", "industry", "sector_name", "industry_name", "gics_sector"],
            required=False,
        )
    if scol is None:
        raise ValueError(
            "universe에서 섹터 컬럼을 찾지 못했습니다. --sector-col 로 지정해 주세요."
        )
    ...
    if hh["sector_u"].isna().mean() > 0.3:
        print(
            "[WARN] 섹터 매칭이 많이 비었습니다. ticker 포맷(6자리) / universe 매핑을 확인하세요."
        )

    # sector weights time series
    sector_ts = (
        hh.groupby(["date", "sector_u"])["weight"].sum().unstack("sector_u").fillna(0.0)
    )
    ...
    w_pivot = hh.pivot_table(
        index="date", columns="ticker_u", values="weight", aggfunc="sum"
    ).fillna(0.0)
    w_diff = w_pivot.diff().abs().sum(axis=1) * 0.5
    w_diff = w_diff.dropna()
    w_diff.rename("turnover_oneway").to_csv(
        outdir / "turnover_series.csv", encoding="utf-8-sig"
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="프로젝트 루트. 미지정 시 현재 작업 디렉토리",
    )
    parser.add_argument(
        "--config", type=str, default=None, help="configs/config.yaml 경로(선택)"
    )
    parser.add_argument(
        "--tag", type=str, required=True, help="실행 tag (예: today_L7C_fix01)"
    )

    # input paths override
    parser.add_argument(
        "--returns-path", type=str, default=None, help="returns 파일 경로(parquet/csv)"
    )
    parser.add_argument(
        "--holdings-path",
        type=str,
        default=None,
        help="holdings 파일 경로(parquet/csv)",
    )
    parser.add_argument(
        "--universe-path",
        type=str,
        default=None,
        help="universe(섹터맵) 파일 경로(parquet/csv)",
    )
    parser.add_argument(
        "--factor-returns-path",
        type=str,
        default=None,
        help="팩터 수익률 파일 경로(parquet/csv)",
    )

    # column overrides
    parser.add_argument(
        "--date-col", type=str, default=None, help="returns 날짜 컬럼명(선택)"
    )
    parser.add_argument(
        "--strat-net-col", type=str, default=None, help="전략 net return 컬럼명(선택)"
    )
    parser.add_argument(
        "--strat-gross-col",
        type=str,
        default=None,
        help="전략 gross return 컬럼명(선택)",
    )
    parser.add_argument(
        "--bench-col", type=str, default=None, help="벤치마크 return 컬럼명(선택)"
    )

    parser.add_argument(
        "--hold-date-col", type=str, default=None, help="holdings 날짜 컬럼명(선택)"
    )
    parser.add_argument(
        "--hold-ticker-col", type=str, default=None, help="holdings ticker 컬럼명(선택)"
    )
    parser.add_argument(
        "--hold-weight-col", type=str, default=None, help="holdings weight 컬럼명(선택)"
    )
    parser.add_argument(
        "--univ-sector-col", type=str, default=None, help="universe 섹터 컬럼명(선택)"
    )

    # stats params
    parser.add_argument(
        "--freq", type=int, default=252, help="연환산 기준 빈도(일봉=252)"
    )
    parser.add_argument(
        "--beta-window", type=int, default=252, help="rolling beta window"
    )
    parser.add_argument(
        "--beta-minp", type=int, default=60, help="rolling beta min periods"
    )
    ...
    df_univ = load_universe_auto(
        paths, Path(args.universe_path).resolve() if args.universe_path else None
    )
