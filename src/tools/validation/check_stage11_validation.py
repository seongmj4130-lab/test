...
def check_ui_top_bottom_daily(
    run_tag: str, expected_top_k: int = 10, expected_bottom_k: int = 10
):
    ...
    print(
        f"Bottom 리스트 길이 범위: {df['bottom_count'].min()} ~ {df['bottom_count'].max()}"
    )
    ...
    df["top_set"] = (
        df["top_list"]
        .str.split(",")
        .apply(lambda x: set([t.strip() for t in x if t.strip()]))
    )
    df["bottom_set"] = (
        df["bottom_list"]
        .str.split(",")
    ...
    df["overlap"] = df.apply(
        lambda row: len(row["top_set"] & row["bottom_set"]), axis=1
    )
    ...
    if top_mismatch > 0:
        print(
            f"[FAIL] Top 리스트 길이 불일치: {top_mismatch}개 (기대: {expected_top_k})"
        )
        pass_check = False
    if bottom_mismatch > 0:
        print(
            f"[FAIL] Bottom 리스트 길이 불일치: {bottom_mismatch}개 (기대: {expected_bottom_k})"
        )
        pass_check = False
    if overlap_count > 0:
        print(f"[FAIL] 동일 ticker가 top/bottom 동시 등장: {overlap_count}개")
        print("\n겹치는 날짜 샘플:")
        overlap_df = df[df["overlap"] > 0][
            ["date", "top_list", "bottom_list", "overlap"]
        ].head(10)
    ...
    print("\n처음 3행:")
    print(
        df[["date", "top_count", "bottom_count", "top_list", "bottom_list"]]
        .head(3)
        .to_string()
    )
...
def check_ui_snapshot(run_tag: str):
    ...
    if has_regime_label:
        print("\nTop10 샘플:")
        top_df = df[df["snapshot_type"] == "top"][
            [
                "snapshot_date",
                "snapshot_type",
                "snapshot_rank",
...
def main():
    if len(sys.argv) < 2:
        ...
        print(
            "Example: python check_stage11_validation.py stage11_ui_payload_20251221_012244 stage10_market_regime_20251221_004433"
        )
        sys.exit(1)
    ...
    results.append(
        (
            "ui_top_bottom_daily",
            check_ui_top_bottom_daily(run_tag, expected_top_k=10, expected_bottom_k=10),
        )
    )
