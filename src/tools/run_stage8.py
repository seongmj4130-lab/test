...
def get_ranking_baseline_tag(base_dir: Path) -> str:
    ...
    if not baselines_path.exists():
        print(f"ERROR: baselines.yaml이 존재하지 않습니다: {baselines_path}", file=sys.stderr)
        print("Stage7을 먼저 완료하여 ranking_baseline_tag를 설정하세요.", file=sys.stderr)
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(description=f"[코드 매니저] Stage {STAGE_NUM} 실행 (Sector-relative 정규화)")
    ...
    parser.add_argument("--baseline-tag", type=str, default=None, help="Baseline tag (없으면 baselines.yaml에서 읽음)")
    parser.add_argument("--change-title", type=str, default="Sector-relative 정규화 적용", help="Change title (History Manifest용)")
    parser.add_argument("--change-summary", type=str, nargs="*", default=[
        "섹터 내 정규화로 업종 편향 완화",
        "Top20 섹터 농도지표(HHI/MaxShare) 산출",
        "랭킹 품질 KPI 추가"
    ], help="Change summary (최대 3개)")
    parser.add_argument("--modified-files", type=str, default="src/tools/run_stage8.py;src/stages/l8_rank_engine.py;src/ranking/score_engine.py", help="Modified files")
    parser.add_argument("--modified-functions", type=str, default="run_stage8;build_ranking_daily;apply_sector_relative_norm;compute_sector_concentration", help="Modified functions")
    ...
    sector_concentration_path = PROJECT_ROOT / "reports" / "ranking" / f"sector_concentration__{run_tag}.csv"
    if not sector_concentration_path.exists():
        print(f"ERROR: 섹터 농도 리포트가 생성되지 않았습니다: {sector_concentration_path}", file=sys.stderr)
        sys.exit(1)
    ...
    if run_command(delta_cmd, PROJECT_ROOT, "Δ 리포트 생성 (Stage7 vs Stage8)", log_file) != 0:
