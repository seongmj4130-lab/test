...
def main():
    parser = argparse.ArgumentParser(
        description=f"[코드 매니저] Stage {STAGE_NUM} 실행 (UI Payload Builder)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Baseline tag (Stage10 run_tag, 없으면 자동 감지)",
    )
    parser.add_argument(
        "--change-title",
        type=str,
        default="UI payload + 단순 성과곡선 생성",
    ...
    parser.add_argument(
        "--modified-files",
        type=str,
        default="src/stages/ui_payload_builder.py;src/stages/ranking_demo_performance.py;src/tools/run_stage11.py",
        help="Modified files",
    )
    parser.add_argument(
        "--modified-functions",
        type=str,
        default="build_ui_payload;build_demo_equity_curves",
        ...
        if stage10_dirs:
            ...
            if not baseline_tag_used:
                ...
                ranking_candidates = list(
                    (PROJECT_ROOT / "data" / "interim").glob("*/ranking_daily.parquet")
                )
                if ranking_candidates:
                    ...
                    print(
                        f"[INFO] 최신 ranking_daily가 있는 run_tag 자동 감지: {baseline_tag_used}"
                    )
                else:
                    print(
                        "ERROR: Stage10 run_tag를 찾을 수 없고 ranking_baseline_tag도 설정되지 않았습니다. --baseline-tag로 지정하세요.",
                        file=sys.stderr,
                    )
                    sys.exit(1)
            else:
    ...
    stage_cmd = [
        ...
        "--skip-l2",  # L2 재사용 강제
        "--baseline-tag",
        baseline_tag_used,  # ranking_daily 로드용
    ]

    if (
        run_command(stage_cmd, PROJECT_ROOT, f"Stage {STAGE_NUM} 실행 (L11)", log_file)
        != 0
    ):
        sys.exit(1)
    ...
    # 산출물 확인
    ui_top_bottom_path = (
        PROJECT_ROOT / "data" / "interim" / run_tag / "ui_top_bottom_daily.parquet"
    )
    ui_equity_path = (
        PROJECT_ROOT / "data" / "interim" / run_tag / "ui_equity_curves.parquet"
    )
    ui_snapshot_path_interim = (
        PROJECT_ROOT / "data" / "interim" / run_tag / "ui_snapshot.parquet"
    )
    ui_metrics_path_interim = (
        PROJECT_ROOT / "data" / "interim" / run_tag / "ui_metrics.parquet"
    )

    if not ui_top_bottom_path.exists():
        print(
            f"ERROR: ui_top_bottom_daily.parquet가 생성되지 않았습니다: {ui_top_bottom_path}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not ui_equity_path.exists():
        print(
            f"ERROR: ui_equity_curves.parquet가 생성되지 않았습니다: {ui_equity_path}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if ui_snapshot_path_interim.exists():
        print(
            f"[OK] ui_snapshot.parquet 생성 완료: {len(pd.read_parquet(ui_snapshot_path_interim)):,} rows"
        )
    if ui_metrics_path_interim.exists():
        print(
            f"[OK] ui_metrics.parquet 생성 완료: {len(pd.read_parquet(ui_metrics_path_interim)):,} rows"
        )
    ...
    if ui_snapshot_path_interim.exists():
        ...
        print(
            f"[OK] ui_snapshot.csv 생성 완료: {ui_snapshot_path} ({len(ui_snapshot)} rows)"
        )
    else:
        print("WARNING: ui_snapshot.parquet가 없습니다. 건너뜁니다.", file=sys.stderr)
    ...
    if ui_metrics_path_interim.exists():
        ...
        print(
            f"[OK] ui_metrics.csv 생성 완료: {ui_metrics_path} ({len(ui_metrics)} rows)"
        )
    else:
        print("WARNING: ui_metrics.parquet가 없습니다. 건너뜁니다.", file=sys.stderr)
