...
def main():
    parser = argparse.ArgumentParser(
        description=f"[코드 매니저] Stage {STAGE_NUM} 실행 (시장 국면 지표 추가)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Baseline tag (Stage9 run_tag, 없으면 자동 감지)",
    )
    parser.add_argument(
        "--change-title",
        type=str,
    ...
    parser.add_argument(
        "--modified-files",
        type=str,
        default="src/stages/market_regime.py;src/tools/run_stage10.py",
        help="Modified files",
    )
    parser.add_argument(
        "--modified-functions",
        type=str,
        default="build_market_regime;merge_regime_to_ranking",
    ...
    if args.baseline_tag:
        ...
        stage9_dirs = glob.glob(
            str(PROJECT_ROOT / "data" / "interim" / "stage9_ranking_explainability_*")
        )
        if stage9_dirs:
            ...
            print(
                "ERROR: Stage9 run_tag를 찾을 수 없습니다. --baseline-tag로 지정하세요.",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    lookback_days = (
        l8.get("regime", {}).get("lookback_days", 60) if l8.get("regime") else 60
    )
    ...
    for path in candidate_paths:
        if path.exists():
            try:
                ...
                print(
                    f"[OK] OHLCV 데이터 로드 (fallback용): {path} ({len(ohlcv_daily):,} rows)"
                )
                break
            except Exception as e:
        ...
        if path.exists():
            try:
                ...
                if "in_universe" in universe_daily.columns:
                    print(
                        f"[OK] Universe 데이터 로드 (fallback용): {path} ({len(universe_daily):,} rows)"
                    )
                    break
            except Exception as e:
                print(f"WARNING: {path} 로드 실패: {e}", file=sys.stderr)
    ...
    try:
        ranking_with_regime = merge_regime_to_ranking(
            baseline_ranking, market_regime_daily
        )
        print(
            f"[OK] ranking_daily에 regime 조인 완료: {len(ranking_with_regime):,} rows"
        )
        print(
            f"  추가된 컬럼: {[c for c in ranking_with_regime.columns if 'regime' in c.lower()]}"
        )
    except Exception as e:
    ...
    try:
        # 연도별 bull/bear 비중 계산
        market_regime_daily["year"] = pd.to_datetime(
            market_regime_daily["date"]
        ).dt.year
        summary_rows = []
            ...
            summary_rows.append(
                {
                    ...
                    "bull_pct": (
                        bull_count / total_count * 100 if total_count > 0 else 0
                    ),
                    "bear_pct": (
                        bear_count / total_count * 100 if total_count > 0 else 0
                    ),
                    "neutral_pct": (
    ...
    if (
        run_command(
            delta_cmd, PROJECT_ROOT, "Δ 리포트 생성 (Stage9 vs Stage10)", log_file
        )
        != 0
    ):
    ...
    print("\n[완료 기준 검증]")
    checks = {
        "market_regime_daily.parquet 존재": regime_output_path.exists()
        and len(market_regime_daily) > 0,
        "ranking_daily.parquet 존재": ranking_output_path.exists()
        and len(ranking_with_regime) > 0,
