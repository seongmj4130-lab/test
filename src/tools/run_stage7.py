...
def update_baselines_yaml(
    base_dir: Path, pipeline_baseline: str, ranking_baseline: str
) -> None:
    ...
    with open(baselines_path, "w", encoding="utf-8") as f:
        yaml.dump(
            baselines, f, default_flow_style=False, allow_unicode=True, sort_keys=False
        )
...
def main():
    parser = argparse.ArgumentParser(
        description=f"[코드 매니저] Stage {STAGE_NUM} 실행 (랭킹 baseline 생성)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--modified-files",
        type=str,
        default="src/tools/run_stage7.py;src/tools/update_history_manifest.py",
        help="Modified files",
    )
    parser.add_argument(
        "--modified-functions",
        type=str,
        default="run_stage7;build_ranking_daily;update_history_manifest",
    ...
    # Baseline 태그 결정
    baseline_tag_used = get_baseline_tag(
        config_path, STAGE_NUM
    )  # pipeline_baseline_tag
    ...
    snapshot_result = snapshot_result[
        ["snapshot_date", "snapshot_type", "ticker", "score_total", "rank_total"]
    ].copy()
    ...
    output_files = [
        ...
        (
            "Snapshot",
            PROJECT_ROOT / "reports" / "ranking" / f"ranking_snapshot__{run_tag}.csv",
        ),
