...
def main():
    parser = argparse.ArgumentParser(
        description=f"[코드 매니저] Stage {STAGE_NUM} 실행 (Final Export Pack)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Baseline tag (Stage11 run_tag, 없으면 자동 감지)",
    )
    parser.add_argument(
        "--change-title",
        type=str,
        default="최종 발표용 export 패키지",
    ...
    parser.add_argument(
        "--modified-files",
        type=str,
        default="src/stages/final_export_pack.py;src/tools/run_stage12.py",
        help="Modified files",
    )
    parser.add_argument(
        "--modified-functions",
        type=str,
        default="build_final_export_pack",
    ...
    if args.baseline_tag:
        ...
        if stage11_dirs:
            ...
            print(
                "ERROR: Stage11 run_tag를 찾을 수 없습니다. --baseline-tag로 지정하세요.",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    try:
        ...
        print(
            f"[OK] timeline_ppt.csv 생성 완료: {timeline_path} ({len(outputs['timeline_ppt'])} rows)"
        )
        ...
        print(
            f"[OK] kpi_onepager.csv 생성 완료: {kpi_csv_path} ({len(outputs['kpi_onepager'])} rows)"
        )
        ...
        outputs["latest_snapshot"].to_csv(
            snapshot_path, index=False, encoding="utf-8-sig"
        )
        print(
            f"[OK] latest_snapshot.csv 생성 완료: {snapshot_path} ({len(outputs['latest_snapshot'])} rows)"
        )
        ...
        print(
            f"[OK] equity_curves.csv 생성 완료: {equity_path} ({len(outputs['equity_curves'])} rows)"
        )
