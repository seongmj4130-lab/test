...
def main():
    parser = argparse.ArgumentParser(description=f"[코드 매니저] Stage {STAGE_NUM} 실행 (랭킹 설명가능성 확장)")
    ...
    parser.add_argument("--baseline-tag", type=str, required=True, help="Baseline tag (Stage8 run_tag)")
    parser.add_argument("--change-title", type=str, default="랭킹 설명가능성 확장", help="Change title (History Manifest용)")
    parser.add_argument("--change-summary", type=str, nargs="*", default=[
        "기여도/상위 기여 피처 저장",
        "Top/Bottom 설명 UI 데이터 제공",
        "ranking_daily 컬럼 확장"
    ], help="Change summary (최대 3개)")
    parser.add_argument("--modified-files", type=str, default="src/stages/ranking_explainability.py;src/tools/run_stage9.py", help="Modified files")
    parser.add_argument("--modified-functions", type=str, default="build_ranking_with_contrib;make_snapshot", help="Modified functions")
    ...
    if args.baseline_tag:
        ...
        import glob
        stage8_dirs = glob.glob(str(PROJECT_ROOT / "data" / "interim" / "stage8_sector_relative_*"))
        if stage8_dirs:
            ...
            print("ERROR: Stage8 run_tag를 찾을 수 없습니다. --baseline-tag로 지정하세요.", file=sys.stderr)
            sys.exit(1)
    ...
    # Python 스크립트로 직접 실행
    stage9_script = PROJECT_ROOT / "src" / "stages" / "run_stage9_ranking_explainability.py"
    ...
    import pandas as pd
    from src.stages.ranking.ranking_explainability import build_ranking_with_explainability, make_snapshot_with_explainability
    ...
    try:
        ...
        print(f"[OK] 설명가능성 컬럼 추가 완료: {len(ranking_with_explainability):,} rows")
        print(f"  추가된 컬럼: {[c for c in ranking_with_explainability.columns if c.startswith('contrib_') or c == 'top_features']}")
    except Exception as e:
    ...
    try:
        snapshot = make_snapshot_with_explainability(ranking_with_explainability, top_k=10)
    ...
    if run_command(delta_cmd, PROJECT_ROOT, "Δ 리포트 생성 (Stage8 vs Stage9)", log_file) != 0:
    ...
    print("\n[완료 기준 검증]")
    checks = {
        "ranking_daily.parquet 존재": ranking_output_path.exists() and len(ranking_with_explainability) > 0,
        "contrib_* 컬럼 존재": any(c.startswith("contrib_") for c in ranking_with_explainability.columns),
