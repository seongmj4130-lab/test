...
def _to_kv(
    ...
    *,
    ...
    if df is None or df.empty:
        return pd.DataFrame(
            columns=["section", "entity", "metric", "value", "value_num"]
        )
...
def _pick_artifact_df(
    combined: pd.DataFrame, artifact_name: str, cols: Optional[list[str]] = None
) -> pd.DataFrame:
...
def _top_bottom_spread_one(
    g: pd.DataFrame, y_true: str, y_pred: str, q: float = 0.2
) -> float:
    x = g[[y_true, y_pred]].dropna()
    if len(x) < 10:
        return np.nan
    ...
    top = pd.to_numeric(
        x.loc[pd.to_numeric(x[y_pred], errors="coerce") >= hi, y_true], errors="coerce"
    ).mean()
    bot = pd.to_numeric(
        x.loc[pd.to_numeric(x[y_pred], errors="coerce") <= lo, y_true], errors="coerce"
    ).mean()
...
def _ic_spread_summary(pred: pd.DataFrame, name: str) -> pd.DataFrame:
    ...
    for (ph, dt), g in df.groupby(["phase", "date"], sort=True):
        ...
        rows.append(
            {"artifact": name, "phase": ph, "date": dt, "ic_rank": ic, "spread_q20": sp}
        )
    ...
    for ph, g in daily.groupby("phase", sort=False):
        ...
        ic_std = (
            float(np.nanstd(ic.to_numpy(), ddof=1))
            if (ic.notna().sum() >= 2)
            else np.nan
        )
...
def main():
    ...
    cfg_map = [
        ...
        (
            "params.rolling_train_years_short",
            ["params.rolling_train_years_short", "l4.rolling_train_years_short"],
        ),
        (
            "params.rolling_train_years_long",
            ["params.rolling_train_years_long", "l4.rolling_train_years_long"],
        ),
        ...
        (
            "params.l7.holding_days",
            ["params.l7.holding_days", "params.l6.l7.holding_days", "l7.holding_days"],
        ),
        ("params.l7.top_k", ["params.l7.top_k", "params.l6.l7.top_k", "l7.top_k"]),
        (
            "params.l7.cost_bps",
            ["params.l7.cost_bps", "params.l6.l7.cost_bps", "l7.cost_bps"],
        ),
        (
            "params.l7.weighting",
            ["params.l7.weighting", "params.l6.l7.weighting", "l7.weighting"],
        ),
        (
            "params.l7.softmax_temp",
            ["params.l7.softmax_temp", "params.l6.l7.softmax_temp", "l7.softmax_temp"],
        ),
        (
            "params.l7.score_col",
            ["params.l7.score_col", "params.l6.l7.score_col", "l7.score_col"],
        ),
        (
            "params.l7.return_col",
            ["params.l7.return_col", "params.l6.l7.return_col", "l7.return_col"],
        ),
    ]

    cfg_rows = [
        {"key": k, "value": _first_non_none(cfg, paths, None)} for k, paths in cfg_map
    ]
    ...
    # Universe monthly
    univ = _pick_artifact_df(
        combined, "universe_k200_membership_monthly", cols=["date", "ticker"]
    )
            ...
            [
                {
                    ...
                    "tickers_mean": (
                        float(univ_monthly["universe_n_tickers"].mean())
                        if len(univ_monthly)
                        else np.nan
                    ),
                    "tickers_min": (
                        int(univ_monthly["universe_n_tickers"].min())
                        if len(univ_monthly)
                        else np.nan
                    ),
                    "tickers_max": (
                        int(univ_monthly["universe_n_tickers"].max())
                        if len(univ_monthly)
                        else np.nan
                    ),
                }
    ...
    def cv_sum(name: str) -> pd.DataFrame:
        ...
        segs = (
            ",".join(sorted(d["segment"].astype(str).dropna().unique().tolist()))
            ...
            [
                {
                    ...
                    "segments": segs,
                    "train_start_min": (
                        d["train_start"].min() if "train_start" in d.columns else None
                    ),
                    "test_end_max": (
                        d["test_end"].max() if "test_end" in d.columns else None
                    ),
                }
            ]
        )
    ...
    cv_summary = pd.concat(
        [cv_sum("cv_folds_short"), cv_sum("cv_folds_long")], ignore_index=True
    )
    ...
    pred_s = _pick_artifact_df(
        combined, "pred_short_oos", cols=["date", "phase", "y_true", "y_pred"]
    )
    pred_l = _pick_artifact_df(
        combined, "pred_long_oos", cols=["date", "phase", "y_true", "y_pred"]
    )
    ic_sum = pd.concat(
        [
            _ic_spread_summary(pred_s, "pred_short_oos"),
            _ic_spread_summary(pred_l, "pred_long_oos"),
        ],
        ignore_index=True,
    )
    ...
    bench_metrics = _pick_artifact_df(
        combined, "bt_benchmark_compare"
    )  # 네 snapshot에 이름이 이거로 보임
    ...
    if not roll_sharpe.empty:
        num_cols = [
            ...
            if c in roll_sharpe.columns
        ]
        if num_cols:
            desc = (
                roll_sharpe[num_cols]
                .apply(pd.to_numeric, errors="coerce")
                .describe()
                .T[["mean", "min", "max"]]
            )
            roll_stats = desc.reset_index().rename(
                columns={"index": "metric"}
            )  # ✅ 'metric' 있어도 _to_kv에서 충돌 회피
    ...
    parts.append(
        _to_kv(univ_monthly, section="universe_monthly_counts", entity_cols=["ym"])
    )
    parts.append(_to_kv(cv_summary, section="cv_summary", entity_cols=["artifact"]))
    parts.append(
        _to_kv(
            mm_sum,
            section="model_metrics_summary",
            entity_cols=[
                c for c in ["horizon", "phase", "model"] if c in mm_sum.columns
            ],
        )
    )
    parts.append(
        _to_kv(
            ic_sum,
            section="ic_spread_summary",
            entity_cols=[c for c in ["artifact", "phase"] if c in ic_sum.columns],
        )
    )
    parts.append(
        _to_kv(
            bt_metrics,
            section="bt_metrics",
            entity_cols=[c for c in ["phase"] if c in bt_metrics.columns],
        )
    )
    parts.append(
        _to_kv(
            bench_metrics,
            section="benchmark_metrics",
            entity_cols=[c for c in ["phase"] if c in bench_metrics.columns],
        )
    )
    parts.append(
        _to_kv(
            roll_stats,
            section="rolling_sharpe_stats",
            entity_cols=["metric"] if "metric" in roll_stats.columns else None,
        )
    )
    parts.append(
        _to_kv(
            dd_top10,
            section="drawdown_top10",
            entity_cols=[
                c
                for c in ["phase", "peak_date", "trough_date"]
                if c in dd_top10.columns
            ],
        )
    )
    parts.append(
        _to_kv(
            ...
            entity_cols=[c for c in ["phase", "year"] if c in yearly.columns],
        )
    )

    all_kv = pd.concat(
        [p for p in parts if p is not None and not p.empty], ignore_index=True
    )
    all_kv = all_kv.sort_values(
        ["section", "entity", "metric"], kind="mergesort"
    ).reset_index(drop=True)
