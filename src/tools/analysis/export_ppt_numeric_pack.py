# C:/Users/seong/OneDrive/Desktop/bootcamp/03_code/src/tools/analysis/export_ppt_numeric_pack.py
"""
...
def find_artifact(
    project_root: Path, extract_dir: Path, tag: str, name: str
) -> FoundArtifact:
    """
    아티팩트 탐색 우선순위:
    ...
    if p1.exists():
        return FoundArtifact(
            path=p1, source_kind="extract", note=str(p1.relative_to(project_root))
        )
    ...
    for ext in (".parquet", ".csv"):
        ...
        if p2.exists():
            return FoundArtifact(
                path=p2, source_kind="tag", note=str(p2.relative_to(project_root))
            )
    ...
    for ext in (".parquet", ".csv"):
        ...
        if p3.exists():
            return FoundArtifact(
                path=p3, source_kind="root", note=str(p3.relative_to(project_root))
            )
...
def inventory_sources(
    project_root: Path, extract_dir: Path, ppt_dir: Path
    ...
    for rel in [
        ...
        p = extract_dir / rel
        sources.append(
            {"source_file": str(p.relative_to(project_root)), "exists": p.exists()}
        )
    ...
    if charts_manifest.exists():
        ...
        for _, r in cm.iterrows():
            ...
            sources.append(
                {"source_file": exported if exported else src_path, "exists": True}
            )
    ...
    def add(
        cat: str, item: str, source_file: str, source_cols: str, derive: str
    ) -> None:
        items.append(
            {
                "category": cat,
    ...
    add(
        "executive_kpi",
        "holdout_kpi_core",
        "reports/extract/kpi_snapshot.csv",
        "net_total_return,net_cagr,net_sharpe,net_mdd,cost_bps_used,n_rebalances",
        "direct (holdout columns in md table parsed)",
    )
    add(
        "executive_kpi",
        "avg_turnover_oneway/avg_total_cost",
        "data/interim/{tag}/bt_metrics.parquet or bt_returns.csv",
        "avg_turnover_oneway + total_cost",
        "bt_metrics holdout preferred; else mean(bt_returns.total_cost) / mean(bt_returns.turnover_oneway)",
    )
    add(
        "executive_kpi",
        "bench_total_return/excess_total_return",
        "bt_benchmark_returns + bt_vs_benchmark",
        "bench_return/excess_return",
        "cumprod(1+r)-1 by phase",
    )

    add(
        "data_snapshot",
        "dataset_daily_shape",
        "data/interim/{tag}/dataset_daily.parquet or data/interim/dataset_daily.parquet",
        "rows,cols",
        "direct",
    )
    add(
        "data_snapshot",
        "unique_tickers/dates/dup",
        "dataset_daily.parquet",
        "ticker,date",
        "nunique + duplicated(ticker,date)",
    )
    add(
        "data_snapshot",
        "feature_stats",
        "dataset_daily.parquet + reports/extract/features_final.json",
        "features_final",
        "NaN%, min/p01/median/p99/max",
    )

    add(
        "model_validation",
        "model_metrics",
        "data/interim/{tag}/model_metrics.parquet or data/interim/model_metrics.parquet",
        "rmse/mae/ic_rank/hit_ratio/...",
        "direct export if exists",
    )
    add(
        "model_validation",
        "pred_based_metrics",
        "pred_short_oos/pred_long_oos",
        "y_true/y_pred",
        "recompute per phase (ic_rank, corr, rmse, hit_ratio)",
    )

    add(
        "ranking_quality",
        "top_bottom_spread",
        "rebalance_scores or ranking_daily",
        "score + return",
        "top_k mean - bottom_k mean by date",
    )
    add(
        "ranking_quality",
        "daily_ic",
        "rebalance_scores or ranking_daily",
        "score + return",
        "Spearman corr(score, return) by date",
    )
    ...
    add(
        ...
        "bt_returns.csv",
        "net_return",
        "p10/p50/p90/min/max by phase",
    )
    add(
        "backtest_performance",
        "monthly_returns",
        "bt_returns + bt_benchmark_returns/bt_vs_benchmark",
        "net_return/bench_return/excess_return",
        "monthly compounded within month",
    )

    add(
        "risk_drawdown",
        "drawdown_key_events",
        "bt_drawdown_events.csv",
        "peak_date,trough_date,drawdown,length_days",
        "top 3 most negative drawdowns",
    )
    add(
        "stability",
        "yearly_metrics",
        "bt_yearly_metrics.csv",
        "year,net_total_return,net_sharpe,net_mdd,...",
        "direct export",
    )
    add(
        "stability",
        "rolling_sharpe_summary",
        "data/interim/bt_rolling_sharpe.parquet",
        "rolling_sharpe",
        "min/max/mean + min date by phase",
    )
    ...
    add(
        "sensitivity",
        "top10pct_sharpe_ranges",
        "bt_sensitivity.csv",
        "net_sharpe + grid",
        "holdout, threshold=90th percentile; min/max of grid params",
    )

    add(
        "benchmark_fairness",
        "benchmark_type",
        "reports/extract/backtest_contract.md",
        "L7C build_universe_benchmark_returns",
        "code contract: mean(universe returns) benchmark",
    )
    add(
        "benchmark_fairness",
        "benchmark_perf",
        "bt_benchmark_returns.csv",
        "bench_return/bench_equity",
        "CAGR/Sharpe/MDD computed from series",
    )
    add(
        "benchmark_fairness",
        "config_mismatch_table",
        "reports/extract/integrity_report.md + config_snapshot.md",
        "cost_bps_used vs config",
        "direct parse",
    )
    ...
    add(
        "repro_runtime",
        "cli_options",
        "reports/extract/cli_run_all.md",
        "argparse table",
        "direct",
    )
    add(
        "repro_runtime",
        "commands_log",
        "reports/extract/repro_commands.md",
        "Command lines",
        "direct",
    )
...
def compute_bench_metrics(
    bench_returns: pd.DataFrame, holding_days: Optional[float]
    ...
    if bench_returns is None:
        return out
    if (
        "bench_return" not in bench_returns.columns
        or "bench_equity" not in bench_returns.columns
    ):
        return out
    try:
    ...
    if (
        ...
        and holding_days > 0
        and "bench_return" in bench_returns.columns
    ):
        try:
            periods_per_year = 252.0 / float(holding_days)
            x = bench_returns["bench_return"].astype(float).to_numpy()
            if len(x) > 1:
                out["bench_sharpe"] = float(
                    (np.mean(x) / (np.std(x, ddof=1) + 1e-12))
...
def build_executive_kpi(
    ...
    unit_md_lines.append(
        f"- source_kpi_snapshot: `{str(kpi_path.relative_to(project_root)) if kpi_path.exists() else UNKNOWN}`"
    )
    unit_md_lines.append("")
    ...
    for tag in [run_tag, baseline_tag]:
        ...
        if bt_vs is not None and "phase" in bt_vs.columns:
            ...
            if len(hv) > 0 and "excess_return" in hv.columns:
                row["excess_total_return"] = _safe_num_or_unknown(
                    compute_total_return_from_series(hv["excess_return"])
                )
            if holding_days is None and len(hv) > 0 and "holding_days" in hv.columns:
                holding_days = _parse_float(hv["holding_days"].iloc[0])

        bench_art = find_artifact(
            project_root, extract_dir, tag, "bt_benchmark_returns"
        )
        ...
        )  # may be percent-scale or decimal
        derived_nt = None
        ...
        if kpi_nt is None or derived_nt is None:
            ...
            unit_md_lines.append(
                f"- unit_check_detail: `net_total_return kpi={kpi_nt}, derived={derived_nt}, ratio={kpi_nt/derived_nt if abs(derived_nt)>1e-12 else 'inf'}`"
            )
        unit_md_lines.append("")
    ...
    _write_text(
        ppt_dir / "ppt_kpi_units_check.md", "\n".join(unit_md_lines).rstrip() + "\n"
    )
...
def build_data_snapshot(
    ...
    if p_tag.exists():
        ds_art = FoundArtifact(
            path=p_tag, source_kind="tag", note=str(p_tag.relative_to(project_root))
        )
    else:
        ...
        if p_root.exists():
            ds_art = FoundArtifact(
                path=p_root,
                source_kind="root",
                note=str(p_root.relative_to(project_root)),
            )
    ...
    if ds_df is not None:
        ...
        if "date" in ds_df.columns:
            snap["unique_dates"] = int(
                pd.to_datetime(ds_df["date"], errors="coerce").nunique()
            )
        if "ticker" in ds_df.columns and "date" in ds_df.columns:
            tmp = ds_df[["ticker", "date"]].copy()
            tmp["ticker"] = tmp["ticker"].astype(str)
            tmp["date"] = pd.to_datetime(tmp["date"], errors="coerce")
            snap["ticker_date_dup_count"] = int(
                tmp.duplicated(subset=["ticker", "date"]).sum()
            )
...
def build_model_validation(
    ...
    def compute_from_preds(
        df: pd.DataFrame, *, tag: str, horizon_label: str
    ) -> list[dict[str, Any]]:
        rows = []
        if df is None:
            return rows
        ...
        cols = set(df.columns)
        y_true_col = (
            "y_true" if "y_true" in cols else ("true" if "true" in cols else None)
        )
        y_pred_col = (
            "y_pred" if "y_pred" in cols else ("pred" if "pred" in cols else None)
        )
        ...
        for ph, g in d.groupby(phase_col, sort=False):
            ...
            corr = (
                float(np.corrcoef(yt.to_numpy(), yp.to_numpy())[0, 1])
                if len(yt) > 1
                else np.nan
            )
            # spearman via rank corr
            ic_rank = float(
                pd.Series(yt).rank(pct=True).corr(pd.Series(yp).rank(pct=True))
            )
            ...
            rows.append(
                {
                    ...
                    "ic_rank": (
                        ic_rank
                        if not (isinstance(ic_rank, float) and math.isnan(ic_rank))
                        else np.nan
                    ),
                    "hit_ratio": hit,
    ...
    for tag in [run_tag, baseline_tag]:
        ...
        pred_s, src_s = load_df(
            find_artifact(project_root, extract_dir, tag, "pred_short_oos")
        )
        pred_l, src_l = load_df(
            find_artifact(project_root, extract_dir, tag, "pred_long_oos")
        )
    ...
    if df_out.empty:
        _write_csv(
            ppt_dir / "ppt_model_validation.csv",
            [{"tag": UNKNOWN, "source": UNKNOWN}],
            fieldnames=["tag", "source"],
        )
        return
...
def build_ranking_quality(
    ...
    for tag in [run_tag, baseline_tag]:
            ...
            out_rows.append(
                {
                    ...
                    "date": "",
                    "phase": "",
                    "top_k_mean_ret": UNKNOWN,
                    "bottom_k_mean_ret": UNKNOWN,
                    "spread": UNKNOWN,
                    "ic_spearman": UNKNOWN,
                    "source": UNKNOWN,
                }
        ...
        score_col = (
            "score_ens"
            if "score_ens" in cols
            else ("score" if "score" in cols else None)
        )
        ret_col = (
            "true_short"
            if "true_short" in cols
            else (
                "ret_fwd_20d"
                if "ret_fwd_20d" in cols
                else ("y_true" if "y_true" in cols else None)
            )
        )
        if (
            date_col is None
            or phase_col is None
        ...
        if k is None:
            md_lines.append(
                f"- {tag}: top_k를 근거로 확정할 수 없어 계산 스킵 ({src_used})"
            )
            out_rows.append(
                {
                    "tag": tag,
        ...
        # by (phase,date)
        for (ph, dt), g in d.dropna(subset=["__score__", "__ret__"]).groupby(
            [phase_col, date_col], sort=False
        ):
            ...
            spread = (
                ...
                if (not math.isnan(top_mean) and not math.isnan(bot_mean))
                else np.nan
            )
            ic = (
                float(g2["__score__"].rank(pct=True).corr(g2["__ret__"].rank(pct=True)))
                if len(g2) > 1
                else np.nan
            )
            out_rows.append(
    ...
    if out_rows:
        pd.DataFrame(out_rows).to_csv(
            ppt_dir / "ppt_ranking_quality.csv", index=False, encoding="utf-8"
        )
    else:
        _write_csv(
            ppt_dir / "ppt_ranking_quality.csv",
            [{"tag": UNKNOWN, "source": UNKNOWN}],
            fieldnames=["tag", "source"],
        )
...
def build_backtest_performance(
    ...
    for tag in [run_tag, baseline_tag]:
        bt_ret, bt_ret_src = load_df(
            find_artifact(project_root, extract_dir, tag, "bt_returns")
        )
        if bt_ret is None:
            ...
            bench, bench_src = load_df(
                find_artifact(project_root, extract_dir, tag, "bt_benchmark_returns")
            )
            if bench is not None and set(["phase", "date", "bench_return"]).issubset(
                set(bench.columns)
            ):
                b = bench.copy()
                b["date"] = pd.to_datetime(b["date"], errors="coerce")
                d = d.merge(
                    b[["phase", "date", "bench_return"]],
                    on=["phase", "date"],
                    how="left",
                )
            # merge excess_return if available
            vs, vs_src = load_df(
                find_artifact(project_root, extract_dir, tag, "bt_vs_benchmark")
            )
            if vs is not None and set(["phase", "date", "excess_return"]).issubset(
                set(vs.columns)
            ):
                v = vs.copy()
                v["date"] = pd.to_datetime(v["date"], errors="coerce")
                d = d.merge(
                    v[["phase", "date", "excess_return"]],
                    on=["phase", "date"],
                    how="left",
                )
            ...
            for (ph, m), g in d.groupby(["phase", "month"], sort=False):
                ...
                br = (
                    pd.to_numeric(g.get("bench_return"), errors="coerce")
                    if "bench_return" in g.columns
                    else None
                )
                er = (
                    pd.to_numeric(g.get("excess_return"), errors="coerce")
                    if "excess_return" in g.columns
                    else None
                )
    ...
    pd.DataFrame(dist_rows).to_csv(
        ppt_dir / "ppt_bt_return_distribution.csv", index=False, encoding="utf-8"
    )
    pd.DataFrame(monthly_rows).to_csv(
        ppt_dir / "ppt_monthly_returns.csv", index=False, encoding="utf-8"
    )
...
def build_drawdown_events(
    ...
    for tag in [run_tag, baseline_tag]:
        dd_df, src = load_df(
            find_artifact(project_root, extract_dir, tag, "bt_drawdown_events")
        )
        if dd_df is None:
            rows.append(
                {
    ...
    pd.DataFrame(rows).to_csv(
        ppt_dir / "ppt_drawdown_key_events.csv", index=False, encoding="utf-8"
    )
...
def build_stability(
    ...
    for tag in [run_tag, baseline_tag]:
        y, src = load_df(
            find_artifact(project_root, extract_dir, tag, "bt_yearly_metrics")
        )
        if y is None:
            all_yearly.append(
                pd.DataFrame([{"tag": tag, "source": src, "note": UNKNOWN}])
            )
        else:
            y2 = y.copy()
            y2["tag"] = tag
    ...
    pd.concat(all_yearly, ignore_index=True).to_csv(
        ppt_dir / "ppt_yearly_metrics.csv", index=False, encoding="utf-8"
    )

    # rolling sharpe summary (root only typical)
    rs_art = find_artifact(
        project_root, extract_dir, run_tag, "bt_rolling_sharpe"
    )  # tag doesn't matter due to find order
    ...
    if rs_df is None or "phase" not in rs_df.columns:
        rs_rows.append(
            {
                "source": rs_src,
                "phase": UNKNOWN,
                "rolling_sharpe_min": UNKNOWN,
                "rolling_sharpe_max": UNKNOWN,
                "rolling_sharpe_mean": UNKNOWN,
                "min_date": UNKNOWN,
            }
        )
    else:
        ...
        if "rolling_sharpe" not in d.columns:
            ...
            for ph, g in d.dropna(subset=["rolling_sharpe"]).groupby(
                "phase", sort=False
            ):
                rs_rows.append(
                    {
                        ...
                        "phase": str(ph),
                        "rolling_sharpe_min": (
                            float(g["rolling_sharpe"].min()) if len(g) else UNKNOWN
                        ),
                        "rolling_sharpe_max": (
                            float(g["rolling_sharpe"].max()) if len(g) else UNKNOWN
                        ),
                        "rolling_sharpe_mean": (
                            float(g["rolling_sharpe"].mean()) if len(g) else UNKNOWN
                        ),
                        "min_date": (
                            str(g.loc[g["rolling_sharpe"].idxmin()]["date"].date())
                            if ("date" in g.columns and len(g))
                            else UNKNOWN
                        ),
                    }
                )
    pd.DataFrame(rs_rows).to_csv(
        ppt_dir / "ppt_rolling_sharpe_summary.csv", index=False, encoding="utf-8"
    )
...
def build_sensitivity(
    ...
    for tag in [run_tag, baseline_tag]:
        g, src = load_df(
            find_artifact(project_root, extract_dir, tag, "bt_sensitivity")
        )
        if g is None:
        ...
        for col in ["grid_top_k", "grid_cost_bps", "grid_buffer_k"]:
            if col in top.columns:
                ...
                if len(v) > 0:
                    md_lines.append(
                        f"- {col}_range_in_top10pct: `{float(v.min())}` ~ `{float(v.max())}`"
                    )
                else:
    ...
    pd.concat(grids, ignore_index=True).to_csv(
        ppt_dir / "ppt_sensitivity_grid.csv", index=False, encoding="utf-8"
    )
    _write_text(
        ppt_dir / "ppt_sensitivity_summary.md", "\n".join(md_lines).rstrip() + "\n"
    )


def build_benchmark_fairness(
    project_root: Path,
    extract_dir: Path,
    ppt_dir: Path,
    run_tag: str,
    ...
    if contract.exists():
        ...
        md.append(
            "- 근거 문서에 `build_universe_benchmark_returns` 및 (phase,date) 평균 수익률로 bench_return를 구성하는 코드 스니펫이 포함되어 있습니다."
        )
        md.append("")
    else:
            ...
            perf_rows.append(
                {
                    "tag": tag,
                    "phase": str(ph),
                    "bench_total_return": _safe_num_or_unknown(
                        bm.get("bench_total_return")
                    ),
    ...
    pd.DataFrame(perf_rows).to_csv(
        ppt_dir / "ppt_benchmark_perf.csv", index=False, encoding="utf-8"
    )
    md.append("- benchmark_perf_csv: `reports/extract/ppt/ppt_benchmark_perf.csv`")
    md.append("")
    ...
    for tag in [run_tag, baseline_tag]:
        ...
        if bt is not None and "top_k" in bt.columns:
            try:
                top_k_used = int(
                    pd.to_numeric(
                        bt[bt["phase"].astype(str) == "holdout"]["top_k"].iloc[0],
                        errors="coerce",
                    )
                )
            except Exception:
        ...
        mismatch_rows.append(
            {
                ...
                "cost_bps_mismatch": (
                    (cfg_cost != used_cost)
                    if (cfg_cost is not None and used_cost is not None)
                    else UNKNOWN
                ),
                "config_top_k": cfg_top_k if cfg_top_k is not None else UNKNOWN,
                "top_k_used": top_k_used if top_k_used is not None else UNKNOWN,
                "top_k_mismatch": (
                    (cfg_top_k != top_k_used)
                    if (cfg_top_k is not None and top_k_used is not None)
                    else UNKNOWN
                ),
                "source_kpi": (
                    str(kpi_path.relative_to(project_root))
                    if kpi_path.exists()
                    else UNKNOWN
                ),
    ...
    pd.DataFrame(mismatch_rows).to_csv(
        ppt_dir / "ppt_config_mismatch_table.csv", index=False, encoding="utf-8"
    )
    md.append(
        "- config_mismatch_csv: `reports/extract/ppt/ppt_config_mismatch_table.csv`"
    )
    md.append("")
...
def build_slide_mapping_readme(ppt_dir: Path) -> None:
    ...
    md.append("|---:|---|---|---|---|")
    md.append(
        "| 1 | Executive KPI | `ppt_executive_kpi.csv` | net_total_return, net_sharpe, net_mdd, cost_bps_used | unit_flag 참고 |"
    )
    md.append(
        "| 2 | KPI 단위 점검 | `ppt_kpi_units_check.md` | (text) | % vs decimal 혼용 주의 |"
    )
    md.append(
        "| 3 | Data Snapshot | `ppt_data_snapshot.csv` | rows, cols, unique_tickers/dates, phase date range | - |"
    )
    md.append(
        "| 4 | Feature Missingness | `ppt_feature_missingness.csv` | nan_pct, p01/p99 | - |"
    )
    md.append(
        "| 5 | Model Validation | `ppt_model_validation.csv` | ic_rank, rmse, hit_ratio | - |"
    )
    md.append(
        "| 6 | Ranking Quality | `ppt_ranking_quality.csv` | top/bottom/spread/ic | bottom은 비교 지표(거래 구현 여부 별도) |"
    )
    md.append(
        "| 7 | Backtest Return Dist | `ppt_bt_return_distribution.csv` | p10/p50/p90/min/max | - |"
    )
    md.append(
        "| 8 | Monthly Returns | `ppt_monthly_returns.csv` | net/bench/excess compounded | - |"
    )
    md.append(
        "| 9 | Drawdown Events | `ppt_drawdown_key_events.csv` | peak/trough/drawdown/length | drawdown scale 주의(unit_flag와 별도) |"
    )
    md.append(
        "| 10 | Stability | `ppt_yearly_metrics.csv` | yearly net_total_return, net_sharpe, net_mdd | - |"
    )
    md.append(
        "| 11 | Rolling Sharpe | `ppt_rolling_sharpe_summary.csv` | rolling_sharpe min/max/mean | - |"
    )
    md.append(
        "| 12 | Sensitivity | `ppt_sensitivity_grid.csv` | grid_* + net_sharpe | - |"
    )
    md.append(
        "| 13 | Sensitivity Summary | `ppt_sensitivity_summary.md` | (text) | - |"
    )
    md.append("| 14 | Benchmark/Fairness | `ppt_benchmark_fairness.md` | (text) | - |")
    md.append(
        "| 15 | Config mismatch | `ppt_config_mismatch_table.csv` | cost_bps/top_k mismatch | - |"
    )
    md.append("")
    md.append("## '알 수 없습니다' 항목")
    md.append("")
    md.append(
        "- 각 CSV에서 값이 `알 수 없습니다(근거 파일 없음)`인 경우, 해당 아티팩트/컬럼이 발견되지 않았음을 의미합니다."
    )
    md.append("")
    _write_text(ppt_dir / "README.md", "\n".join(md).rstrip() + "\n")


def bundle_all_outputs_to_single_file(
    ppt_dir: Path, *, out_name: str = "ppt_numeric_pack_all_in_one.md"
) -> None:
    """
    ...
    lines.append("")
    lines.append(
        "> 본 파일은 `reports/extract/ppt/`의 산출물을 **그대로** 이어붙인 번들입니다."
    )
    lines.append("")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ap = argparse.ArgumentParser(
        description="최종 PPT 숫자 산출물 Exporter (reports/extract/ppt)"
    )
    ...
    # [2] executive KPI
    build_executive_kpi(
        project_root, extract_dir, ppt_dir, args.run_tag, args.baseline_tag
    )
