...
def select_cols(df: pd.DataFrame, required: List[str], optional: List[str] = None) -> pd.DataFrame:
...
def build_core_extracts() -> Dict[str, pd.DataFrame]:
    ...
    if panel is not None:
        outputs["L3_panel_merged_core"] = select_cols(
            panel,
            required=["date", "ticker"],
            optional=["close", "volume", "net_income", "total_liabilities", "equity", "debt_ratio", "roe", "ym", "in_universe"]
        )
    ...
    if ds is not None:
        ...
        outputs["L4_dataset_targets_core"] = select_cols(
            ds,
            required=["date", "ticker"],
            optional=["ret_fwd_20d", "ret_fwd_120d"]
        )
    ...
    if cv_s is not None:
        outputs["L4_cv_folds_short_core"] = select_cols(
            cv_s,
            required=["fold_id", "segment", "train_start", "train_end", "test_start", "test_end"],
            optional=[]
        )
    ...
    if mm is not None:
        outputs["L5_model_metrics_core"] = select_cols(
            ...
            optional=["ic_rank", "rmse", "mae", "hit_ratio", "n_features", "n_train", "n_test"]
        )
    ...
    if rss is not None:
        coverage_col = pick_first_existing_col(
            rss,
            ["coverage_vs_universe_pct", "coverage_ticker_pct", "coverage_vs_universe", "coverage_vs_universe_percent"]
        )
        if coverage_col is None:
            print("[WARN] rebalance_scores_summary: coverage column not found (expected coverage_vs_universe_pct or coverage_ticker_pct)")
    ...
    if bm is not None:
        outputs["L7_bt_metrics_core"] = select_cols(
            ...
            required=["phase"],
            optional=[
                "top_k", "holding_days", "cost_bps", "buffer_k", "weighting",
                "net_total_return", "net_cagr", "net_sharpe", "net_mdd",
                "avg_turnover_oneway", "date_start", "date_end"
            ]
        )
    ...
    if br is not None:
        net_ret_col = pick_first_existing_col(br, ["net_return", "net_ret", "net_period_return"])
        if net_ret_col is None:
            raise KeyError("[F] bt_returns: net return column not found (net_return / net_ret / net_period_return)")
...
def main():
    ...
    # manifest 저장
    manifest = pd.DataFrame(manifest_rows).sort_values("artifact_core_name").reset_index(drop=True)
