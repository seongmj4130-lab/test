...
def resolve_artifact(
    name: str, root: Path, tag: Optional[str] = None
...
def extract_data_kpis(root: Path, tag: Optional[str]) -> list[dict[str, Any]]:
    ...
    if path:
        try:
            ...
            months_count = (
                df["ym"].nunique()
                if "ym" in df.columns
                else df["date"].dt.to_period("M").nunique()
            )
            tickers_per_month = df.groupby(
                "ym" if "ym" in df.columns else df["date"].dt.to_period("M")
            )["ticker"].nunique()
            ...
            add_kpi_row(
                rows,
                "DATA",
                "tickers_per_month_min",
                tickers_per_month.min(),
                None,
                "count",
                str(path),
                "ticker grouped by ym",
                "",
            )
            add_kpi_row(
                rows,
                "DATA",
                "tickers_per_month_mean",
                tickers_per_month.mean(),
                None,
                "count",
                str(path),
                "ticker grouped by ym",
            ...
            add_kpi_row(
                rows,
                "DATA",
                "tickers_per_month_max",
                tickers_per_month.max(),
                None,
                "count",
                str(path),
                "ticker grouped by ym",
    ...
    if path:
        try:
            ...
            n_tickers = (
                df_sample["ticker"].nunique() if "ticker" in df_sample.columns else None
            )
            n_dates = (
                df_sample["date"].nunique() if "date" in df_sample.columns else None
            )
        ...
        add_kpi_row(
            rows, "DATA", "ohlcv_daily", None, None, "count", "", "", "MISSING ARTIFACT"
        )
    ...
    if path:
        try:
            ...
            years_count = (
                df_sample["date"].dt.year.nunique()
                if "date" in df_sample.columns
                else None
            )
            coverage_ratio = (
                rows_total / (n_tickers * years_count)
                if (n_tickers and years_count)
                else None
            )
            ...
            if "lag_source" in df_lag.columns:
                ...
                if total > 0:
                    ...
                    fallback_pct = (
                        lag_source_counts.get("year_end_fallback", 0) / total
                    ) * 100
                    add_kpi_row(
                        rows,
                        "DATA",
                        "fundamentals_lag_source_rcept_pct",
                        rcept_pct,
                        None,
                        "%",
                        str(path),
                    ...
                    add_kpi_row(
                        rows,
                        "DATA",
                        "fundamentals_lag_source_fallback_pct",
                        fallback_pct,
                        None,
                        "%",
                        str(path),
    ...
    if path:
        try:
            ...
            ohlcv_rows_for_check = (
                get_row_count_meta(ohlcv_path) if ohlcv_path else None
            )

            df_sample = load_partial(
                path, ["net_income", "equity", "debt_ratio", "roe"], fmt
            )

            note = ""
            if ohlcv_rows_for_check and panel_rows and ohlcv_rows_for_check > 0:
                diff_pct = (
                    abs(panel_rows - ohlcv_rows_for_check) / ohlcv_rows_for_check * 100
                )
                if diff_pct > 1.0:
            ...
            if ohlcv_rows_for_check:
                keep_ratio = (
                    (panel_rows / ohlcv_rows_for_check * 100)
                    if ohlcv_rows_for_check > 0
                    else None
                )
                add_kpi_row(
                    rows,
                    "DATA",
                    "panel_keep_ratio_vs_ohlcv",
                    keep_ratio,
                    None,
                    "%",
                    str(path),
                    "panel_rows / ohlcv_rows",
                    "",
                )
        ...
        try:
            ...
            if "coverage_vs_universe_pct" in df.columns:
                add_kpi_row(
                    rows,
                    "DATA",
                    "rebalance_coverage_vs_universe_pct_mean",
                    df["coverage_vs_universe_pct"].mean(),
                    None,
                    "%",
                    str(path),
            ...
            score_missing_cols = [
                c for c in df.columns if "missing" in c.lower() and "score" in c.lower()
            ]
            if score_missing_cols:
                # total이면 sum(정수), 평균이면 missing_mean_per_rebalance로 이름 변경
                if all(
                    "total" in c.lower() or "sum" in c.lower()
                    for c in score_missing_cols
                ):
                    score_missing_total = df[score_missing_cols].sum(axis=1).sum()
                    add_kpi_row(
                        rows,
                        "DATA",
                        "rebalance_score_missing_total",
                        int(score_missing_total),
                        None,
                        "count",
                        str(path),
                        ",".join(score_missing_cols),
                        "",
                    )
                else:
                    # 평균인 경우
                    ...
                    add_kpi_row(
                        rows,
                        "DATA",
                        "rebalance_score_missing_mean_per_rebalance",
                        score_missing_mean,
                        None,
                        "count",
                        str(path),
...
def extract_model_kpis(root: Path, tag: Optional[str]) -> list[dict[str, Any]]:
    ...
    if not path:
        add_kpi_row(
            rows, "MODEL", "model_metrics", None, None, "", "", "", "MISSING ARTIFACT"
        )
        return rows
    ...
    try:
        ...
        if df.empty:
            add_kpi_row(
                rows, "MODEL", "model_metrics", None, None, "", str(path), "", "EMPTY"
            )
            return rows
        ...
        for horizon in [20, 120]:
            ...
            for metric_col, stats in metrics.items():
                ...
                for stat in stats:
                    ...
                    add_kpi_row(
                        ...
                        unit,
                        str(path),
                        f"{metric_col} by horizon={horizon}",
                        "",
                    )

            # n_folds
            dev_n_folds = (
                df_dev["fold_id"].nunique()
                if not df_dev.empty and "fold_id" in df_dev.columns
                else None
            )
            holdout_n_folds = (
                df_holdout["fold_id"].nunique()
                if not df_holdout.empty and "fold_id" in df_holdout.columns
                else None
            )
            ...
            if "n_features" in df_horizon.columns:
                dev_feat_mean = (
                    df_dev["n_features"].mean() if not df_dev.empty else None
                )
                ...
                holdout_feat_mean = (
                    df_holdout["n_features"].mean() if not df_holdout.empty else None
                )
                holdout_feat_min = (
                    df_holdout["n_features"].min() if not df_holdout.empty else None
                )
                holdout_feat_max = (
                    df_holdout["n_features"].max() if not df_holdout.empty else None
                )
                ...
                for stat in ["mean", "min", "max"]:
                    ...
                    dev_val = (
                        dev_feat_mean
                        if stat == "mean"
                        else (dev_feat_min if stat == "min" else dev_feat_max)
                    )
                    holdout_val = (
                        holdout_feat_mean
                        if stat == "mean"
                        else (holdout_feat_min if stat == "min" else holdout_feat_max)
                    )
                    add_kpi_row(
                        rows,
                        "MODEL",
            ...
            if has_r2_oos:
                ...
                holdout_r2_oos_mean = (
                    df_holdout["r2_oos"].mean() if not df_holdout.empty else None
                )
                ...
                add_kpi_row(
                    ...
                    "ratio",
                    str(path),
                    f"r2_oos mean for horizon={horizon}",
                    "",
                )

    except Exception as e:
        add_kpi_row(
            rows, "MODEL", "model_metrics", None, None, "", str(path), "", f"ERROR: {e}"
        )

    # [Stage 2] feature_importance_summary에서 feature_sign_stability_top5 추출
    path_summary, fmt_summary = resolve_artifact(
        "feature_importance_summary", root, tag
    )
    if path_summary:
        try:
            df_summary = (
                pd.read_parquet(path_summary)
                if fmt_summary == "parquet"
                else pd.read_csv(path_summary, low_memory=False)
            )
                            ...
                            if not df_phase.empty:
                                ...
                                if phase == "dev":
                                    add_kpi_row(
                                        ...
                                        "str",
                                        str(path_summary),
                                        f"top5 features by abs_coef_mean (horizon={horizon}, phase={phase})",
                                        "",
                                    )
                                else:
...
def extract_backtest_kpis(
    root: Path, tag: Optional[str], config_cost_bps: Optional[float] = None
) -> list[dict[str, Any]]:
    """BACKTEST 섹션 KPI 추출"""
    rows = []
    ...
    if not path:
        add_kpi_row(
            rows, "BACKTEST", "bt_metrics", None, None, "", "", "", "MISSING ARTIFACT"
        )
        return rows
    ...
    try:
        ...
        dev_cost_bps_used = (
            row_dev.get("cost_bps")
            if row_dev is not None and "cost_bps" in row_dev.index
            else None
        )
        holdout_cost_bps_used = (
            row_holdout.get("cost_bps")
            if row_holdout is not None and "cost_bps" in row_holdout.index
            else None
        )
        ...
        if config_cost_bps is not None:
            dev_mismatch = (
                (abs(dev_cost_bps_used - config_cost_bps) > 0.01)
                if (dev_cost_bps_used is not None)
                else None
            )
            holdout_mismatch = (
                (abs(holdout_cost_bps_used - config_cost_bps) > 0.01)
                if (holdout_cost_bps_used is not None)
                else None
            )
            if dev_mismatch is not None or holdout_mismatch is not None:
                add_kpi_row(
                    ...
                    "bool",
                    str(path),
                    f"cost_bps_used vs config({config_cost_bps})",
                    "",
                )
        ...
        if (
            row_dev is not None
            and "gross_total_return" in row_dev.index
            and "net_total_return" in row_dev.index
        ):
        ...
        if (
            row_holdout is not None
            and "gross_total_return" in row_holdout.index
            and "net_total_return" in row_holdout.index
        ):
        ...
        add_kpi_row(
            rows, "BACKTEST", "bt_metrics", None, None, "", str(path), "", f"ERROR: {e}"
        )
    ...
    if path:
        try:
            ...
            if "phase" in df.columns and "net_return" in df.columns:
                ...
                dev_return_mean_raw = (
                    df_dev["net_return"].mean() if not df_dev.empty else None
                )
                holdout_return_mean_raw = (
                    df_holdout["net_return"].mean() if not df_holdout.empty else None
                )
                dev_return_std_raw = (
                    df_dev["net_return"].std() if not df_dev.empty else None
                )
                holdout_return_std_raw = (
                    df_holdout["net_return"].std() if not df_holdout.empty else None
                )

                dev_return_mean, _ = normalize_unit(
                    "net_return_mean", dev_return_mean_raw
                )
                holdout_return_mean, _ = normalize_unit(
                    "net_return_mean", holdout_return_mean_raw
                )
                dev_return_std, _ = normalize_unit("net_return_std", dev_return_std_raw)
                holdout_return_std, _ = normalize_unit(
                    "net_return_std", holdout_return_std_raw
                )

                add_kpi_row(
                    rows,
                    "BACKTEST",
                    "bt_returns_net_return_mean",
                    dev_return_mean,
                    holdout_return_mean,
                    "%",
                    str(path),
                ...
                if "turnover_oneway" in df.columns:
                    dev_turnover_raw = (
                        df_dev["turnover_oneway"].mean() if not df_dev.empty else None
                    )
                    holdout_turnover_raw = (
                        df_holdout["turnover_oneway"].mean()
                        if not df_holdout.empty
                        else None
                    )
                    dev_turnover, _ = normalize_unit(
                        "turnover_oneway_mean", dev_turnover_raw
                    )
                    holdout_turnover, _ = normalize_unit(
                        "turnover_oneway_mean", holdout_turnover_raw
                    )
                    add_kpi_row(
                        rows,
                        "BACKTEST",
                        "bt_returns_turnover_oneway_mean",
                        dev_turnover,
                        holdout_turnover,
                        "%",
                        str(path),
                ...
                if "cost" in df.columns:
                    ...
                    holdout_avg_cost = (
                        df_holdout["cost"].mean() if not df_holdout.empty else None
                    )
                    dev_avg_cost_pct, _ = normalize_unit("avg_cost_pct", dev_avg_cost)
                    holdout_avg_cost_pct, _ = normalize_unit(
                        "avg_cost_pct", holdout_avg_cost
                    )
                    add_kpi_row(
                        rows,
                        "BACKTEST",
                    ...
                    dev_cost = (
                        (df_dev["gross_return"] - df_dev["net_return"]).mean()
                        if not df_dev.empty
                        else None
                    )
                    holdout_cost = (
                        (df_holdout["gross_return"] - df_holdout["net_return"]).mean()
                        if not df_holdout.empty
                        else None
                    )
                    dev_avg_cost_pct, _ = normalize_unit("avg_cost_pct", dev_cost)
                    holdout_avg_cost_pct, _ = normalize_unit(
                        "avg_cost_pct", holdout_cost
                    )
                    add_kpi_row(
                        rows,
                        "BACKTEST",
    ...
    if path_diag:
        try:
            df_diag = (
                pd.read_parquet(path_diag)
                if fmt_diag == "parquet"
                else pd.read_csv(path_diag, low_memory=False)
            )
            if "phase" in df_diag.columns:
                ...
                if "selected_count" in df_diag.columns:
                    dev_k_eff_mean = (
                        df_dev_diag["selected_count"].mean()
                        if not df_dev_diag.empty
                        else None
                    )
                    holdout_k_eff_mean = (
                        df_holdout_diag["selected_count"].mean()
                        if not df_holdout_diag.empty
                        else None
                    )
                    add_kpi_row(
                        rows,
                ...
                if "selected_count" in df_diag.columns and "top_k" in df_diag.columns:
                    dev_fill_rate = (
                        (df_dev_diag["selected_count"] / df_dev_diag["top_k"]).mean()
                        if not df_dev_diag.empty
                        else None
                    )
                    holdout_fill_rate = (
                        (
                            df_holdout_diag["selected_count"] / df_holdout_diag["top_k"]
                        ).mean()
                        if not df_holdout_diag.empty
                        else None
                    )
                    dev_fill_rate_pct = (
                        dev_fill_rate * 100 if dev_fill_rate is not None else None
                    )
                    holdout_fill_rate_pct = (
                        holdout_fill_rate * 100
                        if holdout_fill_rate is not None
                        else None
                    )
                    add_kpi_row(
                        rows,
                        "BACKTEST",
                        "k_eff_fill_rate_mean_dev",
                        dev_fill_rate_pct,
                        None,
                        "%",
                        str(path_diag),
                ...
                if "eligible_count" in df_diag.columns:
                    dev_eligible_mean = (
                        df_dev_diag["eligible_count"].mean()
                        if not df_dev_diag.empty
                        else None
                    )
                    holdout_eligible_mean = (
                        df_holdout_diag["eligible_count"].mean()
                        if not df_holdout_diag.empty
                        else None
                    )
                    add_kpi_row(
                ...
                if "dropped_missing" in df_diag.columns:
                    dev_dropped_missing_mean = (
                        df_dev_diag["dropped_missing"].mean()
                        if not df_dev_diag.empty
                        else None
                    )
                    holdout_dropped_missing_mean = (
                        df_holdout_diag["dropped_missing"].mean()
                        if not df_holdout_diag.empty
                        else None
                    )
                    add_kpi_row(
                    ...
                    add_kpi_row(
                        rows,
                        "BACKTEST",
                        "dropped_by_missing_mean_holdout",
                        None,
                        holdout_dropped_missing_mean,
                        "count",
                        str(path_diag),
                ...
                if "dropped_filter" in df_diag.columns:
                    dev_dropped_filter_mean = (
                        df_dev_diag["dropped_filter"].mean()
                        if not df_dev_diag.empty
                        else None
                    )
                    holdout_dropped_filter_mean = (
                        df_holdout_diag["dropped_filter"].mean()
                        if not df_holdout_diag.empty
                        else None
                    )
                    add_kpi_row(
                    ...
                    add_kpi_row(
                        rows,
                        "BACKTEST",
                        "dropped_by_filter_mean_holdout",
                        None,
                        holdout_dropped_filter_mean,
                        "count",
                        str(path_diag),
                ...
                if "dropped_sectorcap" in df_diag.columns:
                    dev_dropped_sectorcap_mean = (
                        df_dev_diag["dropped_sectorcap"].mean()
                        if not df_dev_diag.empty
                        else None
                    )
                    holdout_dropped_sectorcap_mean = (
                        df_holdout_diag["dropped_sectorcap"].mean()
                        if not df_holdout_diag.empty
                        else None
                    )
                    add_kpi_row(
                    ...
                    add_kpi_row(
                        rows,
                        "BACKTEST",
                        "dropped_by_sectorcap_mean_holdout",
                        None,
                        holdout_dropped_sectorcap_mean,
                        "count",
                        str(path_diag),
    ...
    if path_bt_diag:
        try:
            df_bt_diag = (
                pd.read_parquet(path_bt_diag)
                if fmt_bt_diag == "parquet"
                else pd.read_csv(path_bt_diag, low_memory=False)
            )
            if "phase" in df_bt_diag.columns:
                ...
                if "regime" in df_bt_diag.columns:
                    dev_regime_nonnull_pct = (
                        (
                            df_dev_bt_diag["regime"].notna().sum()
                            / len(df_dev_bt_diag)
                            * 100
                        )
                        if not df_dev_bt_diag.empty
                        else None
                    )
                    holdout_regime_nonnull_pct = (
                        (
                            df_holdout_bt_diag["regime"].notna().sum()
                            / len(df_holdout_bt_diag)
                            * 100
                        )
                        if not df_holdout_bt_diag.empty
                        else None
                    )
                    add_kpi_row(
                        rows,
                        "BACKTEST",
                        "bt_returns_diag_regime_nonnull_pct_dev",
                        dev_regime_nonnull_pct,
                        None,
                        "%",
                        str(path_bt_diag),
                    ...
                    add_kpi_row(
                        rows,
                        "BACKTEST",
                        "bt_returns_diag_regime_nonnull_pct_holdout",
                        None,
                        holdout_regime_nonnull_pct,
                        "%",
                        str(path_bt_diag),
                ...
                if "exposure" in df_bt_diag.columns:
                    dev_exposure_nonnull_pct = (
                        (
                            df_dev_bt_diag["exposure"].notna().sum()
                            / len(df_dev_bt_diag)
                            * 100
                        )
                        if not df_dev_bt_diag.empty
                    ...
                    holdout_exposure_nonnull_pct = (
                        (
                            df_holdout_bt_diag["exposure"].notna().sum()
                            / len(df_holdout_bt_diag)
                            * 100
                        )
                        if not df_holdout_bt_diag.empty
                    ...
                    add_kpi_row(
                        rows,
                        "BACKTEST",
                        "bt_returns_diag_exposure_nonnull_pct_dev",
                        dev_exposure_nonnull_pct,
                        None,
                        "%",
                        str(path_bt_diag),
                    ...
                    add_kpi_row(
                        rows,
                        "BACKTEST",
                        "bt_returns_diag_exposure_nonnull_pct_holdout",
                        None,
                        holdout_exposure_nonnull_pct,
                        "%",
                        str(path_bt_diag),
                ...
                add_kpi_row(
                    rows,
                    "BACKTEST",
                    "bt_returns_diag_saved",
                    True,
                    True,
                    "bool",
                    str(path_bt_diag),
                    "bt_returns_diagnostics saved",
                    "",
                )
                ...
                if path_bt_core:
                    try:
                        df_bt_core = (
                            pd.read_parquet(path_bt_core)
                            if fmt_bt_core == "parquet"
                            else pd.read_csv(path_bt_core, low_memory=False)
                        )
        ...
        try:
            ...
            if "phase" in df.columns:
                ...
                if "net_total_return" in df.columns:
                    returns_dev = (
                        df_dev["net_total_return"] if not df_dev.empty else None
                    )
                    returns_holdout = (
                        df_holdout["net_total_return"] if not df_holdout.empty else None
                    )
                    if returns_dev is not None and len(returns_dev) > 0:
                        # Series를 직접 처리
                        returns_dev_pct = (
                            returns_dev * 100
                            if isinstance(returns_dev, pd.Series)
                            and returns_dev.abs().max() < 10
                            else returns_dev
                        )
                        add_kpi_row(
                            rows,
                            ...
                            (
                                returns_dev_pct.min()
                                if isinstance(returns_dev_pct, pd.Series)
                                else None
                            ),
                            None,
                            "%",
                    ...
                    if returns_holdout is not None and len(returns_holdout) > 0:
                        returns_holdout_pct = (
                            returns_holdout * 100
                            if isinstance(returns_holdout, pd.Series)
                            and returns_holdout.abs().max() < 10
                            else returns_holdout
                        )
                        # 기존 행 업데이트
                        for metric_name, val in zip(
                            [
                                "yearly_return_min",
                                "yearly_return_max",
                                "yearly_return_mean",
                            ...
                            [
                                (
                                    returns_holdout_pct.min()
                                    if isinstance(returns_holdout_pct, pd.Series)
                                    else None
                                ),
                                (
                                    returns_holdout_pct.max()
                                    if isinstance(returns_holdout_pct, pd.Series)
                                    else None
                                ),
                                (
                                    returns_holdout_pct.mean()
                                    if isinstance(returns_holdout_pct, pd.Series)
                                    else None
                                ),
                            ],
                ...
                if "net_sharpe" in df.columns:
                    ...
                    sharpe_holdout = (
                        df_holdout["net_sharpe"] if not df_holdout.empty else None
                    )
                ...
                if "net_mdd" in df.columns:
                    ...
                    mdd_holdout = (
                        df_holdout["net_mdd"] if not df_holdout.empty else None
                    )
                    if mdd_dev is not None and len(mdd_dev) > 0:
                        mdd_dev_pct = (
                            mdd_dev * 100
                            if isinstance(mdd_dev, pd.Series)
                            and mdd_dev.abs().max() < 10
                            else mdd_dev
                        )
                        add_kpi_row(
                            rows,
                            ...
                            (
                                mdd_dev_pct.min()
                                if isinstance(mdd_dev_pct, pd.Series)
                                else None
                            ),
                            None,
                            "%",
                    ...
                    if mdd_holdout is not None and len(mdd_holdout) > 0:
                        mdd_holdout_pct = (
                            mdd_holdout * 100
                            if isinstance(mdd_holdout, pd.Series)
                            and mdd_holdout.abs().max() < 10
                            else mdd_holdout
                        )
                        existing_idx = None
                        for i, r in enumerate(rows):
                            if r.get("metric") == "yearly_mdd_min":
                                ...
                                break
                        if existing_idx is not None:
                            rows[existing_idx]["holdout_value"] = (
                                mdd_holdout_pct.min()
                                if isinstance(mdd_holdout_pct, pd.Series)
                                else None
                            )
                        else:
                            add_kpi_row(
                ...
                if "year" in df.columns:
                    ...
                    years_holdout = (
                        df_holdout["year"].nunique() if not df_holdout.empty else None
                    )
                    add_kpi_row(
                        rows,
                        "STABILITY",
            ...
            if "phase" in df.columns and "net_rolling_sharpe" in df.columns:
                ...
                if not df_holdout.empty and len(df_holdout) > 0:
                    ...
                    for metric_name, val in zip(
                        ...
                        [
                            sharpe_holdout.min(),
                            sharpe_holdout.quantile(0.05),
                            sharpe_holdout.median(),
                            sharpe_holdout.quantile(0.95),
                            sharpe_holdout.max(),
                        ],
                    ):
                        existing_idx = None
        ...
        try:
            ...
            if "phase" in df.columns and "drawdown" in df.columns:
                ...
                if not df_dev.empty:
                    ...
                    worst_dd_dev_pct = (
                        worst_dd_dev * 100
                        if isinstance(worst_dd_dev, (int, float))
                        and abs(worst_dd_dev) < 10
                        else worst_dd_dev
                    )
                    worst_idx_dev = df_dev["drawdown"].idxmin()
                    worst_length_dev = (
                        df_dev.loc[worst_idx_dev, "length_days"]
                        if "length_days" in df_dev.columns
                        else None
                    )
                    add_kpi_row(
                        rows,
                ...
                if not df_holdout.empty:
                    ...
                    worst_dd_holdout_pct = (
                        worst_dd_holdout * 100
                        if isinstance(worst_dd_holdout, (int, float))
                        and abs(worst_dd_holdout) < 10
                        else worst_dd_holdout
                    )
                    worst_idx_holdout = df_holdout["drawdown"].idxmin()
                    worst_length_holdout = (
                        df_holdout.loc[worst_idx_holdout, "length_days"]
                        if "length_days" in df_holdout.columns
                        else None
                    )
                    # 기존 행 업데이트
                    existing_idx = None
...
def extract_settings_kpis(
    root: Path, config_path: Path, tag: str
) -> list[dict[str, Any]]:
    """SETTINGS 섹션 KPI 추출"""
    rows = []
    ...
    for param in l4_params:
        ...
        add_kpi_row(
            ...
            val,
            None,
            "count" if "years" in param or "days" in param else "count",
            str(config_path),
            f"l4.{param}",
            "",
        )
    ...
    for param in l7_params:
        ...
        unit = (
            ...
            if param in ["holding_days", "top_k", "buffer_k"]
            else "str"
        )
        add_kpi_row(
            rows,
    ...
    for section in sections:
        ...
        if section == "MODEL":
            ...
            for horizon in sorted(set(horizons)):
                ...
                lines.append("")
                df_h = df_section[
                    df_section["metric"].str.contains(f"__{horizon}d", na=False)
                ].copy()

                # metric에서 horizon 제거
                df_h["metric_short"] = df_h["metric"].str.replace(
                    f"__{horizon}d", "", regex=False
                )
                ...
                df_h["sort_key"] = df_h["metric_short"].apply(
                    lambda x: next(
                        (i for i, m in enumerate(metric_order) if m in str(x)), 999
                    )
                )
                df_h = df_h.sort_values(["sort_key", "metric_short"]).drop(
                    columns=["sort_key"]
                )
                ...
                for _, row in df_h.iterrows():
                    ...
                    lines.append(
                        f"| {row['metric_short']} | {dev_val} | {holdout_val} | {row['unit']} |"
                    )
                lines.append("")
        else:
            # 정렬: 알파벳 순서 또는 논리적 순서
            if section == "BACKTEST":
                ...
                df_section["sort_key"] = df_section["metric"].apply(
                    lambda x: next(
                        (i for i, m in enumerate(priority_order) if m in str(x)), 999
                    )
                )
                df_section = df_section.sort_values(["sort_key", "metric"]).drop(
                    columns=["sort_key"]
                )
            elif section == "DATA":
                priority_order = [
            ...
            for _, row in df_section.iterrows():
                ...
                lines.append(
                    f"| {metric_display} | {dev_val} | {holdout_val} | {row['unit']} |"
                )
            lines.append("")

            # Note가 있는 행은 하단에 요약
            notes_df = df_section[
                df_section["note"].notna() & (df_section["note"] != "")
            ]
...
def main():
    ...
    if len(df) > 0:
        df_grouped = df.groupby(["section", "metric"], as_index=False).agg(
            {
                "dev_value": lambda x: (
                    x.dropna().iloc[0] if x.dropna().size > 0 else None
                ),
                "holdout_value": lambda x: (
                    x.dropna().iloc[0] if x.dropna().size > 0 else None
                ),
                "unit": "first",
                "source_file": lambda x: (
                    ", ".join(x.dropna().unique()) if x.dropna().size > 0 else ""
                ),
                "source_cols": "first",
                "note": lambda x: (
                    "; ".join(x.dropna().unique()) if x.dropna().size > 0 else ""
    ...
    if len(df) < 40:
        print(
            f"WARNING: KPI table has only {len(df)} rows (expected >= 40)",
            file=sys.stderr,
        )

    backtest_dev = (
        df[(df["section"] == "BACKTEST") & (df["metric"] == "net_total_return")][
            "dev_value"
        ].iloc[0]
        if len(df[(df["section"] == "BACKTEST") & (df["metric"] == "net_total_return")])
        > 0
        else None
    )
    backtest_holdout = (
    ...
    if pd.isna(backtest_dev) and pd.isna(backtest_holdout):
        print(
            "ERROR: Both dev and holdout net_total_return are missing", file=sys.stderr
        )
        sys.exit(2)
    ...
    dev_sharpe = (
        df[(df["section"] == "BACKTEST") & (df["metric"] == "net_sharpe")][
            "dev_value"
        ].iloc[0]
        if len(df[(df["section"] == "BACKTEST") & (df["metric"] == "net_sharpe")]) > 0
        else None
    )
    holdout_sharpe = (
        df[(df["section"] == "BACKTEST") & (df["metric"] == "net_sharpe")][
