# C:/Users/seong/OneDrive/Desktop/bootcamp/03_code/src/tools/run_stage_template.py
"""
[코드 매니저] Stage {N} 실행 엔트리포인트
...
# Stage 설정 (각 파일에서 수정)
STAGE_NUM = 1  # Stage 번호 (0~8)
STAGE_NAME = "L1"  # Stage 이름 (L0, L1, ..., L8)
STAGE_TRACK = "pipeline"  # "pipeline" 또는 "ranking"
...
def get_baseline_tag(config_path: Path, stage: int) -> str:
    ...
    if stage <= 6:
        return cfg.get("baseline", {}).get(
            "pipeline_baseline_tag", "baseline_prerefresh_20251219_143636"
        )
    # Stage7+: ranking_baseline_tag (Stage7이 없으면 에러)
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description=f"[코드 매니저] Stage {STAGE_NUM} 실행"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Baseline tag (없으면 config에서 읽음)",
    )
    parser.add_argument(
        "--change-title",
        type=str,
    ...
    parser.add_argument(
        "--modified-files", type=str, default=None, help="Modified files"
    )
    parser.add_argument(
        "--modified-functions", type=str, default=None, help="Modified functions"
    )
    ...
    # 3) Δ 생성 (Stage7은 선택적)
    if (
        STAGE_NUM != 7 or baseline_tag_used
    ):  # Stage7은 ranking_baseline 생성 단계이므로 선택적
        delta_cmd = [
            sys.executable,
    ...
    if STAGE_NUM == 7:
        print(
            f"Ranking Baseline Tag: {run_tag} (Stage7 완료 시 ranking_baseline_tag로 설정됨)"
        )
    print("\n생성된 주요 파일:")
    ...
    print("  - reports/history/history_manifest.parquet")
    print("=" * 60 + "\n")
