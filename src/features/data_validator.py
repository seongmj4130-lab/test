...
class DataValidator:
    ...
    def validate_feature_consistency(
        ...
        for feature in feature_columns:
            ...
            consistency_score = group_stats["std"].mean() / abs(
                group_stats["mean"].mean() + 1e-8
            )
        ...
        if "date" in df.columns:
            for feature in feature_columns:
                ...
                date_consistency = date_stats["std"].mean() / abs(
                    date_stats["mean"].mean() + 1e-8
                )
    ...
    def validate_feature_correlations(
        ...
        if target_column and target_column in df.columns:
            ...
            correlation_result["target_correlations"] = target_corr.drop(
                target_column
            ).to_dict()
            ...
            low_corr_features = [
                feat
                for feat, corr in correlation_result["target_correlations"].items()
                if abs(corr) < 0.01
            ]
    ...
    def run_comprehensive_validation(
        ...
        # 4. 피쳐 상관관계 검증
        print("  📈 피쳐 상관관계 검증 중...")
        correlation_result = self.validate_feature_correlations(
    ...
    def _generate_validation_summary(
        self, integrity: dict, quality: dict, consistency: dict, correlation: dict
    ) -> dict[str, Any]:
        """검증 결과 요약 생성"""
    ...
    def _calculate_data_quality_score(
        ...
        # 무결성 점수 차감
        missing_ratio = sum(integrity.get("missing_values", {}).values()) / (
            integrity.get("total_rows", 1) * integrity.get("total_columns", 1)
        )
    ...
    def generate_validation_report(self, validation_result: dict) -> str:
        ...
        report.append(
            f"- **데이터 품질 점수**: {summary['data_quality_score']:.1f}/100"
        )
