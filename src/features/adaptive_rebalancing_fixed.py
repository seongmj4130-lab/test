...
class AdaptiveRebalancing:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.signal_strength_threshold_high = config.get(
            "signal_strength_threshold_high", 0.8
        )
        self.signal_strength_threshold_low = config.get(
        ...
        logger.info(
            f"적응형 리밸런싱 초기화: {self.min_rebalance_days}-{self.max_rebalance_days}일 범위"
        )
    ...
    def calculate_rolling_ic(self, scores_df: pd.DataFrame) -> pd.Series:
        ...
        if (
            scores_df.empty
            or "score_total_short" not in scores_df.columns
            or "true_short" not in scores_df.columns
        ):
        ...
        try:
            daily_ic = scores_df.groupby("date").apply(
                lambda x: x["score_total_short"].corr(
                    x["true_short"], method="spearman"
                )
            )
        ...
        rolling_ic = daily_ic.rolling(
            window=self.ic_lookback_window, min_periods=self.ic_min_periods
        ).mean()

        # IC 계산 결과 로깅
        logger.info(
            f"IC 계산 결과 - 일별 IC 범위: {daily_ic.min():.3f} ~ {daily_ic.max():.3f}"
        )
        logger.info(
            f"IC 계산 결과 - 일별 IC 평균: {daily_ic.mean():.3f}, 표준편차: {daily_ic.std():.3f}"
        )
        logger.info(
            f"IC 계산 결과 - 롤링 IC 범위: {rolling_ic.min():.3f} ~ {rolling_ic.max():.3f}"
        )
        logger.info(f"IC 계산 결과 - 롤링 IC 평균: {rolling_ic.mean():.3f}")
        logger.info(
            f"IC 계산 결과 - 유효 IC 개수: {rolling_ic.notna().sum()}/{len(rolling_ic)}"
        )
    ...
    def get_signal_strength_score(self, rolling_ic: pd.Series) -> pd.Series:
        ...
        # 시그널 강도 계산 결과 로깅
        logger.info(
            f"시그널 강도 계산 - 범위: {signal_scores.min():.1f} ~ {signal_scores.max():.1f}"
        )
        logger.info(
            f"시그널 강도 계산 - 평균: {signal_scores.mean():.1f}, 표준편차: {signal_scores.std():.1f}"
        )
        logger.info(
            f"시그널 강도 계산 - 분포: 강함({(signal_scores >= 80).sum()}), 중간({((signal_scores >= 60) & (signal_scores < 80)).sum()}), 약함({(signal_scores < 60).sum()})"
        )
    ...
    def get_adaptive_rebalance_intervals(self, scores_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if rolling_ic.empty:
            ...
            return pd.DataFrame(
                {"date": dates, "rebalance_interval": self.default_rebalance_days}
            )
        ...
        intervals_df["rebalance_interval"] = (
            intervals_df["rebalance_interval"]
            .ffill()
            .fillna(self.default_rebalance_days)
            .astype(int)
        )
        ...
        logger.info(
            f"평균 리밸런싱 간격: {intervals_df['rebalance_interval'].mean():.1f}일"
        )
        logger.info(f"간격 분포 상세: {dict(interval_counts)}")

        # 시그널 강도별 간격 매핑 확인
        strong_signals = intervals_df[
            intervals_df["rebalance_interval"] == self.min_rebalance_days
        ]
        medium_signals = intervals_df[
            intervals_df["rebalance_interval"] == self.default_rebalance_days
        ]
        weak_signals = intervals_df[
            intervals_df["rebalance_interval"] == self.max_rebalance_days
        ]

        logger.info(
            f"강한 시그널 (15일): {len(strong_signals)}개 (평균 강도: {strong_signals['signal_score'].mean():.1f})"
        )
        logger.info(
            f"중간 시그널 (20일): {len(medium_signals)}개 (평균 강도: {medium_signals['signal_score'].mean():.1f})"
        )
        logger.info(
            f"약한 시그널 (25일): {len(weak_signals)}개 (평균 강도: {weak_signals['signal_score'].mean():.1f})"
        )

        return intervals_df[["date", "rebalance_interval"]]

    def filter_adaptive_dates(
        self, all_dates: list[pd.Timestamp], intervals_df: pd.DataFrame
    ) -> list[pd.Timestamp]:
        """
...
def test_adaptive_rebalancing():
    ...
    if not intervals_df.empty:
        ...
        logger.info(
            f"- 간격 분포: {intervals_df['rebalance_interval'].value_counts().to_dict()}"
        )
        ...
        logger.info(
            f"- 강한 시그널 비율: {(intervals_df['signal_score'] >= 80).mean():.1%}"
        )
        logger.info(
            f"- 약한 시그널 비율: {(intervals_df['signal_score'] < 60).mean():.1%}"
        )
