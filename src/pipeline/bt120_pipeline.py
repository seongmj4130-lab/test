...
def run_bt120_pipeline(
    ...
    if (artifact_exists(pred_short_path) and 
        ...
        logger.info(f"  ✓ 캐시에서 로드: pred_long {len(artifacts['pred_long_oos']):,}행")
    else:
    ...
    bt_cfg = BacktestConfig(
        ...
        score_col=str(l7_cfg.get("score_col", "score_total_long" if strategy == "long" else "score_ens")),
        ...
        risk_scaling_bear_multiplier=float(l7_cfg.get("risk_scaling_bear_multiplier", 0.7)),
        risk_scaling_neutral_multiplier=float(l7_cfg.get("risk_scaling_neutral_multiplier", 0.9)),
