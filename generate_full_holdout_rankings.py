...
def recalculate_for_single_date(date: str, horizon: str, cfg: dict) -> pd.DataFrame:
    ...
    result_cols.extend(['top1_feature_group', 'top2_feature_group', 'top3_feature_group', 'original_ranking'])

    return merged[result_cols]


def generate_full_holdout_rankings(horizon: str, max_dates: Optional[int] = None) -> pd.DataFrame:
    """Holdout 전체 기간 랭킹 생성"""
...
def create_formatted_rankings(horizon: str, data: pd.DataFrame) -> pd.DataFrame:
    ...
    formatted = pd.DataFrame({
        '랭킹': data['ranking'],
        '종목명(ticker)': data['ticker'].apply(lambda x: f"{ticker_to_name.get(x, 'Unknown')}({x})"),
        '날짜': data['date'],
...
def save_results(horizon: str, formatted_data: pd.DataFrame, unknown_stocks: pd.DataFrame):
    """결과 파일들 저장"""

    # 포맷팅된 랭킹 데이터 저장
    csv_path = project_root / "data" / f"holdout_daily_ranking_{horizon}_top20.csv"
    parquet_path = project_root / "data" / f"holdout_daily_ranking_{horizon}_top20.parquet"
...
def main():
    ...
    try:
        ...
        all_unknown_path = project_root / "data" / "all_unknown_stocks_for_hardcoding.csv"
