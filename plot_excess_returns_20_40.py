...
def load_and_process_data():
    """데이터 로드 및 가공"""
    file_path = Path(__file__).parent / "data" / "dummy_excess_return_monthly_2023_2024_by_horizon_and_rank.csv"
...
def create_comparison_chart(df):
    ...
    fig.suptitle('20일 vs 40일 트렌치 초과수익률 비교\n(KOSPI200 대비)', fontsize=16, fontweight='bold')
    ...
    for idx, horizon in enumerate(horizons):
        ...
        for strategy in ['단기랭킹', '장기랭킹', '통합랭킹(5:5)']:
            ...
            if not strategy_data.empty:
                ax.plot(strategy_data['date'], strategy_data['excess_return_pct'],
                       label=strategy, color=colors[strategy], linewidth=2.5,
...
def create_overlay_chart(df):
    ...
    for s_idx, strategy in enumerate(strategies):
        for h_idx, horizon in enumerate(horizons):
            strategy_data = df[(df['rank_strategy'] == strategy) &
    ...
    ax.set_title('20일 vs 40일 트렌치 초과수익률 오버레이 비교', fontsize=16, fontweight='bold')
    ax.set_xlabel('날짜', fontsize=12)
    ax.set_ylabel('초과수익률 (%)', fontsize=12)
    ax.grid(True, alpha=0.3)
...
def main():
    ...
    chart1_path = Path(__file__).parent / "results" / "excess_returns_20_40_comparison.png"
