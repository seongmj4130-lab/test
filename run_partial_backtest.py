...
def get_strategy_config(cfg: dict, strategy_name: str, holding_days: int) -> dict:
    ...
    if not config_section:
        ...
        logger.info(
            f"✅ {strategy_name} 설정: top_k={config_section.get('top_k')}, cost_bps={config_section.get('cost_bps')}, slippage_bps={config_section.get('slippage_bps')}"
        )
...
def run_single_backtest(cfg: dict, strategy_name: str, holding_days: int) -> dict:
    ...
    try:
        ...
        if len(metrics_df) > 0:
            ...
            logger.warning(
                f"메트릭 데이터가 없습니다: {strategy_name} {holding_days}일"
            )
            return None
...
def run_strategy_batch(cfg: dict, strategy_name: str, holding_days_list: list):
    ...
    if results:
        ...
        output_file = (
            project_root / "results" / f"backtest_{strategy_name}_{timestamp}.csv"
        )
...
def main():
    ...
    if all_results:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file = (
            project_root / "results" / f"backtest_all_strategies_{timestamp}.csv"
        )
        df.to_csv(output_file, index=False)
        ...
        print(
            f"   최고 Sharpe: {df['sharpe'].max():.3f} ({df.loc[df['sharpe'].idxmax(), 'strategy']} {df.loc[df['sharpe'].idxmax(), 'holding_days']}일)"
        )
