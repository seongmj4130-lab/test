...
def calculate_topk_direction_hit_ratio_dev_holdout(
    ...
    for strategy_col in strategies:
        ...
        holdout_ranking = strategy_ranking[strategy_ranking['date'].isin(holdout_test_dates)]
        ...
        for _, row in holdout_ranking.iterrows():
            ...
            # 20일 수익률 우선 사용
            return_col = 'ret_fwd_20d' if 'ret_fwd_20d' in return_cols else return_cols[0]

            # Top-K 종목의 미래 수익률
            top_k_returns = future_returns[future_returns['ticker'].isin(top_k_tickers)][return_col]
...
def analyze_ranking_hit_ratios_dev_holdout():
    ...
    for top_k in top_k_values:
        ...
        for strategy, data in results.items():
            hit_ratio_pct = data['holdout_hit_ratio'] * 100 if not np.isnan(data['holdout_hit_ratio']) else np.nan
            avg_return_pct = data['holdout_avg_return'] * 100 if not np.isnan(data['holdout_avg_return']) else np.nan

            print(f'  {strategy}:')
            print(f'    - 방향적중률: {hit_ratio_pct:.1f}% (샘플: {data["holdout_samples"]}일)')
