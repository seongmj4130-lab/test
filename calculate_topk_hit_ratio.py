...
def calculate_topk_direction_hit_ratio(
    ...
    for date in valid_dates:
        ...
        top_k_tickers = rank_data.nsmallest(top_k, 'rank' if 'rank' in rank_data.columns else 'ranking')['ticker'].tolist()

        # 해당 종목들의 수익률 가져오기
        top_k_returns = return_data[return_data['ticker'].isin(top_k_tickers)][return_col].values
...
def analyze_ranking_hit_ratios():
    ...
    if rebalance_scores_path.exists():
        ...
        if 'score_short' in rebalance_data.columns:
            ...
            ranking_short['rank'] = ranking_short.groupby('date')['score'].rank(method='min', ascending=True)
            ranking_sources.append(('단기랭킹 (rebalance)', ranking_short, 'ret_fwd_20d'))
        ...
        if 'score_long' in rebalance_data.columns:
            ...
            ranking_long['rank'] = ranking_long.groupby('date')['score'].rank(method='min', ascending=True)
            ranking_sources.append(('장기랭킹 (rebalance)', ranking_long, 'ret_fwd_120d'))
        ...
        if 'score_ens' in rebalance_data.columns:
            ...
            ranking_ensemble['rank'] = ranking_ensemble.groupby('date')['score'].rank(method='min', ascending=True)
            ranking_sources.append(('통합랭킹 (rebalance)', ranking_ensemble, 'ret_fwd_20d'))
    ...
    for strategy_name, strategy_data in results.items():
        ...
        for top_k, result in strategy_data.items():
            if 'error' not in result:
                ...
                print(f'  Top-{top_k}: 방향적중률 {hit_ratio:.3f} ({samples}일), '
