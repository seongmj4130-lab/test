...
def calculate_ranking_performance(df_short, df_long, df_returns):
    ...
    # 공통 날짜 필터링
    common_dates = set(df_short['date'].unique()) & set(df_long['date'].unique()) & set(df_returns['date'].unique())
    ...
    print(f'Holdout 기간: {holdout_dates[0]} ~ {holdout_dates[-1]} ({len(holdout_dates)}일)')
    ...
    # 수익률 컬럼 확인
    return_cols = [col for col in df_returns.columns if 'return' in col.lower() or 'ret' in col.lower()]
    ...
    # IC 계산 함수
    def calculate_ic(ranking_df, returns_df, score_col='score', return_col='true_short', top_k=20):
        ...
        for date in ranking_df['date'].unique():
            ...
            # 랭킹과 수익률 매칭
            merged = rank_data.merge(return_data[['ticker', return_col]], on='ticker', how='inner')
    ...
    # 단기 랭킹 IC 계산
    short_ic_values = calculate_ic(df_short_holdout, df_returns_holdout,
                                   score_col='score', return_col=short_return_col)

    # 장기 랭킹 IC 계산
    long_ic_values = calculate_ic(df_long_holdout, df_returns_holdout,
                                  score_col='score', return_col=long_return_col)

    # IC 통계
    if short_ic_values:
        short_ic_mean = sum(short_ic_values) / len(short_ic_values)
        short_ic_std = (sum((x - short_ic_mean)**2 for x in short_ic_values) / len(short_ic_values))**0.5
    ...
    if long_ic_values:
        ...
        long_ic_std = (sum((x - long_ic_mean)**2 for x in long_ic_values) / len(long_ic_values))**0.5
