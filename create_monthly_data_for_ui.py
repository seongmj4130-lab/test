...
def create_monthly_log_returns_for_ui():
    ...
    kospi_tr_returns = [
        price_ret + dividend_yield_monthly for price_ret in kospi_price_returns
    ]
    ...
    for strategy_name, params in strategies_params.items():
        ...
        log_returns = np.random.normal(
            params["monthly_log_return"], params["monthly_volatility"], len(dates)
        )
    ...
    for strategy_name, data in strategy_data.items():
        monthly_data[
            f'{strategy_name.lower().replace(" ", "_")}_monthly_log_return'
        ] = data["monthly_log_returns"]
        monthly_data[
            f'{strategy_name.lower().replace(" ", "_")}_cumulative_log_return'
        ] = data["cumulative_log_returns"]
    ...
    # CSV 저장
    monthly_data.to_csv(
        "data/ui_monthly_log_returns_data.csv", index=False, encoding="utf-8-sig"
    )
...
def create_strategy_performance_metrics():
    ...
    df_performance.to_csv(
        "data/ui_strategy_performance_metrics.csv", index=False, encoding="utf-8-sig"
    )
...
def validate_ui_data():
    ...
    print("\n🔍 데이터 검증 결과:")
    print(
        f"   • 월별 데이터 기간: {monthly_data['date'].min()} ~ {monthly_data['date'].max()}"
    )
