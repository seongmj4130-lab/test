...
def redesign_strategy_params():
    ...
    for strategy in df["strategy"].unique():
        ...
        avg_performance = (
            strategy_data[
                ["CAGR (%)", "sharpe", "MDD (%)", "Total Return (%)", "profit_factor"]
            ].mean()
            / 100
        )  # 백분율로 변환
    ...
    print("\n🎯 업계평균 도달을 위한 파라미터 조정:")
    print(
        "   • 현재 CAGR: {:.3f} → 목표: {:.3f} (차이: {:.3f})".format(
            ens_avg["CAGR (%)"],
            industry_benchmarks["cagr"],
            industry_benchmarks["cagr"] - ens_avg["CAGR (%)"],
        )
    )
    print(
        "   • 현재 Sharpe: {:.3f} → 목표: {:.3f} (차이: {:.3f})".format(
            ens_avg["sharpe"],
            industry_benchmarks["sharpe"],
