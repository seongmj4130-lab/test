...
# 파일 읽고 쓰기
with open("data/daily_all_business_days_long_ranking_top20.csv") as f:
    reader = csv.DictReader(f)

    # 새로운 파일 생성
    with open(
        "data/daily_all_business_days_long_ranking_top20_formatted.csv", "w", newline=""
    ) as out_f:
        fieldnames = [
            "ranking",
            "ticker_formatted",
        ...
        for row in reader:
            ...
            # 종목명 찾기
            company_name = ticker_to_name.get(
                ticker_formatted, f"Unknown({ticker_formatted})"
            )
...
print(
    "포맷팅된 파일이 생성되었습니다: data/daily_all_business_days_long_ranking_top20_formatted.csv"
)
