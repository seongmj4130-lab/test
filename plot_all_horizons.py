...
# 전체 제목
fig.suptitle(
    "2023-2024 전략별 초과 수익률 비교 (20~120일 보유 기간)",
    fontsize=16,
    fontweight="bold",
    y=0.95,
)
...
plt.tight_layout()
plt.savefig(
    "data/excess_return_all_horizons_comparison.png", dpi=300, bbox_inches="tight"
)
plt.show()
...
final_returns = (
    df[df["month"] == "2024-12"]
    .pivot_table(
        values="excess_return_pct", index="horizon_days", columns="rank_strategy"
    )
    .round(2)
)
...
for horizon in all_horizons:
    ...
    print(
        f"{horizon}일: 최고 {best_strategy}({best_return}%), 최저 {worst_strategy}({worst_return}%)"
    )
