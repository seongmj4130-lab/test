...
def plot_strategies_comparison():
    ...
    fig.suptitle('3전략 6구간 월별 누적 수익률 비교 (2023.01 ~ 2024.12)',
    ...
    for idx, (strategy_name, cols) in enumerate(strategies.items()):
        ...
        for i, (col, period, color) in enumerate(zip(cols, periods, period_colors)):
            ax.plot(df['month'], df[col], label=period, color=color, linewidth=2,
                   marker='o', markersize=3, alpha=0.8)

        # KOSPI200 벤치마크 추가
        ax.plot(df['month'], df['kospi200'], label='KOSPI200', color='black',
    ...
    for strategy_name, cols in strategies.items():
        ...
        for period, col in zip(periods, cols):
            ...
            excess = value - kospi
            print(f"  {period}: {value:.1f}% (KOSPI200: {kospi:.1f}%, 초과: {excess:+.1f}%)")
def plot_period_comparison():
    ...
    fig.suptitle('6구간별 3전략 월별 누적 수익률 비교 (2023.01 ~ 2024.12)',
                 fontsize=16, fontweight='bold', y=0.95)
    ...
    for idx, (period, period_label) in enumerate(zip(periods, period_labels)):
        ...
        for strategy_name, color in strategy_colors.items():
            ...
            ax.plot(df['month'], df[col_name], label=strategy_name,
                   color=color, linewidth=2, marker='o', markersize=3)

        # KOSPI200 추가
        ax.plot(df['month'], df['kospi200'], label='KOSPI200',
