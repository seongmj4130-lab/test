...
def calculate_combined_metrics(dev_metrics, holdout_metrics):
    ...
    combined_cagr = (
        (1 + dev_cagr) ** dev_years * (1 + holdout_cagr) ** holdout_years
    ) ** (1 / total_years) - 1
    ...
    combined_sharpe = (
        dev_sharpe * dev_years + holdout_sharpe * holdout_years
    ) / total_years
    ...
    combined_hit_ratio = (
        dev_hit_ratio * dev_trades + holdout_hit_ratio * holdout_trades
    ) / (dev_trades + holdout_trades)
...
print(
    track_b_holdout[["strategy", "sharpe_ratio", "cagr", "mdd"]].to_string(index=False)
)

print("\nTrack B (백테스트 전략) - Combined:")
track_b_combined["달성률"] = "실제 성과"
print(
    track_b_combined[["strategy", "sharpe_ratio", "cagr", "mdd"]].to_string(index=False)
)
