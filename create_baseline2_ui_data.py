...
def create_baseline2_ui_data():
    ...
    monthly_dates = [d.replace(day=1) for d in dates] + [
        pd.to_datetime(holdout_end).replace(day=1)
    ]
    ...
    kospi_tr_returns = np.random.normal(
        base_return, 0.02, n_months
    )  # 약간의 변동성 추가
    ...
    for strategy_key, strategy_name in strategy_names.items():
        try:
            ...
            monthly_data = (
                holdout_data.groupby("year_month")
                .agg({"net_return": "sum", "date": "first"})  # 월별 수익률 합계
                .reset_index()
            )

            # 누적 수익률 계산
            monthly_data = monthly_data.sort_values("date")
            monthly_data["cumulative_return"] = (
                1 + monthly_data["net_return"]
            ).cumprod() - 1
            monthly_data["log_cumulative_return"] = np.log(
                1 + monthly_data["cumulative_return"]
            )

            strategy_data[strategy_name] = monthly_data[
                [
                    "year_month",
                    "date",
                    "net_return",
                    "cumulative_return",
                    "log_cumulative_return",
                ]
            ]
            ...
            strategy_data[strategy_name]["net_return"] = np.random.normal(
                0.008, 0.015, len(kospi_tr_data)
            )
            strategy_data[strategy_name]["cumulative_return"] = (
                np.cumprod(1 + strategy_data[strategy_name]["net_return"]) - 1
            )
            strategy_data[strategy_name]["log_cumulative_return"] = np.log(
                1 + strategy_data[strategy_name]["cumulative_return"]
            )
    ...
    kospi_mdd = np.min(
        kospi_tr_data["kospi_tr_cumulative_return"]
        - np.maximum.accumulate(kospi_tr_data["kospi_tr_cumulative_return"])
    )
    ...
    for strategy_name, strategy_df in strategy_data.items():
        ...
        mdd = np.min(
            ...
            - np.maximum.accumulate(strategy_df["cumulative_return"])
        )
