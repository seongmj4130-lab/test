...
if price_path.exists():
    ...
    kospi_monthly = (
        df_price.groupby("month").agg({"close": ["first", "last"]}).reset_index()
    )
    kospi_monthly.columns = ["month", "first_price", "last_price"]
    kospi_monthly["kospi200_mret_pct"] = (
        kospi_monthly["last_price"] / kospi_monthly["first_price"] - 1
    ) * 100
...
if strategy_path.exists():
    ...
    if dummy_path.exists() and price_path.exists():
        ...
        for horizon in df_dummy["horizon_days"].unique():
            ...
            for month_str in months:
                try:
                    ...
                    if kospi_mask.any():
                        kospi_return = kospi_monthly.loc[
                            kospi_mask, "kospi200_mret_pct"
                        ].iloc[0]
                        df_real.loc[
                            (df_real["month"] == month_str)
                            & (df_real["horizon_days"] == horizon),
        ...
        for horizon in df_dummy["horizon_days"].unique():
            ...
            # 누적 수익률 계산
            horizon_data["kospi200_pr_cum_return_pct"] = (
                1 + horizon_data["kospi200_pr_mret_pct"] / 100
            ).cumprod() - 1
            horizon_data["short_cum_return_pct"] = (
                1 + horizon_data["short_mret_pct"] / 100
            ).cumprod() - 1
            horizon_data["long_cum_return_pct"] = (
                1 + horizon_data["long_mret_pct"] / 100
            ).cumprod() - 1
            horizon_data["mix_cum_return_pct"] = (
                1 + horizon_data["mix_mret_pct"] / 100
            ).cumprod() - 1

            # 백분율로 변환
            for col in [
                "kospi200_pr_cum_return_pct",
                "short_cum_return_pct",
                "long_cum_return_pct",
                "mix_cum_return_pct",
            ]:
            ...
            for idx, row in horizon_data.iterrows():
                mask = (df_real["month"] == row["month"]) & (
                    df_real["horizon_days"] == horizon
                )
                df_real.loc[mask, "kospi200_pr_cum_return_pct"] = row[
                    "kospi200_pr_cum_return_pct"
                ]
