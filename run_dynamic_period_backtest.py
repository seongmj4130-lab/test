...
def get_strategy_config(cfg: dict, strategy_name: str, holding_days: int) -> dict:
    ...
    if redesigned_path.exists():
        ...
        if "params" in redesigned and strategy_name in redesigned["params"]:
            ...
            logger.info(
                f"✅ {strategy_name} 전략에 개선 파라미터 적용: {config_section}"
            )
        else:
            logger.warning(
                f"❌ {strategy_name} 전략 redesigned 파라미터 없음, 기존 설정 사용"
            )
            config_section = {}
    else:
        config_section = {}
    ...
    if not config_section:
        ...
        # regime: semi (중간적 접근)
        config_section["regime"] = {
            "enabled": True
        }  # semi 대신 True로 설정 (config 파싱용)
...
def run_single_backtest(cfg: dict, strategy_name: str, holding_days: int) -> dict:
    ...
    try:
        ...
        backtest_cfg = BacktestConfig(
            ...
            weighting="equal",
            softmax_temp=1.0,
            overlapping_tranches_enabled=strategy_config.get(
                "overlapping_tranches_enabled", False
            ),
            tranche_holding_days=int(
                strategy_config.get("tranche_holding_days", 120) or 120
            ),
            ...
            regime_top_k_bull_strong=strategy_config.get("regime", {}).get(
                "top_k_bull_strong", 10
            ),
            regime_top_k_bull_weak=strategy_config.get("regime", {}).get(
                "top_k_bull_weak", 12
            ),
            regime_top_k_bear_strong=strategy_config.get("regime", {}).get(
                "top_k_bear_strong", 30
            ),
            regime_top_k_bear_weak=strategy_config.get("regime", {}).get(
                "top_k_bear_weak", 30
            ),
            regime_top_k_neutral=strategy_config.get("regime", {}).get(
                "top_k_neutral", 20
            ),
            regime_exposure_bull_strong=strategy_config.get("regime", {}).get(
                "exposure_bull_strong", 1.5
            ),
            regime_exposure_bull_weak=strategy_config.get("regime", {}).get(
                "exposure_bull_weak", 1.2
            ),
            regime_exposure_bear_strong=strategy_config.get("regime", {}).get(
                "exposure_bear_strong", 0.7
            ),
            regime_exposure_bear_weak=strategy_config.get("regime", {}).get(
                "exposure_bear_weak", 0.9
            ),
            regime_exposure_neutral=strategy_config.get("regime", {}).get(
            ...
            regime_exposure_bull=strategy_config.get("regime", {}).get(
                "exposure_bull", 1.0
            ),
            regime_exposure_bear=strategy_config.get("regime", {}).get(
                "exposure_bear", 1.0
            ),
            smart_buffer_enabled=strategy_config.get("smart_buffer_enabled", True),
            smart_buffer_stability_threshold=strategy_config.get(
                "smart_buffer_stability_threshold", 0.7
            ),
            volatility_adjustment_enabled=strategy_config.get(
                "volatility_adjustment_enabled", True
            ),
            volatility_lookback_days=strategy_config.get(
                "volatility_lookback_days", 60
            ),
            target_volatility=strategy_config.get("target_volatility", 0.15),
            volatility_adjustment_max=strategy_config.get(
                "volatility_adjustment_max", 1.2
            ),
            volatility_adjustment_min=strategy_config.get(
                "volatility_adjustment_min", 0.7
            ),
            risk_scaling_enabled=strategy_config.get("risk_scaling_enabled", True),
            risk_scaling_bear_multiplier=strategy_config.get(
                "risk_scaling_bear_multiplier", 0.7
            ),
            risk_scaling_neutral_multiplier=strategy_config.get(
                "risk_scaling_neutral_multiplier", 0.9
            ),
            risk_scaling_bull_multiplier=strategy_config.get(
        ...
        if "phase" in rebalance_scores.columns:
            ...
            logger.info(
                f"HOLDOUT 구간 필터링: 전체 {len(rebalance_scores)} → HOLDOUT {len(holdout_data)} 행"
            )
        ...
        baseline_dir = project_root / "baseline_20260112_145649"
        market_regime_path = (
            baseline_dir / "data" / "interim" / "l1d_market_regime.parquet"
        )
        market_regime = None
        ...
        if len(metrics_df) > 0:
            try:
                ...
                avg_sharpe = (
                    float(metrics_df["net_sharpe"].mean())
                    if "net_sharpe" in metrics_df.columns
                    and not metrics_df["net_sharpe"].isna().all()
                    else 0.0
                )
                avg_cagr = (
                    float(metrics_df["net_cagr"].mean())
                    if "net_cagr" in metrics_df.columns
                    and not metrics_df["net_cagr"].isna().all()
                    else 0.0
                )
                avg_total_return = (
                    float(metrics_df["net_total_return"].mean())
                    if "net_total_return" in metrics_df.columns
                    and not metrics_df["net_total_return"].isna().all()
                    else 0.0
                )
                avg_mdd = (
                    float(metrics_df["net_mdd"].mean())
                    if "net_mdd" in metrics_df.columns
                    and not metrics_df["net_mdd"].isna().all()
                    else 0.0
                )
                avg_calmar = (
                    float(metrics_df["net_calmar_ratio"].mean())
                    if "net_calmar_ratio" in metrics_df.columns
                    and not metrics_df["net_calmar_ratio"].isna().all()
                    else 0.0
                )
                avg_hit_ratio = (
                    float(metrics_df["net_hit_ratio"].mean())
                    if "net_hit_ratio" in metrics_df.columns
                    and not metrics_df["net_hit_ratio"].isna().all()
                    else 0.0
                )
                avg_turnover = (
                    float(metrics_df["avg_turnover_oneway"].mean())
                    if "avg_turnover_oneway" in metrics_df.columns
                    and not metrics_df["avg_turnover_oneway"].isna().all()
                    else 0.0
                )
                avg_profit_factor = (
                    float(metrics_df["net_profit_factor"].mean())
                    if "net_profit_factor" in metrics_df.columns
                    and not metrics_df["net_profit_factor"].isna().all()
                    else 0.0
                )
                avg_trade_duration = (
                    float(metrics_df["avg_trade_duration"].mean())
                    if "avg_trade_duration" in metrics_df.columns
                    and not metrics_df["avg_trade_duration"].isna().all()
                    else 0.0
                )
                ...
                avg_hit_ratio = (
                    avg_turnover
                ) = avg_profit_factor = avg_trade_duration = 0.0
        else:
            avg_sharpe = avg_cagr = avg_total_return = avg_mdd = avg_calmar = 0.0
        ...
        logger.info(
            f"   Sharpe: {result['sharpe']:.4f}, CAGR: {result['cagr']:.4f}, MDD: {result['mdd']:.4f}"
        )
...
def main():
    ...
    # 전략 표시명 매핑
    strategy_names = {"bt20_short": "단기", "bt120_long": "장기", "bt20_ens": "통합"}

    print("📋 테스트 설정:")
    print(
        f"   • 전략: {len(strategies)}개 ({', '.join([strategy_names[s] for s in strategies])})"
    )
    ...
    if successful_results:
        ...
        mdd_pivot = results_df.pivot_table(
            index="strategy_name", columns="holding_days", values="mdd", aggfunc="first"
        ).round(4)
        print(mdd_pivot)
        ...
        output_file = (
            project_root / f"results/dynamic_period_backtest_results_{timestamp}.csv"
        )
        ...
        report_file = (
            project_root
            / f"artifacts/reports/dynamic_period_backtest_report_{timestamp}.md"
        )
    ...
    if error_results:
        ...
        for error_result in error_results:
            print(
                f"   • {error_result['strategy']} {error_result['holding_days']}일: {error_result['error']}"
            )
...
def generate_report(results_df: pd.DataFrame, report_file: Path, strategy_names: dict):
    ...
    with open(report_file, "w", encoding="utf-8") as f:
        ...
        for strategy in results_df["strategy"].unique():
            ...
            f.write(
                f"- **최고 Sharpe**: {best_sharpe['sharpe']:.4f} ({best_sharpe['holding_days']}일)\n"
            )
            f.write(
                f"- **최고 CAGR**: {best_cagr['cagr']:.4f} ({best_cagr['holding_days']}일)\n"
            )
        ...
        # 기간별 평균 성과 계산
        period_avg = (
            results_df.groupby("holding_days")[["sharpe", "cagr", "mdd"]]
            .mean()
            .round(4)
        )
        f.write("### 기간별 평균 성과\n\n")
        ...
        # 전략별 평균 성과 계산
        strategy_avg = (
            results_df.groupby("strategy")[["sharpe", "cagr", "mdd"]].mean().round(4)
        )
        f.write("### 전략별 평균 성과\n\n")
        ...
        f.write("## 📁 결과 파일\n\n")
        f.write(
            f"- **CSV 데이터**: `results/dynamic_period_backtest_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv`\n"
        )
