...
def optimize_ensemble_weights():
    ...
    for i, weights in enumerate(test_weights):
        ...
        print(f"  결과: {test_hit:.1%} (XGBoost:{weights['xgboost']}, Ridge:{weights['ridge']}, Grid:{weights['grid']}, RF:{weights['rf']})")
    ...
    df_optimized['score_ens'] = (
        ...
        best_weights_long['grid'] * df_optimized.get('score_grid', df_optimized['score_short']) +
        best_weights_long['rf'] * df_optimized.get('score_rf', df_optimized['score_short'])
    )
