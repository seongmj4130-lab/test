...
def extract_track_a_metrics_updated():
    ...
    try:
        ...
        with open(
            "artifacts/reports/hit_ratio_optimization_final_summary.md",
            encoding="utf-8",
        ) as f:
            content = f.read()
        ...
        track_a_metrics["bt120_long"] = {
            "hit_ratio_dev": 50.5,
            "hit_ratio_holdout": 49.2,
        }
        track_a_metrics["bt120_ens"] = {
            "hit_ratio_dev": 51.2,
    ...
    try:
        ...
        with open(
            "artifacts/reports/model_overfitting_analysis_report.md",
            encoding="utf-8",
        ) as f:
            content = f.read()
                    ...
                    for j in range(i + 1, min(i + 20, len(lines))):
                        if "Grid Search" in lines[j] and "-0.0400" in lines[j]:
                            ...
                            if len(parts) >= 6:
                                ...
                                track_a_metrics["bt120_long"] = track_a_metrics.get(
                                    "bt120_long", {}
                                )
...
def analyze_overall_performance_updated(track_a, track_b):
    ...
    for strategy in ["bt20_short", "bt20_ens", "bt120_long", "bt120_ens"]:
        if strategy in track_a and strategy in track_b:
            ...
            # 모델링 성과 (Track A)
            hit_ratio_avg = (
                a_data.get("hit_ratio_dev", 0) + a_data.get("hit_ratio_holdout", 0)
            ) / 2
            ...
            # 종합 점수 (단순 평균)
            modeling_score = (
                hit_ratio_avg / 100 + max(0, ic_avg + 0.1)
            ) / 2  # 0-1 스케일
    ...
    sorted_strategies = sorted(
        analysis.items(), key=lambda x: x[1]["overall_score"], reverse=True
    )
...
def create_performance_report_updated(track_a, track_b):
    ...
    report = """# Track A & Track B 성과지표 종합 보고서 (최신 데이터)
...
"""
    ...
    with open(
        "artifacts/reports/track_a_b_performance_analysis_updated.md",
        "w",
        encoding="utf-8",
    ) as f:
        f.write(report)
