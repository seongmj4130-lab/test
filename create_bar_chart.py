...
if perf_path.exists():
    ...
    # KOSPI200 관련 데이터 찾기
    kospi_data = df_perf[df_perf['strategy'].str.contains('kospi|KOSPI|benchmark|BENCHMARK', case=False, na=False)]
    if len(kospi_data) > 0:
...
if returns_path.exists():
    ...
    # 전략별 누적 수익률
    strategies = {
        'bt20_short': df_returns.get('BT20 단기 (20일)', pd.Series([0]*len(df_returns))).sum() * 100,
        'bt120_long': df_returns.get('BT120 장기 (120일)', pd.Series([0]*len(df_returns))).sum() * 100,
        'bt120_ens': df_returns.get('BT120 앙상블 (120일)', pd.Series([0]*len(df_returns))).sum() * 100
    }
    ...
    bars = ax.bar(chart_data.keys(), chart_data.values(),
    ...
    # 그래프 설정
    ax.set_title('Track B 전략 vs KOSPI200 벤치마크 비교\n(홀드아웃 기간: 2023-01-01 ~ 2024-10-31)',
