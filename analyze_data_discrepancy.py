...
def analyze_data_discrepancy():
    ...
    print("3. 데이터 기간 비교:")
    print(
        f'누적 수익률 데이터 기간: {cumulative_df["date"].min()} ~ {cumulative_df["date"].max()}'
    )
    ...
    print("🔍 주요 발견사항:")
    print(
        "1. holdout_performance_metrics.csv와 backtest_performance_metrics.csv는 일치"
    )
    print(
        "2. strategies_kospi200_monthly_cumulative_returns.csv는 다른 데이터 소스 사용"
    )
