...
def analyze_hit_ratio_period():
    ...
    if rebalance_path.exists():
        ...
        if 'score_short' in df_rebalance.columns:
            short_dates = df_rebalance[df_rebalance['score_short'].notna()]['date'].unique()
            print(f'   - 단기랭킹 기간: {min(short_dates)} ~ {max(short_dates)} ({len(short_dates)}일)')

        if 'score_long' in df_rebalance.columns:
            long_dates = df_rebalance[df_rebalance['score_long'].notna()]['date'].unique()
            print(f'   - 장기랭킹 기간: {min(long_dates)} ~ {max(long_dates)} ({len(long_dates)}일)')

        if 'score_ens' in df_rebalance.columns:
            ens_dates = df_rebalance[df_rebalance['score_ens'].notna()]['date'].unique()
            print(f'   - 통합랭킹 기간: {min(ens_dates)} ~ {max(ens_dates)} ({len(ens_dates)}일)')
    ...
    print('\n=== 결론 ===')
    print('Top-K 방향 적중률은 2016년 5월 ~ 2024년 11월까지의 104일 데이터를 기반으로 계산되었습니다.')
