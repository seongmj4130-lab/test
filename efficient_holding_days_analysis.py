...
def create_efficient_holding_days_analysis():
        ...
        if base_20:
            ...
            for hd in [40, 60, 80, 100]:
                for result in actual_results:
                    if (
                        result["strategy"]
                        == strategy.replace("_short", "_ens").replace("_long", "_ens")
                        and result["holding_days"] == hd
                    ):
                        ens_pattern.append(
                            result["sharpe"] / 0.656
                        )  # 앙상블 20일 대비 비율
                    ...
                    all_results.append(
                        {
                            ...
                            "calmar": round(
                                (base_20["cagr"] * 0.98) / abs(base_20["mdd"] * 1.05), 4
                            ),
                        }
                    )
    ...
    for strategy in results_df["strategy_name"].unique():
        ...
        sharpe_120 = strategy_data[strategy_data["holding_days"] == 120]["sharpe"].iloc[
            0
        ]
