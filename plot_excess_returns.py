...
for i, horizon in enumerate([20, 40]):
    ...
    # 그래프 설정
    ax.set_title(
        f"{horizon}일 보유 기간 전략별 초과 수익률", fontsize=14, fontweight="bold"
    )
...
# 전체 제목
fig.suptitle(
    "2023-2024 전략별 초과 수익률 비교 (20일 vs 40일 보유)",
    fontsize=16,
    fontweight="bold",
    y=0.98,
)
...
plt.tight_layout()
plt.savefig(
    "data/excess_return_20_40_days_comparison.png", dpi=300, bbox_inches="tight"
)
plt.show()
