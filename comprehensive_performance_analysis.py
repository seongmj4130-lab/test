...
def analyze_performance_metrics(df):
    ...
    sharpe_analysis = df.groupby('strategy')['sharpe'].agg(['mean', 'max', 'min', 'std']).round(3)
    print("전략별 Sharpe 통계:")
    ...
    def classify_sharpe(x):
        if x >= 1.0: return "⭐ 매우 우수"
        elif x >= 0.5: return "✅ 우수"
        elif x >= 0.0: return "⚠️ 보통"
        elif x >= -0.5: return "❌ 저조"
        else: return "💀 매우 저조"
    ...
    cagr_analysis = df.groupby('strategy')['cagr(%)'].agg(['mean', 'max', 'min']).round(2)
    print("전략별 CAGR 통계:")
    print(cagr_analysis)

    # CAGR 등급 분류
    def classify_cagr(x):
        if x >= 10: return "⭐ 매우 우수"
        elif x >= 5: return "✅ 우수"
        elif x >= 0: return "⚠️ 보통"
        elif x >= -5: return "❌ 저조"
        else: return "💀 매우 저조"
    ...
    calmar_analysis = df.groupby('strategy')['calmar'].agg(['mean', 'max', 'min']).round(3)
    print("전략별 Calmar 통계:")
    ...
    hit_analysis = df.groupby('strategy')['hit_ratio(%)'].agg(['mean', 'max', 'min']).round(1)
    ...
    turnover_analysis = df.groupby('strategy')['avg_turnover'].agg(['mean', 'max', 'min']).round(3)
    print("전략별 Turnover 통계:")
    print(turnover_analysis)

    # Turnover 등급 (낮을수록 좋음)
    def classify_turnover(x):
        if x <= 0.2: return "⭐ 매우 효율적"
        elif x <= 0.4: return "✅ 효율적"
        elif x <= 0.6: return "⚠️ 보통"
        elif x <= 0.8: return "❌ 비효율적"
        else: return "💀 매우 비효율적"

    df['turnover_grade'] = df['avg_turnover'].apply(classify_turnover)
    print("\nTurnover 등급 분포:")
    turnover_grades = df.groupby(['strategy', 'turnover_grade']).size().unstack(fill_value=0)
    ...
    pf_analysis = df.groupby('strategy')['profit_factor'].agg(['mean', 'max', 'min']).round(3)
    print("전략별 Profit Factor 통계:")
    print(pf_analysis)

    # Profit Factor 등급 (1 이상이면 수익)
    def classify_pf(x):
        if x >= 2.0: return "⭐ 매우 우수"
        elif x >= 1.5: return "✅ 우수"
        elif x >= 1.0: return "⚠️ 수익"
        elif x >= 0.8: return "❌ 손실"
        else: return "💀 큰 손실"
...
def create_strategy_comparison(df):
    ...
    print(f"최고 Sharpe: {best_sharpe['strategy']} {best_sharpe['holding_days']}일 - {best_sharpe['sharpe']:.3f}")
    print(f"최고 CAGR: {best_cagr['strategy']} {best_cagr['holding_days']}일 - {best_cagr['cagr(%)']:.1f}%")
    print(f"최고 Calmar: {best_calmar['strategy']} {best_calmar['holding_days']}일 - {best_calmar['calmar']:.3f}")
...
def create_period_analysis(df):
    ...
    # 기간별 평균
    period_avg = df.groupby('holding_days').agg({
        'sharpe': ['mean', 'max', 'min'],
        'cagr(%)': 'mean',
        'mdd(%)': 'mean',
        'hit_ratio(%)': 'mean'
...
def create_practical_insights(df):
    ...
    # 투자 가능 전략 식별
    investable = df[(df['sharpe'] > 0) & (df['cagr(%)'] > 0) & (df['profit_factor'] > 1)]
    print(f"✅ 투자 가능 케이스: {len(investable)}/{len(df)} ({len(investable)/len(df)*100:.1f}%)")

    if len(investable) > 0:
        print("\n투자 가능 전략:")
        for _, row in investable.iterrows():
            print(f"{row['strategy']} {row['holding_days']}일: Sharpe {row['sharpe']:.3f}, CAGR {row['cagr(%)']:.1f}%")
    # 전략별 강점 분석
    print("\n🎯 전략별 강점 분석:")
    ...
    if len(profitable) > 0:
        ...
        for _, row in profitable.iterrows():
            print(f"{row['strategy']} {row['holding_days']}일: Sharpe {row['sharpe']:.3f}, Turnover {row['avg_turnover']:.3f}")
    return investable
...
def save_analysis_report(df, investable):
    ...
    output_file = project_root / 'results' / f'comprehensive_performance_analysis_{timestamp}.csv'
    ...
    # 투자 가능 전략만 별도 저장
    if len(investable) > 0:
        investable_file = project_root / 'results' / f'investable_strategies_{timestamp}.csv'
