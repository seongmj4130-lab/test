...
def create_strategies_cumulative_comparison_chart(df, output_path):
    ...
    # KOSPI TR 먼저 그리기
    plt.plot(df['year_month'], df['kospi_tr_cumulative_log_return'],
             label='KOSPI TR', color=colors['KOSPI TR'], linewidth=3, alpha=0.9)
    ...
    for col, display_name in strategy_mapping.items():
        plt.plot(df['year_month'], df[col],
                 label=display_name, color=colors[display_name], linewidth=2.5, alpha=0.9)

    # 0선 추가
    plt.axhline(y=0, color='black', linestyle='--', alpha=0.8, linewidth=1)

    # 그래프 설정
    plt.title('전략별 누적 로그 수익률 비교 (2023-2024)', fontsize=16, fontweight='bold')
...
def create_performance_summary(df):
    ...
    # CSV로 저장
    summary_df.to_csv('results/strategies_cumulative_performance_summary.csv', index=False, encoding='utf-8-sig')
...
def create_comparison_table(df):
    ...
    for col, name in strategy_names.items():
        ...
        end_val = df[col].iloc[-1]
        total_return = end_val - start_val
        max_drawdown = start_val - min_val if (min_val := df[col].min()) < start_val else 0
