...
def create_formatted_ranking_files():
    ...
    # 장기 파일 처리
    print("=== 장기 랭킹 파일 생성 중 ===")
    long_file = project_root / "data" / "daily_all_business_days_long_ranking_top20_formatted.csv"
    long_df = pd.read_csv(long_file)

    # 필요한 컬럼들만 선택 및 재구성
    long_result = pd.DataFrame({
        '랭킹': long_df['ranking'],
        '종목명(ticker)': long_df['company_name'].astype(str) + '(' + long_df['ticker_formatted'].astype(str) + ')',
        '날짜': long_df['date'],
    ...
    # 단기 파일 처리
    print("\n=== 단기 랭킹 파일 생성 중 ===")
    short_file = project_root / "data" / "daily_all_business_days_short_ranking_top20_formatted.csv"
    short_df = pd.read_csv(short_file)

    # 필요한 컬럼들만 선택 및 재구성
    short_result = pd.DataFrame({
        '랭킹': short_df['ranking'],
        '종목명(ticker)': short_df['company_name'].astype(str) + '(' + short_df['ticker_formatted'].astype(str) + ')',
        '날짜': short_df['date'],
