...
def create_baseline2_simple():
    ...
    for strategy in strategies:
        ...
        performance_metrics[strategy.replace('bt20_단기', 'BT20 단기').replace('bt20_앙상블', 'BT20 앙상블').replace('bt120_장기', 'BT120 장기').replace('bt120_앙상블', 'BT120 앙상블')] = {
    ...
    for name, metrics in performance_metrics.items():
        print(f"• {name}: 총수익률 {metrics['총수익률']:.1%}, Sharpe {metrics['Sharpe']:.2f}")
