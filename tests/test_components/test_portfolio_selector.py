...
class TestSelectTopKWithFallback:
    ...
    def test_select_basic_topk(self, sample_rankings):
        """기본적인 top-k 선택 테스트"""
        selected_df, diagnostics = select_topk_with_fallback(sample_rankings, top_k=3)
    ...
    def test_select_more_than_available(self, sample_rankings):
        ...
        selected_df, diagnostics = select_topk_with_fallback(
            sample_rankings, top_k=requested_k
        )
    ...
    def test_select_with_required_cols_filtering(self, sample_rankings):
        ...
        selected_df, diagnostics = select_topk_with_fallback(
            df_with_missing, top_k=3, required_cols=["score"]
        )
    ...
    def test_select_with_prev_holdings_buffer(self, sample_rankings):
        ...
        selected_df, diagnostics = select_topk_with_fallback(
            sample_rankings, top_k=3, buffer_k=2, prev_holdings=prev_holdings
        )
    ...
    def test_select_with_group_constraints(self, sample_rankings):
        ...
        selected_df, diagnostics = select_topk_with_fallback(
            df_with_groups, top_k=4, group_col="sector", max_names_per_group=2
        )
    ...
    def test_select_empty_dataframe(self, sample_data_empty):
        """빈 DataFrame 입력 테스트"""
        selected_df, diagnostics = select_topk_with_fallback(sample_data_empty, top_k=5)
    ...
    def test_select_single_row(self):
        """단일 행 데이터 테스트"""
        single_row_df = pd.DataFrame({"ticker": ["TEST"], "score": [0.8], "rank": [1]})

        selected_df, diagnostics = select_topk_with_fallback(single_row_df, top_k=5)
    ...
    def test_select_with_price_filtering(self):
        ...
        selected_df, diagnostics = select_topk_with_fallback(
            df_with_price, top_k=3, filter_missing_price=True
        )
    ...
    def test_select_with_suspended_filtering(self):
        ...
        selected_df, diagnostics = select_topk_with_fallback(
            df_with_suspended, top_k=3, filter_suspended=True
        )
    ...
    def test_select_edge_case_zero_topk(self, sample_rankings):
        """top_k가 0인 경우 테스트"""
        selected_df, diagnostics = select_topk_with_fallback(sample_rankings, top_k=0)
