...
from src.utils.validate import validate_df, ValidationError, ValidationResult
...
class TestValidateDf:
    ...
    def test_validate_duplicate_keys(self, sample_data_basic):
        """중복 키 검증 테스트"""
        # 중복 데이터 생성
        duplicate_df = pd.concat([sample_data_basic, sample_data_basic.head(1)], ignore_index=True)
    ...
    def test_validate_optional_columns_exclusion(self, sample_data_with_missing):
        ...
        non_optional_missing = {
            k: v for k, v in missing_stats.items()
            if k not in ["price", "volume"]
        }
