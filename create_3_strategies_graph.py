...
for i, (strategy, label) in enumerate(zip(strategies_3, labels_3)):
    plt.plot(df_pct.index, df_pct[strategy] * 100,
             label=label, linewidth=3, color=colors[i],
             marker='o' if i < 2 else 's', markersize=5, alpha=0.8)

plt.title('3가지 주요 전략 누적 수익률 비교 (2023-2024)', fontsize=18, fontweight='bold', pad=20)
